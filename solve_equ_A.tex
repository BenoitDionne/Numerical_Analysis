\chapter{Iterative Methods to Solve Nonlinear Equations}
\label{chaptSeqA}
...
used to justify the numerical methods presented in these lecture notes.

\begin{theorem}
If $\DS \{x_n\}_{n=0}^\infty$ is a bounded and increasing
...
$\DS f^{(n+1)}(x)$ exists for all $x \in ]a,b[$, and that
...
$\DS \left\{ [a_n,b_n] \right\}_{n=0}^\infty$ of decreasing
...
We have $\DS b_0 - a_0 = b - a = (b-a)/2^0$.  Hence, the interval
$[a_0,b_0]$ is of length $\DS (b-a)/2^0$.

Suppose that the interval $[a_n,b_n]$ is of length $\DS (b-a)/2^n$;
namely, $\DS b_n-a_n = (b-a)/2^n$.  Since $[a_{n+1},b_{n+1}]$ is
...
By induction, we then have that $[a_n,b_n]$ is of length
$\DS (b-a)/2^n$ for all $n\geq 0$.
\end{proof}

\begin{cor}
In the algorithm for the bisection method,, the approximation $x_{n}$
is within $\DS (b-a)/2^n$ of a root $r$ of $f$ in the
interval $[a,b]$.
\label{bisectCor}
\end{cor}
...
satisfies $\DS |x_n - r | < (b_{n-1} - a_{n-1})/2 = (b-a)/2^n$
according to Proposition~\ref{bisecProp1}.
\end{proof}
...
$\DS \{a_n\}_{n=0}^\infty$ is an increasing and bounded sequence.  It
follows from Theorem~\ref{Th0} that $\DS \{a_n\}_{n=0}^\infty$
converges.  Let $\alpha$ be this limit.

Similarly, since $b_0 \geq b_1 \geq b_2 \geq \ldots \geq a$, the sequence
$\DS \{b_n\}_{n=0}^\infty$ is a decreasing and bounded sequence.  Thus
$\DS \{b_n\}_{n=0}^\infty$ converges.  Let $\beta$ be this limit.
...
theorem that $\DS \{x_n\}_{n=0}^\infty$ also converge to
$\alpha = \beta$.
...
when the length of the interval is less than $\DS 10^{-2}$.
Find a bound on the absolute error.

The question is to find the positive root of
$\DS f(x) = x^2 -2 = 0$.  Let
...
$\DS (1.421875 - 1.4140625)/2 = 1/2^8 = 0.00390625$ is an
...
absolute error lest than $\DS 10^{-4}$\ ?

We choose $n$ such that
$\DS |b_n- a_n| = (b-a)/2^n < 10^{-4}$.  This is
$\DS 2^{-n}< 10^{-4}$.  Thus,
\[
...
$\DS \{x_n\}_{n=0}^\infty$ defined by $x_{n+1} = g(x_n)$ for
$n \geq 0$ is a well defined sequence in $[a,b]$.  Hence,
\[
\begin{split}
...
\begin{egg}
Find an approximation to a root of $\DS f(x)=x^3+9\,x-9$.

Because $f(0)\,f(1)=-9<0$, the function $f$ has a root between $0$ and
$1$.  In Example~\ref{fixit}. we saw that $\DS f(x)=x^3+9\,x-9=0$ is
equivalent to $\DS g(x)=(9-x^3)/9 =x$.  Thus, the problem is to
approximate a fixed point of $g$ in $[0,1]$.

We show that $g$ on the interval $[0,1]$ satisfies the hypotheses of
the Fixed Point Theorem,  Because
$\DS g'(x) = -x^2/3 < 0$ for all $x>0$,
...
Theorem is satisfied with $K=1/3$ because
$\DS |g'(x)|=|-x^2/3|\leq1/3$ for all $x$ in $[0,1]$.
...
\begin{egg}
Suppose that we want to approximate a root of
$\DS f(x) = x^3+4\,x^2-10$.
...
The idea is to construct a sequence $\DS \{x_i\}_{i=0}^\infty$ which
...
$\DS g(x)=x - \frac{f(x)}{f'(x)}$.  If $p$ is a fixed point
of $g$, then $\DS p = p - \frac{f(p)}{f'(p)}$ and we get
$f(p) = 0$.
\end{rmk}
...
positive root of $\DS f(x) = x^2 - 2$.  We have
\[
x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}
...
between two consecutive iterations is smaller than $\DS 10^{-10}$.
...
sequence $\DS \{x_i\}_{i=0}^\infty$ that converges to a root $p$ of
...
\[
F(x,y) = \begin{cases}
\DS \frac{f(x)-f(y)}{x-y} & \quad \text{if} \quad x\neq y \\
f'(x) & \quad \text{if} \quad x = y
\end{cases}
\]
The point $p$ is a root of $f$ if and only if
$\DS \begin{pmatrix} p \\ p \end{pmatrix}$ is a fixed point
of $g$,  The sequence $\DS \{x_n\}_{n=0}^\infty$ converges
to a root $p$ of $f$ if and only if the sequence
$\DS \left\{ \begin{pmatrix} x_n \\
x_{n+1} \end{pmatrix}\right\}_{n=0}^\infty$ 
converges to a fixed point
$\DS \begin{pmatrix} p \\ p \end{pmatrix}$  of $g$.

We will study the fixed point method in $\DS \RR^n$ in
Chapter~\ref{chaptSeqD}.
\label{secantFP}
\end{rmk}
...
\begin{defn}
Suppose that the sequence $\DS \{x_n\}_{n=0}^\infty$ converges to $p$.
Let $e_n = x_n - p$.  We say that $\DS \{x_n\}_{n=0}^\infty$
...
$x_0 \in [p-\delta, p + \delta]$, the sequence
$\DS \{x_n\}_{n=0}^\infty$ produced by the
...
that is, $\DS f(p) = f'(p) = \ldots = f^{(k-1)}(p) = 0$
and $\DS f^k(p) \neq 0$. 
Instead of (\ref{formula2}), one uses the fixed point method
with the function
\[
g(x) = \begin{cases}
\DS x - \frac{k\,f(x)}{f'(x)} & \quad \text{if} \quad x \neq p \\
...
$\DS f(x) = (x-p)^k\,q(x)$ for some function $q$ such that
$q(p) \neq 0$. Hence, for $x \neq p$,
\[
...
sequence $\DS \{x_n\}_{n=0}^\infty$ produced by
...
\section{Aitken's $\DS \Delta^2$ Process and Steffensen's Algorithm}

Suppose that $p_0$ is an initial approximation for a fixed point $p$
of a function $g$.  Moreover, suppose that the sequence
$\DS \{p_n\}_{n=0}^\infty$ defined by $p_{n+1} = g(p_n)$
for $n \geq 0$ converges linearly to $p$.  We give a procedure to build a new
sequence $\DS \{\hat{p}_n\}_{n=0}^\infty$ that converges
``faster'' to $p$ than $\DS \{p_n\}_{n=0}^\infty$.
...
The sequence $\DS \{\hat{p}_n\}_{n=0}^\infty$ defined by
...
$\DS \{\hat{p}_n\}_{n=0}^\infty$ to $p$ is greater than 1.
The procedure used to construct $\DS \{\hat{p}_n\}_{n=0}^\infty$ is
called the
{\bfseries Aitken's $\DS \Delta^2$ process}\index{Aitken's
$\Delta^2$ process}.
...
$\DS \{\hat{p}_n\}_{n=0}^\infty$ defined by (\ref{formula4})
converges to $p$ of order two as $n$ goes to $\infty$.
\end{theorem}
...
G(x) =
\begin{cases}
\DS x - \frac{(g(x)-x)^2}{g(g(x)) -2g(x) + x} & \quad
\text{if} \quad x \neq p \\
p & \quad \text{if} \quad x = p
\end{cases}  \qedhere
\]
...
Let
$\DS p(x) = a_m\,x^m + a_{m-1}\,x^{m-1} + \ldots + a_1\,x + a_0$ and
\[
\begin{split}
b_m &= a_m \\
...
$\DS q(x) = b_m\,x^{m-1} + b_{m-1}\,x^{m-2} + \ldots + b_2\,x + b_1$.
...
To evaluate a polynomial $\DS p(x) = \sum_{i=0}^n a_i x^i$
...
sequence $\DS \{x_n\}_{n=0}^\infty$ that converges to $c$.
A bad choice for
...
\]
where only the coefficient of $\DS x^2$ has be changed from
$13132$ to $13133$, has the roots (rounded to seven decimals) $1.0013976$,
...
\begin{egg}
Let $\DS p(x) = x^7 - 28x^6 +322x^5 - 1960x^4 + 6769x^3
...
\begin{egg}
Let $\DS p(x) = 5 -3x -4x^2+x^4$.  Approximate all the roots
of $p$ within $\DS 10^{-10}$.
...
interval) of $\DS 4x^2 - e^x = 0$.  Do not forget to justify
your answer.
\end{question}

\sol{\SOL}{
We will roughly sketch the graph of $\DS f(x) = 4x^2 - e^x$.
This is not easy because even the critical points are hard to find.
However, it is easier to determine the intervals of concavity of the function.
Since $\DS f''(x) = 8 -e^x$, a potential point of inflection is
...
correct to within $\DS 10^{-4}$.
\end{question}

\sol{\SOL}{
$\sqrt[3]{25}$ is the unique root of $\DS f(x) = x^3 -25$.
...
$\sqrt[3]{25}$ with an accuracy of $\DS 10^{-4}$, we need to
select the number of iterations $n$ such that
\[
\frac{3-2}{2^n} < 10^{-4}
\Rightarrow 10^4 < 2^n
...
\end{equation}
Show that the $\DS n^{th}$ iteration $x_n$ of the bisection method 
...
\begin{question}
In the algorithm for the bisection method, Algorithm~\ref{BisectAlgo},
show that $\DS | x_n - x_{n+1}| = 2^{-n-1}(b_0 - a_0)$.
\end{question}
...
$\DS \{a_n\}_{n=0}^\infty$, none of $a_n$ or $b_n$ for
...
$\DS \{b_n - a_n \|_{n=0}^\infty$ cannot converge to $0$.
This is our contradiction.
}

\begin{question}
Find a solution accurate to within $\DS 10^{-4}$ for $x=\tan(x)$ on
$\pi/2 < x < 3 \pi/2$.\\
...
\begin{question}
Find the solutions accurate to within $\DS 10^{-5}$ for
$\DS x^2 + 11\cos(x)=0$.\\
\subQ{a} Use the bisection method.\\
\subQ{b} Use the fixed point method.\\
\subQ{c} Use Newton's method.
...
$\DS y=2x/(1+x^2)$ (in blue) on the same coordinate system.
Note that the two functions are odd.  There is only one intersection for $x>0$
...
accuracy of at least $\DS 10^{-12}$.
}

\begin{question}
For which functions $f$ is the iterative equation
$\DS  x_{n+1} = 2x_n - C x_n^2$
...
\begin{question}
If $x_0 = 0$ and
$\DS x_{n+1} = x_n - \frac{\tan(x_n) - 1}{\sec^2(x_n)}$
...
because:
\begin{enumerate}
\item $\DS f'(x) = \sec^2(x) > 1$ for
$-\pi/2 < x < \pi/2$ with $x\neq 0$ and $f'(0) = 1$,
\item $\DS f''(x) = 2\sec^2(x)\tan(x) \geq 0$ for
$0 \leq x < \pi/2$ and $f''(x) \leq 0$ for $-\pi/2<x\leq 0$, and
\item $\DS \lim_{x\to -pi/2^-} f(x) = -\infty$
and $\DS \lim_{x\to pi/2^+} f(x) = \infty$.
\end{enumerate}
...
root of $\DS f(x) = e^x - \tan(x)$ with an accuracy of
$\DS 10^{-8}$.\\
Hint: To choose $x_0$ and $x_1$, draw the graph of $\DS e^x$
and $\tan(x)$.
\end{question}
...
intersection of $\tan(x)$ (in black) and $\DS e^x$ (in blue) in the
following figure.
...
$\DS \{x_n\}_{n=0}^\infty$ converging to a root $r$ of a function $f$. 
Let $e_n = x_n-r$.  Show that
\[
e_{n+1} = \frac{f''(\xi_n)}{2f'(x_n)}\, e_n^2
\]
for some $\xi_n$ between $r$ and $x_n$.\\
\subQ{b} Let $\DS f(x) = x-e^{-x}$ and assume that the
Newton's method is used to generate a sequence
$\DS \{x_n\}_{n=0}^\infty$ converging to the
...
an accuracy of $\DS 10^{-5}$; namely, such that
$\DS |e_n| < 10^{-5}$.
\end{question}
...
$\DS f(x) = p(x) + \frac{1}{2}\,f''(\xi_n)(x-x_n)^2$
...
\subQ{b} We may assume that $x_n\geq 0$ for all $n$ if $x_0\geq 0$,
because
\[
x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}
...
$\DS f'(x) = 1 + e^{-x}$ and $\DS f''(x) = -e^{-x}$.  Hence,
\[
...
three roots of $\DS f(x) = x^3 -x$; namely, to approximate $p_1=-1$,
...
Since $\DS g'(x) = -2x/(1+x^2)^2 < 0$ for $x>0$, 
...
maximum of $\DS G(x) = |g'(x)| = 2x/(1+x^2)^2$ on $[0,1]$ is
less than $1$ and use this maximum as our constant $K$ as explained in
...
\begin{question}
Consider the function $\DS g(x)=2^{-x}$.\\
...
Theorem, we will have that the sequence $\DS \{x_n\}_{n=0}^\infty$
generated by $x_{n+1} = g(x_n)$ for $n\geq 0$ and $x_0\in [1/3,1]$
converges to the unique fixed point $p$ of $g$ in the interval $[1/3,1]$.
\begin{enumerate}
\item Since $\DS g'(x) = -2^{-x}\ln(2)<0$ for all $x$, the
function $g$ is strictly decreasing on $[1/3,1]$.  Thus,
\[
1 > 2^{-1/3} = g(1/3) \geq g(x) \geq g(1) = 0.5 
\]
for all $x \in [1/3,1]$.  Hence, $g:[1/3,1]\rightarrow [1/3,1]$.
\item Since $\DS |g'(x)| = 2^{-x}\ln(2) < 2^{-1/3}\ln(2)$ for
$x \in [1/3,1]$.  We have that 
$|g(x)-g(y)| \leq K |x-y|$ for all $x,y\in [1/3,1]$ with
$\DS K= 2^{-1/3}\ln(2) < 1$ according to Remark~\ref{FxPtThDer}.
\end{enumerate}

\subQ{b} Since $\DS |x_n - p | \leq \frac{K^n}{1-K} |x_1-x_0|$,
...
until $\DS |x_{n+1} - x_n| < 10^{-4}$.  The first time that
this happen is for $n=10$.  We get $x_{11} \approx 0.64120525$ as an
approximation of the fixed point of $g$ in the interval $[1/3,1]$.
}

\begin{question}
Consider the function $\DS g(x) = 12 - \frac{20}{x}$.\\
...
the sequence $\DS \{x_n\}_{n=0}^\infty$ generated by
...
Point Theorem, we will have that the sequence
$\DS \{x_n\}_{n=0}^\infty$
...
\subQ{d} Since $\DS g'(x) = 20/x^2 > 0$ for $x\in [9.5,11.5]$, we
...
\]
for $n=-1$, $0$, $1$, \ldots until
$\DS |\hat{x}_{n+1} - \hat{x}_n|< 10^{-7}$.
We get $\DS |\hat{x}_{n+1} - \hat{x}_n|<10^{-7}$ for the
first time with $n=1$.  We have $\hat{x}_2 = 10$ to 16 significant digits.
...
$\DS \{x_n\}_{n=0}^\infty$ generated by $x_{n+1} = g(x_n)$
...
$\DS f(1) = e - 3 < 0 < f(2) = e^2-5$, it follows from the
Intermediate Value Theorem that $f$ has at least one root in the
interval $[1,2]$.  To prove that it is unique, we note that
$\DS f'(x) = e^x-2>0$ for all
...
Point Theorem.\\
\subQ{a} Show that $\sqrt[3]{25}$ is the unique root of
$\DS f(x) = x^3-25$.\\
\subQ{b} Show that $p>0$ is a root of $f$ if and only if $p$ is a
fixed point of $\DS g(x) = 5/\sqrt{x}$, and conclude that
...
the $\DS (n+1)^{th}$ term in the sequence
$\DS \{x_n\}_{n=0}^\infty$ produced by
the Fixed Point Theorem applied to the function $g$, is an approximation of 
$p$ with an accuracy of $\DS 10^{-5}$.\\ 
\subQ{e} Use the Fixed Point Theorem to find an approximation $x_n$ to
the fixed point of $g$ in the interval $[a,b]$ such that
$\DS |x_n - x_{n-1}| < 10^{-5}$.  Use the $x_0$ that you
have chosen in (d).
\end{question}

\sol{\SOL}{
\subQ{a} $\sqrt[3]{25}$ is obviously the unique root of
$\DS f(x) = x^3 -25$ because
\[
...
that the sequence $\DS \{x_n\}_{n=0}^\infty$ generated by
...
\item Since $\DS |g'(x)| = \frac{5}{2x^{3/2}} \leq \frac{5}{2^{5/2}}$
for $x\in[2,4]$.  We have that $|g(x)-g(y)| \leq K |x-y|$ for all
$x,y\in [1,2]$ with $\DS K= \frac{5}{2^{5/2}} < 1$
according to Remark~\ref{FxPtThDer}.
\end{enumerate}

\subQ{d} Since $\DS |x_n - p | \leq \frac{K^n}{1-K} |x_1-x_0|$,
we need to find $n$ such that
\[
...
until $\DS |x_n - x_{n-1}| < 10^{-5}$.  The first time that
this happen is for $n=15$.  We get $x_{15} \approx 2.924015$ as an
...
of $\DS 10^{-5}$ if we start with $x_0 = 3$.  The formula
used in (d) to estimate $n$ will generally give a large
overestimation.  However, we have to keep in mind that the value $n$
obtained in (d) is valid for all $x_0$, not just for $x_0 = 3$.
}

\begin{question}
Let $\DS f(x) = e^{2-x} -x^2$.\\
...
of $n$ such that $x_n$, the $\DS (n+1)^{th}$ term in the sequence
$\DS \{x_n\}_{n=0}^\infty$ produced by the Fixed Point
Theorem applied to the function $g$, is an approximation of $p$ with
an accuracy of $\DS 10^{-5}$.\\
...
and $3\pi/2$.  Let $\DS g(x) = \pi + \arctan(x)$.\\
...
\subQ{b} We have $\DS g'(x) = 1/(1+x^2)$.  Hence $g$ is strictly
increasing on $[\pi,3\pi/2]$ and
$\DS |g'(x)| \leq g'(\pi) = 1/(1+\pi^2) < 1$ on
...
\item With $\DS K=1/(1+\pi^2)$, we have
\[
|g'(x)| \leq K <1
\]
...
$\DS \{x_i\}_{i=0}^\infty$ defined by $x_0\in [\pi,3\pi/2]$ and
...
\end{equation}
Given $x_0>0$, let $\DS \{x_n\}_{n=0}^{\infty}$ be the
...
the sequence $\DS \{x_n\}_{n=0}^{\infty}$ converges to the
...
both sides, we get $p = a/p$.  Thus $\DS p^2=a$ or
$p=\sqrt{a}$ since we assume that $a>0$.

\subQ{b} Suppose that $x>0$.  Since $\DS (x-\sqrt{a})^2 >0$, we get
that $\DS x^2 -2\sqrt{a} x + a >0$.  Thus
$\DS x^2 + a > 2\sqrt{a} x$ and,
...
sequence $\DS \{x_n\}_{n=0}^{\infty}$ converge to this fixed
...
the sequence $\DS \{x_n\}_{n=0}^{\infty}$
converges to the fixed point $\sqrt{a}$ whatever $x_0>0$
because the sequences
$\DS \{x_n\}_{n=0}^{\infty}$ and
$\DS \{x_n\}_{n=1}^{\infty}$ have the same limit.
}
...
$\DS \{x_n\}_{n=0}^\infty$ generated by the iteration
...
$g(m)=m$, prove or disprove that the sequence
$\DS \{x_n\}_{n=0}^\infty$
...
$\DS \{x_n\}_{n=0}^\infty$ does not converge to $p$.  If there are
sequences $\DS \{x_n\}_{n=0}^\infty$ that converge to $p$,
describe all of them.
\end{question}

\sol{\SOL}{
It is clear that if $\DS \{x_n\}_{n=0}^\infty$ is a sequence
defined by $x_{n+1} = g(x_n)$ for $n \geq 0$ such that
$x_N = p$ for some $N$, then $x_n = p$ for all $n \geq N$ because
$g(p) =p$.  Thus, $\DS \{x_n\}_{n=0}^\infty$ converges to
$p$ in a finite number of iterations.

Are there other sequences $\DS \{x_n\}_{n=0}^\infty$ that
converge to $p$?  To show that there are no other sequence converging
to $p$ requires a little bit of work.
...
Suppose that $\DS \{x_n\}_{n=0}^\infty$ is a sequence defined by
...
$\DS \{x_n\}_{n=0}^\infty$ is a sequence converging to the
fixed point $p$.
}
...
$\DS \rho = \frac{|g(x_0)-x_0|}{1-\lambda}$.
Prove that the sequence $\DS \{x_n\}_{n=0}^\infty$ defined by
...
for $n \geq 0$ yields a sequence $\DS \{x_n\}_{n=0}^\infty$
...
$\DS f(x) = (x-p)^m q(x)$ with $q(p) \neq 0$  if and only if
$\DS f(p) = f'(p) = ... = f^{(m-1)}(p) = 0$ and
$\DS f^{(m)}(p) \neq 0$; namely,
...
all $x$.  Thus $\DS f(x) = r_{m-1}(x) = q(x) \, (x-p)^m$ with
$\DS q(x) = \frac{1}{m!}\,f^{(m)}(\xi(x))$.

Since $\xi(x)$ is between $x$ and $p$, we have
that $\DS \lim_{x\to p} \xi(x) = p$.  Therefore, since
$\DS f^{(m)}$ is continuous, we have
\[
  \lim_{x\to p} q(x) = \frac{1}{m!}\,f^{(m)}(p) \neq 0 \ .
\]
Note that $q$ is a continuous function on $\RR\setminus \{p\}$
because $\DS q(x) = \frac{f(x)}{(x-p)^m}$ for $x \neq p$,
where $f$ and $\DS (x-p)^m$ are continuous functions of $x$.
The function $q$ is also continuous at $p$ because
$\DS \lim_{x\to p} \frac{f(x)}{(x-p)^m} =
\frac{1}{m!}\, f^{(m)}(p) = q(p)$ according to l'Hospital Rule. 

\stage{$\mathbf{\Rightarrow}$}
From $\DS f(x) = (x-p)^m q(x)$, we have by induction that
...
\end{cases}
\]
Hence, $\DS f^{(k)}(p) = 0$ for $0\leq k <m$ because each term in
...
From $F'(r) = 0$, we get that
\[
0 = 1 - (f'(r))^2 - f(r)f''(r) = 1 - (f'(r))^2
\]
because $f(r)=0$.  Hence $f'(r) = \pm 1$.   From $F''(r) = 0$, we get that
\[
...
\]
because $f(r)=0$.  Hence $\DS g(r) = -\frac{1}{f'(r)}$.
From $F''(r) = 0$, we get that
\[
0 = f''(r)g(r) + 2f'(r)g'(r) + f(r)g''(r)
= -\frac{f''(r)}{f'(r)} + 2f'(r)g'(r)
\]
because $f(r)=0$ and $\DS g(r) = -\frac{1}{f'(r)}$.  Hence
$\DS g'(r) = \frac{f''(r)}{2(f'(r))^2}$.  From $F'''(r) \neq 0$,
we get that
\begin{align*}
...
+ 3 f'(r) g''(r)
\end{align*}
because $f(r)=0$, $\DS g(r) = -\frac{1}{f'(r)}$ and
$\DS g'(r) = \frac{f''(r)}{2(f'(r))^2}$.  Hence
$\DS g''(r) \neq
\frac{f'''(x)}{3(f'(r))^2} - \frac{(f''(r))^2}{2(f'(r))^3}$.

The conditions on $g$ are 
$\DS g(r) = -\frac{1}{f'(r)}$,
$\DS g'(r) = \frac{f''(r)}{2(f'(r))^2}$ and
$\DS g''(r) \neq
\frac{f'''(x)}{3(f'(r))^2} - \frac{(f''(r))^2}{2(f'(r))^3}$.
}
...
\subQ{a} & $\DS \left\{\frac{1}{n^2}\right\}_{n=1}^\infty$ &
\subQ{b} & $\DS \left\{\frac{1}{2^{2^n}}\right\}_{n=0}^\infty$ &
\subQ{c} & $\DS \left\{\frac{1}{\sqrt{n}}\right\}_{n=1}^\infty$ \\
\subQ{d} & $\DS \left\{\frac{1}{e^n}\right\}_{n=0}^\infty$ &
\subQ{e} & $\DS \left\{\frac{1}{n^n}\right\}_{n=1}^\infty$ & &
\end{tabular}
\end{center}
\end{question}
...
\begin{question}
\subQ{a} Show that the convergence of the sequence
$\DS p_n = 10^{-k^n}$ to $0$ is of order $k$.\\
\subQ{b} Show that the sequence $\DS p_n = 10^{-n^k}$ does
not converge to $0$ quadratically regardless of the size of the
exponent $k > 1$.
\end{question}

\sol{\SOL}{
\subQ{a} Let $\DS e_n = 10^{-k^n} - 0$.  We have
\[
...
\subQ{b} Let $\DS e_n = 10^{-n^k}-0$.  We have
\[
\frac{|e_{n+1}|}{|e_n|^\alpha}
...
\ ,
\]
where \mbox{l.o.t.} stands for the terms in
$\DS n^j$ with $0 \leq j < n-1$.
Hence,
\[
\lim_{n\rightarrow \infty} \frac{|e_{n+1}|}{|e_n|^\alpha} =
\begin{cases}
...
\begin{question}
Solve $\DS x - 2^{-x} = 0$ for $x \in [0,1]$ with an
accuracy of $\DS 10^{-4}$ using Steffensen's Algorithm.
\end{question}
...
\]
with an accuracy of $\DS 10^{-10}$.  Do not used any formula
to computer the roots of a polynomial of degree two.
\end{question}
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=r_1$ and a tolerance of $\DS 10^{-10}$, we find
...
with $x_0=r_2$ and a tolerance of $\DS 10^{-10}$, we find
...
\begin{question}
Use the method of deflation to approximate all the roots of
$\DS p(x) = x^4 -2 x^3 -12 x^2 + 16 x -40$ with an accuracy
of $\DS 10^{-9}$. 
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=1$ and a tolerance of $\DS 10^{-9}$, we find
$r_0 = -3.548232897979703$ as an approximation of a root of $p$.

The deflated polynomial $q_1$ such that $p(x) = (x-r_0) q_1(x)$ is\\
$\DS q_1(x) = x^3 -5.548232897979703 x^2 + 7.686422494264843 x
-11.273217161921718$\ .
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=r_1$ and a tolerance of $\DS 10^{-9}$, we find
$c_1 = 4.381113440995943$ as an approximation of a root of $p$.

The deflated polynomial $q_2$ such that $q_1(x) = (x-r_1) q_2(x)$ is\\
$\DS q_2(x) =  x^2 -1.167119456983760 x + 2.573139754025407$\ .
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=r_2$ and a tolerance of $\DS 10^{-9}$, we find
...
\begin{question}
Use the method of deflation to approximate all the roots of
$\DS x^3 - 53 x^2 + 151 x - 3$ with an accuracy of
$\DS 10^{-3}$.
You must use Newton's method with Horner's Algorithm.
\end{question}
