...
\begin{theorem}
If $\displaystyle \{x_n\}_{n=0}^\infty$ is a bounded and increasing
...
$\displaystyle f^{(n+1)}(x)$ exists for all $x \in ]a,b[$, and that
...
$\displaystyle b_n - a_n = (b-a)/2^n$.
\label{bisecProp1}
\end{prop}

\begin{proof}
We prove by induction that the interval $[a_n,b_n]$ is of length
$\displaystyle (b-a)/2^n$.

We have $\displaystyle b_0 - a_0 = b - a = (b-a)/2^0$.  Hence, the interval
$[a_0,b_0]$ is of length $\displaystyle (b-a)/2^0$.

Suppose that the interval $[a_n,b_n]$ is of length $\displaystyle (b-a)/2^n$;
namely, $\displaystyle b_n-a_n = (b-a)/2^n$.  Since $[a_{n+1},b_{n+1}]$ is
...
satisfies $\displaystyle |x_n - r | < (b_{n-1} - a_{n-1})/2 = (b-a)/2^n$
...
$\displaystyle \{a_n\}_{n=0}^\infty$ is an increasing and bounded sequence.  It
follows from Theorem~\ref{Th0} that $\displaystyle \{a_n\}_{n=0}^\infty$
converges.  Let $\alpha$ be this limit.

Similarly, since $b_0 \geq b_1 \geq b_2 \geq \ldots \geq a$, the sequence
$\displaystyle \{b_n\}_{n=0}^\infty$ is a decreasing and bounded sequence.  Thus
$\displaystyle \{b_n\}_{n=0}^\infty$ converges.  Let $\beta$ be this limit.
...
theorem that $\displaystyle \{x_n\}_{n=0}^\infty$ also converge to
$\alpha = \beta$.
...
when the length of the interval is less than $\displaystyle 10^{-2}$.
Find a bound on the absolute error.

The question is to find the positive root of
$\displaystyle f(x) = x^2 -2 = 0$.  Let
...
$\displaystyle (1.421875 - 1.4140625)/2 = 1/2^8 = 0.00390625$ is an
...
absolute error lest than $\displaystyle 10^{-4}$\ ?

We choose $n$ such that
$\displaystyle |b_n- a_n| = (b-a)/2^n < 10^{-4}$.  This is
$\displaystyle 2^{-n}< 10^{-4}$.  Thus,
\[
...
$\displaystyle \{x_n\}_{n=0}^\infty$ defined by $x_{n+1} = g(x_n)$ for
...
\begin{egg}
Find an approximation to a root of $\displaystyle f(x)=x^3+9\,x-9$.

Because $f(0)\,f(1)=-9<0$, the function $f$ has a root between $0$ and
$1$.  In Example~\ref{fixit}. we saw that $\displaystyle f(x)=x^3+9\,x-9=0$ is
equivalent to $\displaystyle g(x)=(9-x^3)/9 =x$.  Thus, the problem is to
...
$\displaystyle g'(x) = -x^2/3 < 0$ for all $x>0$,
...
$\displaystyle |g'(x)|=|-x^2/3|\leq1/3$ for all $x$ in $[0,1]$.
...
The idea is to construct a sequence $\displaystyle \{x_i\}_{i=0}^\infty$ which
...
than $\displaystyle 10^{-4}$.

As in Example~\ref{ExSqrt2}, we find an approximation of the
positive root of $\displaystyle f(x) = x^2 - 2$.  We have
\[
x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}
...
between two consecutive iterations is smaller than $\displaystyle 10^{-10}$.
...
sequence $\displaystyle \{x_i\}_{i=0}^\infty$ that converges to a root $p$ of
...
of $g$,  The sequence $\displaystyle \{x_n\}_{n=0}^\infty$ converges
...
We will study the fixed point method in $\displaystyle \RR^n$ in
...
\begin{defn}
Suppose that the sequence $\displaystyle \{x_n\}_{n=0}^\infty$ converges to $p$.
Let $e_n = x_n - p$.  We say that $\displaystyle \{x_n\}_{n=0}^\infty$
...
$x_0 \in [p-\delta, p + \delta]$, the sequence
$\displaystyle \{x_n\}_{n=0}^\infty$ produced by the
...
that is, $\displaystyle f(p) = f'(p) = \ldots = f^{(k-1)}(p) = 0$
and $\displaystyle f^k(p) \neq 0$. 
...
$\displaystyle f(x) = (x-p)^k\,q(x)$ for some function $q$ such that
$q(p) \neq 0$. Hence, for $x \neq p$,
\[
...
sequence $\displaystyle \{x_n\}_{n=0}^\infty$ produced by
...
\section{Aitken's $\displaystyle \Delta^2$ Process and Steffensen's Algorithm}

Suppose that $p_0$ is an initial approximation for a fixed point $p$
of a function $g$.  Moreover, suppose that the sequence
$\displaystyle \{p_n\}_{n=0}^\infty$ defined by $p_{n+1} = g(p_n)$
...
``faster'' to $p$ than $\displaystyle \{p_n\}_{n=0}^\infty$.
...
The sequence $\displaystyle \{\hat{p}_n\}_{n=0}^\infty$ defined by
...
$\displaystyle \{\hat{p}_n\}_{n=0}^\infty$ to $p$ is greater than 1.
The procedure used to construct $\displaystyle \{\hat{p}_n\}_{n=0}^\infty$ is
called the
{\bfseries Aitken's $\displaystyle \Delta^2$ process}\index{Aitken's
$\Delta^2$ process}.
...
$\displaystyle \{\hat{p}_n\}_{n=0}^\infty$ defined by (\ref{formula4})
...
Let
$\displaystyle p(x) = a_m\,x^m + a_{m-1}\,x^{m-1} + \ldots + a_1\,x + a_0$ and
\[
\begin{split}
b_m &= a_m \\
...
$\displaystyle q(x) = b_m\,x^{m-1} + b_{m-1}\,x^{m-2} + \ldots + b_2\,x + b_1$.
...
sequence $\displaystyle \{x_n\}_{n=0}^\infty$ that converges to $c$.
A bad choice for
...
\]
where only the coefficient of $\displaystyle x^2$ has be changed from
...
\begin{egg}
Let $\displaystyle p(x) = x^7 - 28x^6 +322x^5 - 1960x^4 + 6769x^3
...
\begin{egg}
Let $\displaystyle p(x) = 5 -3x -4x^2+x^4$.  Approximate all the roots
of $p$ within $\displaystyle 10^{-10}$.
...
interval) of $\displaystyle 4x^2 - e^x = 0$.  Do not forget to justify
your answer.
\end{question}

\sol{\SOL}{
We will roughly sketch the graph of $\displaystyle f(x) = 4x^2 - e^x$.
...
Since $\displaystyle f''(x) = 8 -e^x$, a potential point of inflection is
...
correct to within $\displaystyle 10^{-4}$.
\end{question}

\sol{\SOL}{
$\sqrt[3]{25}$ is the unique root of $\displaystyle f(x) = x^3 -25$.
...
$\sqrt[3]{25}$ with an accuracy of $\displaystyle 10^{-4}$, we need to
...
\end{equation}
Show that the $\displaystyle n^{th}$ iteration $x_n$ of the bisection method 
...
\begin{question}
In the algorithm for the bisection method, Algorithm~\ref{BisectAlgo},
show that $\displaystyle | x_n - x_{n+1}| = 2^{-n-1}(b_0 - a_0)$.
\end{question}
...
$[a_n,b_n]$ is $\displaystyle (b_0-a_0)/2^n$.
}
...
\begin{question}
Find a solution accurate to within $\displaystyle 10^{-4}$ for $x=\tan(x)$ on
$\pi/2 < x < 3 \pi/2$.\\
...
\begin{question}
Find the solutions accurate to within $\displaystyle 10^{-5}$ for
$\displaystyle x^2 + 11\cos(x)=0$.\\
\subQ{a} Use the bisection method.\\
\subQ{b} Use the fixed point method.\\
\subQ{c} Use Newton's method.
\end{question}
...
$\displaystyle y=2x/(1+x^2)$ (in blue) on the same coordinate system.
...
accuracy of at least $\displaystyle 10^{-12}$.
}

\begin{question}
For which functions $f$ is the iterative equation
$\displaystyle  x_{n+1} = 2x_n - C x_n^2$
...
\begin{question}
If $x_0 = 0$ and
$\displaystyle x_{n+1} = x_n - \frac{\tan(x_n) - 1}{\sec^2(x_n)}$
...
because:
\begin{enumerate}
\item $\displaystyle f'(x) = \sec^2(x) > 1$ for
$-\pi/2 < x < \pi/2$ with $x\neq 0$ and $f'(0) = 1$,
\item $\displaystyle f''(x) = 2\sec^2(x)\tan(x) \geq 0$ for
...
root of $\displaystyle f(x) = e^x - \tan(x)$ with an accuracy of
$\displaystyle 10^{-8}$.\\
Hint: To choose $x_0$ and $x_1$, draw the graph of $\displaystyle e^x$
and $\tan(x)$.
\end{question}
...
intersection of $\tan(x)$ (in black) and $\displaystyle e^x$ (in blue) in the
following figure.
...
$\displaystyle \{x_n\}_{n=0}^\infty$ converging to a root $r$ of a function $f$. 
Let $e_n = x_n-r$.  Show that
\[
e_{n+1} = \frac{f''(\xi_n)}{2f'(x_n)}\, e_n^2
\]
for some $\xi_n$ between $r$ and $x_n$.\\
\subQ{b} Let $\displaystyle f(x) = x-e^{-x}$ and assume that the
Newton's method is used to generate a sequence
$\displaystyle \{x_n\}_{n=0}^\infty$ converging to the
...
an accuracy of $\displaystyle 10^{-5}$; namely, such that
$\displaystyle |e_n| < 10^{-5}$.
\end{question}
...
$\displaystyle f'(x) = 1 + e^{-x}$ and $\displaystyle f''(x) = -e^{-x}$.  Hence,
\[
...
three roots of $\displaystyle f(x) = x^3 -x$; namely, to approximate $p_1=-1$,
...
of the sequence $\displaystyle \{ x_n\}_{n=0}^\infty$ given by $x_{n+1} = g(x_n)$
...
maximum of $\displaystyle G(x) = |g'(x)| = 2x/(1+x^2)^2$ on $[0,1]$ is
...
\begin{question}
Consider the function $\displaystyle g(x)=2^{-x}$.\\
...
of the fixed point of $g$ has an accuracy of $\displaystyle 10^{-4}$.  You may
assume that $x_0=0.5$\ .\\
\subQ{c} Use the Fixed Point Theorem to find an approximation
$x_{n+1}$ to the fixed point of $g$ in the interval $[1/3,1]$ such
that $\displaystyle |x_{n+1} - x_n| < 10^{-4}$.  As in (b), you may assume that
$x_0=0.5$.
\end{question}
...
Theorem, we will have that the sequence $\displaystyle \{x_n\}_{n=0}^\infty$
generated by $x_{n+1} = g(x_n)$ for $n\geq 0$ and $x_0\in [1/3,1]$
converges to the unique fixed point $p$ of $g$ in the interval $[1/3,1]$.
\begin{enumerate}
\item Since $\displaystyle g'(x) = -2^{-x}\ln(2)<0$ for all $x$, the
...
\item Since $\displaystyle |g'(x)| = 2^{-x}\ln(2) < 2^{-1/3}\ln(2)$ for
$x \in [1/3,1]$.  We have that 
$|g(x)-g(y)| \leq K |x-y|$ for all $x,y\in [1/3,1]$ with
$\displaystyle K= 2^{-1/3}\ln(2) < 1$ according to Remark~\ref{FxPtThDer}.
\end{enumerate}
...
$\displaystyle 10^{-7}$?  You may assume that $x_0 = 9.5$.\\
\subQ{d} What is the order of convergence of the sequence
$\displaystyle \{x_n\}_{n=0}^\infty$ that is generated by
...
with an accuracy of $\displaystyle 10^{-7}$.  Use $x_0 = 9.5$.  Why does this
...
\begin{enumerate}
\item Since $\displaystyle g'(x) = 20/x^2>0$ for all $x\in[9.5,11.5]$,
...
\item Since $\displaystyle |g'(x)| = 20/x^2 < 20/9.5^2 = 80/361$ for
...
\subQ{d} Since $\displaystyle g'(x) = 20/x^2 > 0$ for $x\in [9.5,11.5]$, we
...
$\displaystyle |\hat{x}_{n+1} - \hat{x}_n|< 10^{-7}$.
We get $\displaystyle |\hat{x}_{n+1} - \hat{x}_n|<10^{-7}$ for the
...
that the sequence $\displaystyle \{x_n\}_{n=0}^\infty$ generated by
...
until $\displaystyle |x_n - x_{n-1}| < 10^{-5}$.  The first time that
...
of $\displaystyle 10^{-5}$ if we start with $x_0 = 3$.  The formula
...
\begin{question}
Let $\displaystyle f(x) = e^{2-x} -x^2$.\\
...
of $n$ such that $x_n$, the $\displaystyle (n+1)^{th}$ term in the sequence
$\displaystyle \{x_n\}_{n=0}^\infty$ produced by the Fixed Point
Theorem applied to the function $g$, is an approximation of $p$ with
an accuracy of $\displaystyle 10^{-5}$.\\
...
and $3\pi/2$.  Let $\displaystyle g(x) = \pi + \arctan(x)$.\\
...
\subQ{b} We have $\displaystyle g'(x) = 1/(1+x^2)$.  Hence $g$ is strictly
increasing on $[\pi,3\pi/2]$ and
$\displaystyle |g'(x)| \leq g'(\pi) = 1/(1+\pi^2) < 1$ on
...
\item With $\displaystyle K=1/(1+\pi^2)$, we have
\[
|g'(x)| \leq K <1
\]
...
$\displaystyle \{x_i\}_{i=0}^\infty$ defined by $x_0\in [\pi,3\pi/2]$ and
...
both sides, we get $p = a/p$.  Thus $\displaystyle p^2=a$ or
$p=\sqrt{a}$ since we assume that $a>0$.

\subQ{b} Suppose that $x>0$.  Since $\displaystyle (x-\sqrt{a})^2 >0$, we get
that $\displaystyle x^2 -2\sqrt{a} x + a >0$.  Thus
$\displaystyle x^2 + a > 2\sqrt{a} x$ and,
...
\]
because $\displaystyle x^2 \geq a$.  Hence,
...
$\displaystyle \{x_n\}_{n=0}^\infty$ does not converge to $p$.  If there are
sequences $\displaystyle \{x_n\}_{n=0}^\infty$ that converge to $p$,
describe all of them.
\end{question}

\sol{\SOL}{
It is clear that if $\displaystyle \{x_n\}_{n=0}^\infty$ is a sequence
defined by $x_{n+1} = g(x_n)$ for $n \geq 0$ such that
$x_N = p$ for some $N$, then $x_n = p$ for all $n \geq N$ because
$g(p) =p$.  Thus, $\displaystyle \{x_n\}_{n=0}^\infty$ converges to
$p$ in a finite number of iterations.

Are there other sequences $\displaystyle \{x_n\}_{n=0}^\infty$ that
...
Suppose that $\displaystyle \{x_n\}_{n=0}^\infty$ is a sequence defined by
...
$\displaystyle \{x_n\}_{n=0}^\infty$ is a sequence converging to the
fixed point $p$.
}
...
Prove that the sequence $\displaystyle \{x_n\}_{n=0}^\infty$ defined by
...
for $n \geq 0$ yields a sequence $\displaystyle \{x_n\}_{n=0}^\infty$
...
$\displaystyle f(x) = (x-p)^m q(x)$ with $q(p) \neq 0$  if and only if
$\displaystyle f(p) = f'(p) = ... = f^{(m-1)}(p) = 0$ and
$\displaystyle f^{(m)}(p) \neq 0$; namely,
...
all $x$.  Thus $\displaystyle f(x) = r_{m-1}(x) = q(x) \, (x-p)^m$ with
...
$\displaystyle f^{(m)}$ is continuous, we have
\[
...
where $f$ and $\displaystyle (x-p)^m$ are continuous functions of $x$.
...
From $\displaystyle f(x) = (x-p)^m q(x)$, we have by induction that
...
\end{cases}
\]
Hence, $\displaystyle f^{(k)}(p) = 0$ for $0\leq k <m$ because each term in
...
$\displaystyle p_n = 10^{-k^n}$ to $0$ is of order $k$.\\
\subQ{b} Show that the sequence $\displaystyle p_n = 10^{-n^k}$ does
...
$\displaystyle n^j$ with $0 \leq j < n-1$.
Hence,
\[
\lim_{n\rightarrow \infty} \frac{|e_{n+1}|}{|e_n|^\alpha} =
\begin{cases}
...
\begin{question}
Solve $\displaystyle x - 2^{-x} = 0$ for $x \in [0,1]$ with an
accuracy of $\displaystyle 10^{-4}$ using Steffensen's Algorithm.
\end{question}
...
\]
with an accuracy of $\displaystyle 10^{-10}$.  Do not used any formula
...
with $x_0=1$ and a tolerance of $\displaystyle 10^{-10}$, we find
$r_0 = 0.333333333308985$ as an approximation of a root of $p$.

The deflated polynomial $q_1$ such that $p(x) = (x-r_0) q_1(x)$ is\\
$\displaystyle q_1(x) = x^2 -5.641592653691014 x + 7.853981634573692$\ .
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=r_1$ and a tolerance of $\displaystyle 10^{-10}$, we find
...
with $x_0=r_2$ and a tolerance of $\displaystyle 10^{-10}$, we find
...
\begin{question}
Use the method of deflation to approximate all the roots of
$\displaystyle p(x) = x^4 -2 x^3 -12 x^2 + 16 x -40$ with an accuracy
of $\displaystyle 10^{-9}$. 
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=1$ and a tolerance of $\displaystyle 10^{-9}$, we find
$r_0 = -3.548232897979703$ as an approximation of a root of $p$.

The deflated polynomial $q_1$ such that $p(x) = (x-r_0) q_1(x)$ is\\
$\displaystyle q_1(x) = x^3 -5.548232897979703 x^2 + 7.686422494264843 x
-11.273217161921718$\ .
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=r_1$ and a tolerance of $\displaystyle 10^{-9}$, we find
$c_1 = 4.381113440995943$ as an approximation of a root of $p$.

The deflated polynomial $q_2$ such that $q_1(x) = (x-r_1) q_2(x)$ is\\
$\displaystyle q_2(x) =  x^2 -1.167119456983760 x + 2.573139754025407$\ .
...
Using Newton's Method with Horner's Algorithm, Code~\ref{NRHorner},
with $x_0=r_2$ and a tolerance of $\displaystyle 10^{-9}$, we find
...
\begin{question}
Use the method of deflation to approximate all the roots of
$\displaystyle x^3 - 53 x^2 + 151 x - 3$ with an accuracy of
$\displaystyle 10^{-3}$.
You must use Newton's method with Horner's Algorithm.
\end{question}
...
\]
with an acuracy of $\displaystyle 10^{-5}$.
You must use Newton's method with Horner's Algorithm.
\end{question}
