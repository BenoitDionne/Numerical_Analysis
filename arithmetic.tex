...
$\displaystyle \pm 0.d_1d_2d_3\ldots \times 10^m$, where $m$ is an
...
\begin{defn}
Let $\displaystyle \pm 0.d_1 d_2 \dots \times 10^N$ be the normalized scientific
...
$\displaystyle \pm 0.d_1 d_2 \dots d_k \times 10^N$, and the\\
{\bfseries k-digit rounding representation}\index{k-digit Rounding
  Representation} of $a$ to be
$\displaystyle \pm 0.d_1 d_2 \dots d_k \times 10^N + \epsilon \,
10^{-k} \times 10^N$,
...
$\displaystyle |a-\tilde{a}| < 10^{-k}\times 10^N$.  If $\tilde{a}$ is 
k-digit rounding representation of $a$, then
$\displaystyle |a-\tilde{a}| \leq 0.5 \times 10^{-k}\times 10^N$.
...
$\displaystyle 0.12752 \times 10^2$ is a $5$-digit rounding
...
$\displaystyle \pm(0.b_1 b_2 b_3 \ldots )_2 \times 2^m$, where
$b_i \in \{0,1\}$, 
...
\end{align}
In general, we get that the $\displaystyle i^{th}$ digit $d_i$ is the
integer part of
...
$\displaystyle x = \pm(0.b_1 b_2 b_3 \ldots )_2 \times 2^m$ is given by
\[
\begin{array}{|c|c|c|}
\hline
...
$(e_8 e_7 \ldots e_1)_2$.  Then $\displaystyle 0 \leq e < 2^8 = 256$ but, in
...
$\displaystyle (0.\overline{1100})_2 \times 2^{-(11)_2}$.  Hence, the machine
...
$\displaystyle x=(0.b_1 b_2 b_3\ldots)_2 \times 2^m$, we 
...
computer numbers
$\displaystyle x_1 = (0.b_1b_2b_3\ldots b_{24})_2 \times 2^m$ and
$\displaystyle x_2 = ((0.b_1b_2b_3\ldots b_{24})_2 + 2^{-24}) \times 2^m$.
...
\]
because
$\displaystyle (0.b_1 b_2 b_3\ldots)_2 \geq (0.b_1)_2 = (0.1)_2 = 2^{-1}$.
...
\leq 2^{m-25}
\]
because $\displaystyle (1)_2-(0.b_{25}b_{26} \ldots)_2 \leq 2^{-1}$.  Thus
\[
\frac{|x-\fl(x)|}{|x|} \leq
\frac{2^{m-25}}{(0.b_1 b_2 b_3\ldots)_2 \times 2^m}
...
< 2^{-24}
\]
because $\displaystyle (0.b_1b_2b_3\ldots)_2 \geq (0.b_1)_2 =
(0.1)_2 = 2^{-1}$.
...
< 2^{-23}
\]
because $\displaystyle (0.b_1b_2b_3\ldots)_2 \geq (0.b_1)_1 = (0.1)_2 = 2^{-1}$.
\end{rmk}
...
is the largest integer such that $\displaystyle \beta^s \leq |p|$.
\end{defn}
...
\ ,
\]
where $s$ is the largest integer such that $\displaystyle 10^s \leq |p|$.  Thus
\[
...
$\displaystyle \epsilon_r < 5 \times 10^{-r}$.
\end{egg}
...
Let $\displaystyle x= 0.1234 \times 10^5$ and $y=0.4321$.  Using
$4$-digit rounding
arithmetic to add this two numbers, we get $x+y=x$ because
$y=0.000004321$ and so $x+y$ is $\displaystyle 0.123404321 \times 10^5$.
Rounding this number to $4$-digits gives
$\displaystyle x = 0.1234 \times 10^5$.
\end{enumerate}
...
polynomial $\displaystyle x^2-20\,x+1=0$.

The standard formulae to compute the
roots of the polynomial of degree two $\displaystyle a x^2+b x+c=0$ are
\begin{equation}
...
$\displaystyle a x^2+b x+c=0$ are also
...
$\displaystyle x^2-20\,x+1=0$, the second formula to compute $x_-$ is
...
arithmetic gives $\displaystyle f(x) = -0.140 \times 10^2$.  Using the
...
$\displaystyle f(x) = -0.146 \times 10^2$.  The absolute error is
$\displaystyle 0.36489 \times 10^{-1}$, the relative error is about
$\displaystyle 0.25 \times 10^{-2}$, and the approximation is to $3$ significant
digits.
...
\end{align*}
The exact value of the sum is $\displaystyle 0.107101023 \times 10^5$.
\[
...
The exact solution of (\ref{C1L10}) is $\displaystyle x_j = (1/3)^j$ for
...
$B$. Since $\displaystyle (1/7)^j$ converge to $0$ faster than
$\displaystyle (1/3)^j$ as
...
$B$.  Since $\displaystyle 4^j$ converges to $\infty$ while
$\displaystyle (1/3)^j$ converges to
$0$ as $j\rightarrow \infty$, the term $\displaystyle B 4^j$ of the general
...
$\displaystyle (f''(\zeta)h^2)/(2f(x))$ because
$\displaystyle h^2$ goes to $0$ faster than $h$.
Hence,
\[
...
$\displaystyle |e- 2.717| \approx 0.128183 \times 10^{-2}$ and
the relative error is
$\displaystyle \frac{|e-2.717|}{e} \approx 0.4715583 \times 10^{-3}$.
Since $4$ is the largest value of $r$
such that the relative error is smaller than
$\displaystyle5 \times 10^{-r}$, there are $4$ significant digits.
}
...
we have that
$\displaystyle 1-\cos(0.25) \approx 0.310875783 \times 10^{-1}$.  The
...
$x_-$ of the polynomial $\displaystyle p(x) = x^2 - 235 x + 3$\ .  Justify your
choice of formula.\\
...
of the polynomial $\displaystyle ax^2 + b x + c$, either
...
and $\displaystyle \sqrt{b^2 -4ac} \approx b$, the operation
$\displaystyle -b -\sqrt{b^2-4ac}$ is
...
$\displaystyle b^2 = 55230$,
$\displaystyle 4ac = 12$, $\displaystyle b^2-4ac = 55220$,
$\displaystyle \sqrt{b^2-4ac} = 235$,
$\displaystyle -b+\sqrt{b^2-4ac} = 470$, $2c=6$ and finally
\[
\tilde{x}_- = \frac{2c}{-b+\sqrt{b^2-4ac}} = \frac{6}{470} = 0.01277 \ .
\]

\subQ{c} Using $x_- = 0.012766651010\ldots$, we get the absolute error
$\displaystyle \epsilon = |\tilde{x}_- - x_-| = 0.334899 \times 10^{-5}$
and the relative 
...
\begin{question}
What can go wrong with the operation $\displaystyle \sqrt{x^2+y^2}$
for very large values of $x$ and $y$.  How can you avoid such problem?
\end{question}

\sol{\SOL}{
If $x$ and $y$ are very large, $\displaystyle x^2+y^2$ can produce an overflow.
To avoid overflow, we use one of the following equivalent expressions
for $\displaystyle \sqrt{x^2+y^2}$.
\[
...
\begin{question}
Find a way to compute $\displaystyle f(x) = \sqrt{x^4+4}-2$ for $x$
small that will minimize the loss of significant digits.
\end{question}

\sol{\SOL}{
The problem with $\displaystyle \sqrt{x^4+4}-2$ for $x$ near $0$ is
the subtraction of almost equal numbers.    If $x$ is closed to $0$,
then $\displaystyle x^4$ is closer to $0$ and
$\displaystyle \sqrt{x^4+4} \approx \sqrt{4} = 2$.  One way to
...
\]
Note that $\displaystyle \sqrt{x^4+4}+2\approx 4$ for $x$ near $0$ and
...
\sol{\SOL}{
We seek a solution of the form $\displaystyle x_n = \lambda^n$.  If we
substitute this value of $x_n$ into (\ref{finite_diff}), we get
\[
\lambda^n = 2 \lambda^{n-1} + \lambda^{n-2} \ .
\]
If $\lambda \neq 0$, we can divide by $\displaystyle \lambda^{n-2}$ on
both sides of the equality to get
$\displaystyle \lambda^2 = 2 \lambda +1$; namely,
$\displaystyle \lambda^2 -2\lambda -1 = 0$.  The roots of this polynomial are
