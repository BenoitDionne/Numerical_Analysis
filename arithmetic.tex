...
$\DS \pm 0.d_1d_2d_3\ldots \times 10^m$, where $m$ is an
...
\begin{defn}
Let $\DS \pm 0.d_1 d_2 \dots \times 10^N$ be the normalized scientific
...
$\DS \pm 0.d_1 d_2 \dots d_k \times 10^N$, and the\\
{\bfseries k-digit rounding representation}\index{k-digit Rounding
  Representation} of $a$ to be
$\DS \pm 0.d_1 d_2 \dots d_k \times 10^N + \epsilon \,
10^{-k} \times 10^N$,
...
$\DS |a-\tilde{a}| < 10^{-k}\times 10^N$.  If $\tilde{a}$ is 
k-digit rounding representation of $a$, then
$\DS |a-\tilde{a}| \leq 0.5 \times 10^{-k}\times 10^N$.
...
$\DS 0.12752 \times 10^2$ is a $5$-digit rounding
approximation of $y$, find
...
$\DS \pm(0.b_1 b_2 b_3 \ldots )_2 \times 2^m$, where
$b_i \in \{0,1\}$, 
...
\end{align}
In general, we get that the $\DS i^{th}$ digit $d_i$ is the
integer part of
...
$\DS x = \pm(0.b_1 b_2 b_3 \ldots )_2 \times 2^m$ is given by
\[
\begin{array}{|c|c|c|}
\hline
...
$(e_8 e_7 \ldots e_1)_2$.  Then $\DS 0 \leq e < 2^8 = 256$ but, in
...
$\DS (0.\overline{1100})_2 \times 2^{-(11)_2}$.  Hence, the machine
...
However, when the numbers $p$ and $\tilde{p}$ are larger than
$\DS 10^6$
...
\begin{rmk}
For our 32-bit computer, if
$\DS x=(0.b_1 b_2 b_3\ldots)_2 \times 2^m$, we 
have that $\DS \frac{|x-\fl(x)|}{|x|} \leq 2^{-24}$ if rounding
is used and $\DS \frac{|x-\fl(x)|}{|x|} \leq 2^{-23}$ if
chopping is used.

\stage{i} We prove that $\DS \frac{|x-\fl(x)|}{|x|}
\leq 2^{-24}$ if rounding is used.  The number $x$ is between the
computer numbers
$\DS x_1 = (0.b_1b_2b_3\ldots b_{24})_2 \times 2^m$ and
$\DS x_2 = ((0.b_1b_2b_3\ldots b_{24})_2 + 2^{-24}) \times 2^m$.
...
\]
because
$\DS (0.b_1 b_2 b_3\ldots)_2 \geq (0.b_1)_2 = (0.1)_2 = 2^{-1}$.
...
\leq 2^{m-25}
\]
because $\DS (1)_2-(0.b_{25}b_{26} \ldots)_2 \leq 2^{-1}$.  Thus
\[
\frac{|x-\fl(x)|}{|x|} \leq
\frac{2^{m-25}}{(0.b_1 b_2 b_3\ldots)_2 \times 2^m}
...
< 2^{-24}
\]
because $\DS (0.b_1b_2b_3\ldots)_2 \geq (0.b_1)_2 =
(0.1)_2 = 2^{-1}$.

\stage{ii} To prove that
$\DS \frac{|x-\fl(x)|}{|x|} \leq 2^{-23}$ if
...
< 2^{-23}
\]
because $\DS (0.b_1b_2b_3\ldots)_2 \geq (0.b_1)_1 = (0.1)_2 = 2^{-1}$.
\end{rmk}
...
is the largest integer such that $\DS \beta^s \leq |p|$.
\end{defn}
...
\ ,
\]
where $s$ is the largest integer such that $\DS 10^s \leq |p|$.  Thus
\[
...
= 10^{-4} < 5 \times 10^{-4}
\]
and $4$ is the largest integer $r$ such that
$\DS \epsilon_r < 5 \times 10^{-r}$.
\end{egg}
...
Let $\DS x= 0.1234 \times 10^5$ and $y=0.4321$.  Using
$4$-digit rounding
arithmetic to add this two numbers, we get $x+y=x$ because
$y=0.000004321$ and so $x+y$ is $\DS 0.123404321 \times 10^5$.
Rounding this number to $4$-digits gives
$\DS x = 0.1234 \times 10^5$.
\end{enumerate}
...
polynomial $\DS x^2-20\,x+1=0$.

The standard formulae to compute the
roots of the polynomial of degree two $\DS a x^2+b x+c=0$ are
\begin{equation}
...
$\DS \frac{-b-\sqrt{b^2-4ac}}{-b-\sqrt{b^2-4ac}}$ and the
formula for $x_{-}$ in (\ref{C1L6}) by
$\DS \frac{-b+\sqrt{b^2-4ac}}{-b+\sqrt{b^2-4ac}}$ to get the
...
$\DS x^2-20\,x+1=0$, the second formula to compute $x_-$ is
...
arithmetic gives $\DS f(x) = -0.140 \times 10^2$.  Using the
fact that $f(x) = -14.636489$, we find that the absolute error is
...
$\DS f(x) = -0.146 \times 10^2$.  The absolute error is
$\DS 0.36489 \times 10^{-1}$, the relative error is about
$\DS 0.25 \times 10^{-2}$, and the approximation is to $3$ significant
digits.
...
\end{align*}
The exact value of the sum is $\DS 0.107101023 \times 10^5$.
\[
...
The exact solution of (\ref{C1L10}) is $\DS x_j = (1/3)^j$ for
...
$B$. Since $\DS (1/7)^j$ converge to $0$ faster than
$\DS (1/3)^j$ as
...
$B$.  Since $\DS 4^j$ converges to $\infty$ while
$\DS (1/3)^j$ converges to
$0$ as $j\rightarrow \infty$, the term $\DS B 4^j$ of the general
...
$\DS (f''(\zeta)h^2)/(2f(x))$ because
$\DS h^2$ goes to $0$ faster than $h$.
Hence,
\[
...
\begin{question}
Compute $\DS \left(\frac{1}{3}-\frac{3}{11}\right) + \frac{3}{20}$
...
\]
The relative error is
$\DS \frac{|0.210- 139/660|}{139/660} 
\approx 0.00288$. 
}

\begin{question}
Using $3$-digit chopping arithmetic, compute
$\DS \sum_{i=1}^{10} \frac{1}{i^2}$ in ascending and
...
$\DS \frac{|1.53-1.549767731166541|}{1.549767731166541}
\approx 0.012755$\ .
...
\end{align*}
The relative error is about
$\DS \frac{|1.54-1.549767731166541|}{1.549767731166541}
\approx 0.0063027$\ .
...
\begin{question}
We know that $\DS e = \sum_{n=0}^\infty \frac{1}{n!}$.
Using $4$-digit rounding arithmetic, compute the approximation
$\DS \sum_{n=0}^5 \frac{1}{n!}$ of $e$ using the best method
...
$\DS |e- 2.717| \approx 0.128183 \times 10^{-2}$ and
the relative error is
$\DS \frac{|e-2.717|}{e} \approx 0.4715583 \times 10^{-3}$.
Since $4$ is the largest value of $r$
such that the relative error is smaller than
$\DS5 \times 10^{-r}$, there are $4$ significant digits.
}
...
we have that
$\DS 1-\cos(0.25) \approx 0.310875783 \times 10^{-1}$.  The
...
$x_-$ of the polynomial $\DS p(x) = x^2 - 235 x + 3$\ .  Justify your
choice of formula.\\
...
of the polynomial $\DS ax^2 + b x + c$, either
$\DS x_- = \frac{-b - \sqrt{b^2-4ac}}{2a}$ or
$\DS x_- = \frac{2c}{-b + \sqrt{b^2-4ac}}$.  For the
polynomial given in the question, since $b<0$ 
and $\DS \sqrt{b^2 -4ac} \approx b$, the operation
$\DS -b -\sqrt{b^2-4ac}$ is
...
$\DS x_- = \frac{2c}{-b + \sqrt{b^2-4ac}}$.

\subQ{b} Using $4$-digit rounding arithmetic, we have 
$\DS b^2 = 55230$,
$\DS 4ac = 12$, $\DS b^2-4ac = 55220$,
$\DS \sqrt{b^2-4ac} = 235$,
$\DS -b+\sqrt{b^2-4ac} = 470$, $2c=6$ and finally
\[
\tilde{x}_- = \frac{2c}{-b+\sqrt{b^2-4ac}} = \frac{6}{470} = 0.01277 \ .
\]

\subQ{c} Using $x_- = 0.012766651010\ldots$, we get the absolute error
$\DS \epsilon = |\tilde{x}_- - x_-| = 0.334899 \times 10^{-5}$
and the relative 
error $\DS \epsilon_r = \frac{\epsilon}{|x_-|}
...
\begin{question}
What can go wrong with the operation $\DS \sqrt{x^2+y^2}$
for very large values of $x$ and $y$.  How can you avoid such problem?
\end{question}

\sol{\SOL}{
If $x$ and $y$ are very large, $\DS x^2+y^2$ can produce an overflow.
...
\begin{question}
Find a way to compute $\DS f(x) = \sqrt{x^4+4}-2$ for $x$
small that will minimize the loss of significant digits.
\end{question}

\sol{\SOL}{
The problem with $\DS \sqrt{x^4+4}-2$ for $x$ near $0$ is
the subtraction of almost equal numbers.    If $x$ is closed to $0$,
then $\DS x^4$ is closer to $0$ and
$\DS \sqrt{x^4+4} \approx \sqrt{4} = 2$.  One way to
...
\]
Note that $\DS \sqrt{x^4+4}+2\approx 4$ for $x$ near $0$ and
so there is no risk of division by a really small number.
}
...
\hline
\rule[1em]{0em}{1em}
\DS \frac{5505001}{294911} & 18.66600092909 &
18.6666519729681\ldots \\[1em]
\DS \frac{4.999999}{14.999999} & 0.333329 & 0.3333332888888\ldots
\\[1em]
\DS \frac{41.95835}{31.45727} & 1.33382 &
...
\sol{\SOL}{
We seek a solution of the form $\DS x_n = \lambda^n$.  If we
substitute this value of $x_n$ into (\ref{finite_diff}), we get
\[
\lambda^n = 2 \lambda^{n-1} + \lambda^{n-2} \ .
\]
If $\lambda \neq 0$, we can divide by $\DS \lambda^{n-2}$ on
both sides of the equality to get
$\DS \lambda^2 = 2 \lambda +1$; namely,
$\DS \lambda^2 -2\lambda -1 = 0$.  The roots of this polynomial are
...
$\DS (1+\sqrt{2})^n$ will dominate as $n$ increase.  Thus, because of
rounding errors, the formula for the solution of (\ref{finite_diff})
will always eventually produce a sequence
$\DS \{x_n\}_{n=0}^\infty$ that
