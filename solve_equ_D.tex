...
f(\VEC{x})  = \VEC{0} \ ,
\end{equation}
where $\DS f:\RR^n \rightarrow \RR^n$ is a given function.  Namely, we
have to find the vectors $\DS \VEC{p} \in \RR^n$ such that
$f(\VEC{p}) = 0$.
...
\VEC{x}  =  g(\VEC{x}) \ ,
\end{equation}
where $\DS g:\RR^n \rightarrow \RR^n$, and a fixed point of
$g$ is a root of $f$ and vice-versa.  Recall that a vector
$\DS \VEC{p} \in \RR^n$ is a
...
$\DS \{\VEC{x}_k\}_{k=0}^\infty$ defined by
...
Let $S$ be a closed and bounded subset of $\DS \RR^n$ and suppose that
$\DS g:S \rightarrow \RR^n$ satisfies:
\begin{enumerate}
...
norm.
\item Suppose that $\DS g:S\to \RR^n$ is continuously differentiable;
namely, that all the partial derivatives
$\DS \pdydx{g_i}{x_j}(\VEC{x})$ of $g$
...
\DS \pdydx{g_1}{x_1}(\VEC{x}) &
\DS \pdydx{g_1}{x_2}(\VEC{x}) & \dots &
\DS \pdydx{g_1}{x_n}(\VEC{x}) \\[0.8em]
\DS \pdydx{g_2}{x_1}(\VEC{x}) &
\DS \pdydx{g_2}{x_2}(\VEC{x}) & \ldots &
\DS \pdydx{g_2}{x_n}(\VEC{x}) \\
\vdots & \vdots & \ddots & \vdots \\
\DS \pdydx{g_n}{x_1}(\VEC{x}) &
\DS \pdydx{g_n}{x_2}(\VEC{x}) & \ldots &
\DS \pdydx{g_n}{x_n}(\VEC{x})
\end{pmatrix} \ .
\]
If $S$ is convex and
...
\end{equation}
then $\DS K \equiv \max_{\VEC{x} \in S} \|\diff g(x)\|_\infty$ can
...
functions on $\DS \RR^n$.  The previous inequality is also
...
\end{align*}
with an accuracy of $\DS 3 \times 10^{-5}$ using the norm
$\|\cdot\|_\infty$.
...
\DS \frac{x_1^2 + x_2^2 +8}{10} \\[0.7em]
\DS \frac{x_1x_2^2 +x_1 +8}{10}
\end{pmatrix}
\]
and
\[
...
\end{align*}
Hence $\DS K = \max_{\VEC{x} \in S} \|\diff g(\VEC{x})\|_\infty \leq
\max\{3/10 + 3/10,13/40 + 9/20\} = 31/40 <1$.

With $\DS \VEC{x}_0 = \begin{pmatrix} 0 \\ 0 \end{pmatrix}$, we get
$\DS \VEC{x}_1 = \begin{pmatrix} 0.8 \\ 0.8 \end{pmatrix}$,
$\DS \VEC{x}_2 = \begin{pmatrix} 0.928 \\ 0.9312 \end{pmatrix}$,
\ldots ,
$\DS \VEC{x}_{10} = \begin{pmatrix} 0.9999570565 \\
0.9999570577 \end{pmatrix}$,
$\DS \VEC{x}_{11} = \begin{pmatrix} 0.9999828232 \\
0.9999828234 \end{pmatrix}$,
$\DS \VEC{x}_{12} = \begin{pmatrix} 0.9999931294 \\
0.9999931294 \end{pmatrix}$.
We get $\DS \|\VEC{x}_k - \VEC{x}_{k-1}\|_\infty < 3\times 10^{-5}$
...
Let $\DS f:\RR^n \rightarrow \RR^n$ be a continuously differentiable
function; namely, all partial derivatives
$\DS \pdydx{f_i}{x_j}(\VEC{x})$ exist and are continuous.
...
\DS \pdydx{f_1}{x_n}(\VEC{x}) \\[0.7em]
\DS \pdydx{f_2}{x_1}(\VEC{x}) &
\DS \pdydx{f_2}{x_2}(\VEC{x}) & \ldots &
\DS \pdydx{f_2}{x_n}(\VEC{x}) \\
\vdots & \vdots & \ddots & \vdots \\
\DS \pdydx{f_n}{x_1}(\VEC{x}) &
\DS \pdydx{f_n}{x_2}(\VEC{x}) & \ldots &
\DS \pdydx{f_n}{x_n}(\VEC{x})
\end{pmatrix} \ .
\]
...
Let $\DS S=\{ \VEC{x} \in \RR^n : \| \VEC{x} - \VEC{p} \|
\leq \eta\}$.
Suppose that $\diff f(\VEC{x})$ is invertible for all $\VEC{x} \in S$
and let
$\DS g(\VEC{x}) = \VEC{x}- \diff f(\VEC{x})^{-1} f(\VEC{x})$.
If the partial derivatives of order two
$\DS \pdydxnm{g_i}{x_j}{x_k}{2}{}{}$ exist and are
...
\begin{rmk}
Any norm on $\DS \RR^n$ can be used.  However, the norm
$\|\cdot\|_\infty$
...
3x_1x_2^2 + x_1^3 - 1 &= 0
\end{align*}
near $\DS \begin{pmatrix} 1 \\ 1 \end{pmatrix}$
with an accuracy of $\DS 10^{-6}$ using $\|\cdot\|_\infty$.
...
Let $\DS \VEC{x}_0 = \begin{pmatrix} 1 \\ 1 \end{pmatrix}$.
\begin{enumerate}
...
$\DS \VEC{y} =  \begin{pmatrix} 0.3\overline{8} \\
0.1\overline{6} \end{pmatrix}$.  Hence
$\DS \VEC{x}_1 = \VEC{x}_0 - \VEC{y} =
\begin{pmatrix} 0.6\overline{1} \\ 0.8\overline{3} \end{pmatrix}$.
\item $\DS f(\VEC{x}_1) = \begin{pmatrix} 0.4\overline{259}
...
$\DS \VEC{y} = \begin{pmatrix} 0.10745204 \\ -0.019161089
\end{pmatrix}$.
Hence, $\DS \VEC{x}_2 = \VEC{x}_1 - \VEC{y} =
\begin{pmatrix} 0.50365909 \\ 0.85249442 \end{pmatrix}$.
\item And so on.
\end{enumerate}
We get
$\DS \|\VEC{x}_k - \VEC{x}_{k-1}\|_\infty < 10^{-6}$ for $k\geq 5$.
So $\DS \VEC{x}_5 = \begin{pmatrix} 0.50000000 \\
...
f(\VEC{x}) = \VEC{0} \ ,
\end{equation}
where $\DS f:\RR^n \rightarrow \RR^n$ is continuously differentiable.
...
$\DS \{ \VEC{x} \}_{k=0}^\infty$ from an iterative formula of the
form
...
$\DS \{ \VEC{x} \}_{k=0}^\infty$ and thus a solution of
$f(\VEC{x})=\VEC{0}$.
...
The sequence $\DS \{ \VEC{x} \}_{k=0}^\infty$ produced by the 
...
as follows.
\begin{enumerate}
\item $\DS A_0 = J_f(\VEC{x}_0)$.  This is the only time that
...
vectors such that $\DS \VEC{y}^\top A^{-1} \VEC{x} + 1 \neq 0$, then
$\DS A + \VEC{x}\VEC{y}^\top$ is nonsingular and
...
$\DS \Id + A^{-1} \VEC{x}\VEC{y}^\top$ is nonsingular to prove that
$\DS A + \VEC{x} \VEC{y}^\top$ is nonsingular.

If $\DS \VEC{z} \in \KE\left( \Id
...
\]
We conclude that
$\DS \KE\left( \Id + A^{-1} \VEC{x}\VEC{y}^\top \right)
= \left\{ \VEC{0}\right\}$
and $\DS \Id + A^{-1} \VEC{x}\VEC{y}^\top$ is nonsingular.

To prove that the right hand side of (\ref{iter_NLE_qn3}) is the
inverse of $\DS A + \VEC{x} \VEC{y}^\top$, it suffices to multiply
the right hand side of (\ref{iter_NLE_qn3}) by
$\DS A + \VEC{x} \VEC{y}^\top$.  This is a simple
...
\end{align*}
Once $\DS A_0^{-1}$ has been computed, it becomes
``relatively easy'' to compute the $\DS A_k^{-1}$ with the
...
f(\VEC{x})  = 0 \ ,
\end{equation}
where $\DS f:\RR^n \rightarrow \RR$ is continuously
...
$\DS \VEC{x}_{k+1} = \VEC{x}_k - t_k \nabla f(\VEC{x}_k)$.
Repeating this procedure, we hope to get a sequence of vectors
$\DS \{\VEC{x}_k\}_{k=0}^\infty$ converging toward the
solution of $f(\VEC{x})$.
...
$\DS S = \{ \VEC{x} \in \RR^2 : -1\leq x_1,x_2 \leq 1 \}$.\\
\subQ{b} Use the fixed point method to approximate
the fixed point of $g$ in $S$ with an accuracy of $\DS 10^{-5}$.\\
\subQ{c} Let $\VEC{p}$ be the fixed point of $g$ in $S$, find a small value
of $n$ for which $\DS \| \VEC{x}_n - \VEC{p}\|_\infty < 10^{-5}$, where
$\DS \{\VEC{x}_n\}_{n=1}^\infty$ is the sequence generated by
...
solution of $f(\VEC{x}) = \VEC{0}$ with an accuracy of $\DS 10^{-5}$.
Start with $\VEC{x}_0 = \VEC{0}$.\\
\subQ{e} Determine a small value of $n$ for which
$\DS \| \VEC{x}_n - \VEC{p}\|_\infty < 10^{-5}$, where $\VEC{p}$ is the
...
$\DS \|\VEC{x}_n - \VEC{x}_{n-1}\|_\infty < 10^{-5}$ for the
...
$\DS \VEC{x}_1 = \begin{pmatrix} 1/4 & 1/6 \end{pmatrix}^\top$.
...
$f(\VEC{x})=\VEC{0}$ to within $\DS 10^{-5}$, where
\[
f(\VEC{x}) =
\begin{pmatrix}
4 x_1 - x_2 - 5 \\
...
We have that $\DS g:\RR^2 \to \RR^2$ satisfies the two
hypotheses of the Fixed Point Theorem, Theorem~\ref{FixedPTinRN}.
\begin{enumerate}
\item Since 
$\DS (x_2+5)/4 \leq \big((3/4) + 5\big)/4 = 23/16 < 2$,
$\DS (x_2+5)/4 \geq \big((1/4) + 5\big)/4 = 21/16 > 1$,
$\DS (1+\sqrt{x_1})^{1/3} - 1 \leq (1+\sqrt{2})^{1/3} - 1 =
0.3415\ldots < 3/4$ and
$\DS (1+\sqrt{x_1})^{1/3} - 1 \geq (1+\sqrt{1})^{1/3} - 1 =
...
$J_g$ of $g$ satisfies\\
$\DS \max_{\VEC{x} \in S} \|J_g(x)\|_\infty < 1$ and use
Remark~\ref{rmkKcondFP}.  Since
\[
J_g(\VEC{x}) = \begin{pmatrix}
\DS \pdydx{g_1}{x_1}(\VEC{x}) &
\DS \pdydx{g_1}{x_2}(\VEC{x}) \\[0.7em]
\DS \pdydx{g_2}{x_1}(\VEC{x}) &
\DS \pdydx{g_2}{x_2}(\VEC{x})
\end{pmatrix}
= \begin{pmatrix}
0 & \DS \frac{1}{4} \\[0.7em]
\DS \frac{1}{6\sqrt{x_1}\,(1+\sqrt{x_1})^{2/3}} & 0
\end{pmatrix} \ ,
\]
we get that
\[
...
$S$ with
$\DS K = \max_{\VEC{x} \in S} \|J_g(\VEC{x})\|_\infty < 1$.
\end{enumerate}

Starting with
$\DS \VEC{x}_0 = \begin{pmatrix} 1.5 & 0.5 \end{pmatrix}^\top$,
we compute $\VEC{x}_{k+1} = g(\VEC{x}_k)$ until
$\DS \|\VEC{x}_{k+1} - \VEC{x}_k\|_\infty < 10^{-5}$.
It takes $7$ iterations to get the first approximation
$\DS \VEC{x}_7 \approx \begin{pmatrix} 1.32266994 & 0.29067777
...
$\DS 10^{-5}$.  Start with
$\DS \VEC{x}_0 = \begin{pmatrix} 0 & 1 \end{pmatrix}^\top$.\\
...
$\DS 10^{-5}$.  Start with
$\DS \VEC{x}_0 = \begin{pmatrix} 1 & 1 & 1 \end{pmatrix}^\top$.\\
...
with an accuracy $\DS 10^{-6}$, where
\[
f(\VEC{x}) = \begin{pmatrix}
...
\DS \pdydx{f_1}{x_1}(\VEC{x}) &
\DS \pdydx{f_1}{x_2}(\VEC{x}) &
\DS \pdydx{f_1}{x_3}(\VEC{x}) \\[0.7em]
\DS \pdydx{f_2}{x_1}(\VEC{x}) &
\DS \pdydx{f_2}{x_2}(\VEC{x}) &
\DS \pdydx{f_2}{x_3}(\VEC{x}) \\[0.7em]
\DS \pdydx{f_3}{x_1}(\VEC{x}) &
\DS \pdydx{f_3}{x_2}(\VEC{x}) &
\DS \pdydx{f_3}{x_3}(\VEC{x})
\end{pmatrix}
= \begin{pmatrix}
...
\]
until $\DS \| \VEC{x}_{k+1} - \VEC{x}_k \| < 10^{-6}$.  It takes $12$
iterations to get the first approximation\\
$\DS \VEC{x}_{12} \approx \begin{pmatrix}
