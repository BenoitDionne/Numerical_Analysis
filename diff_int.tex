...
truncation error is the term $\DS -\frac{1}{6}f^{(3)}(\eta)\,h^2$.
\end{rmk}
...
Moreover, if we assume that $\DS f^{(3)}$ is almost constant near $a$,
...
$\DS |E_- - E_+| \approx 10^{-8}$ and $\eta \approx 2$, then
\[
...
$\DS R(y)=10^{-8}/(2h) + h^2/24$.}{errorgraph}
...
for some $c \in \RR$ (or $\DS L(f) = \int_a^b f(x) \dx{x}$
...
If we replace $h$ in (\ref{oneone}) by $h/2$ and $\DS h/2^2$, we
respectively get
\begin{align}
...
$\DS - K_2\,h^4/4 + O(h^6)$.  Recall that the expression
$\DS O(h^k)$ replaces a function $g(h)$ for which there
exists a constant $M$ such that $\DS |g(h)| \leq M h^k$ for
$h$ closed to the origin.
In theory, $\DS L_{h/2}^1(f)$ is a better approximation of
$L(f)$ than $L_{h/2}(f)$ because, for $h<1$ given, the truncation error for
$\DS L_{h/2}^1(f)$ in (\ref{twotwo}) is generally smaller than the
...
\]
is an approximation of $L(f)$ with truncation error $\DS O(h^4)$.

If we subtract (\ref{twotwo}) from $\DS 4^2$ times
(\ref{threetwo}) and divide the result by $\DS 4^2-1$, we get
\begin{align}
...
\]
is an approximation of $L(f)$ with truncation error
$\DS K_3 h^6/64 + O(h^8)$.  We may assume 
that $\DS L_{h/2^2}^2(f)$ is the best approximation of
$L_h(f)$ that we have found so far in this section.  For small $h<1$,
the truncation error is generally smaller for
$\DS L_{h/2^2}^2(f)$ than for the other
approximations.
...
\end{array}
\]
where $\DS L_{h/2^k}^0(f) = L_{h/2^k}(f)$.  The general formula is
...
\]
for all $h$.  Replacing $h$ by $\DS h/2^k$ with
...
\begin{rmkList}
\begin{enumerate}
\item Before using $\DS L_{h/2^k}^n$ as a good approximation
...
proposition,
\begin{align*}
\frac{\DS L_{h/2^k}^{n-1}(f) -L_{h/2^{k-1}}^{n-1}(f)}
{\DS L_{h/2^{k+1}}^{n-1}(f) -L_{h/2^k}^{n-1}(f)}
&= \frac{\DS \sum_{j=n}^\infty \hat{K}_{j,n-1}
\left( \left(\frac{h}{2^k}\right)^{2j} -
\left(\frac{h}{2^{k-1}}\right)^{2j} \right)}
{\DS \sum_{j=n}^\infty \hat{K}_{j,n-1}
\left( \left(\frac{h}{2^{k+1}}\right)^{2j} -
 \left(\frac{h}{2^k}\right)^{2j} \right)} \\
&= \frac{\DS \sum_{j=n}^\infty \hat{K}_{j,n-1}
\left( \left(\frac{h}{2^k}\right)^{2j} -
\left(\frac{h}{2^{k-1}}\right)^{2j}\right)}
{\DS \sum_{j=n}^\infty \hat{K}_{j,n-1} 2^{-2j}
\left( \left(\frac{h}{2^k}\right)^{2j} -
\left(\frac{h}{2^{k-1}}\right)^{2j}\right)} \ .
\end{align*}
...
$\DS L_{h/2^{k+1}}^{n-1}(f) \approx L_{h/2^k}^{n-1}(f)$,
...
\end{align*}
With $\DS h=1.6/2^n$ for $0 \leq n \leq 7$, we give the values of
...
$\DS L_{0.05}^4(f) \approx 0.54030230587$.  The exact value is
...
$\DS \int_a^b\,f(x) \dx{x}$ is given by
$\DS \int_a^b\,p(x) \dx{x}$,
...
$\DS \int_a^b f(x)\dx{x} \approx
\frac{f(a) + f(b)}{2}(b-a)$
with the truncation error\\
$\DS -f''(\xi)\frac{(b-a)^3}{2}$.
\end{enumerate}
\end{rmkList}
...
$\DS (x-a)\left(x-\frac{a+b}{2}\right)(x-b)$ changes
same sign on $[a,b]$.  But, from
\[
f\left[\frac{a+b}{2},a,\frac{a+b}{2},b,x\right] =
\frac{\DS f\left[a,\frac{a+b}{2},b,x\right] -
...
$\DS (x-a)\left(x-\frac{a+b}{2}\right)(x-b)$ is like an odd
function with respect to the line $\DS x= \frac{a+b}{2}$.  We can
use the Mean Value Theorem for Integrals for the second
integral because
$\DS (x-a)\left(x-\frac{a+b}{2}\right)^2(x-b)$ does not
...
We consider the interpolating polynomial $p$ of $f$ at
the point
$\DS x_0=\frac{a+b}{2}$; namely,
$\DS p(x) = f\left(\frac{a+b}{2}\right)$.  We have that
$\DS f(x) = f\left(\frac{a+b}{2}\right)
...
We cannot use the Mean Value Theorem for Integrals to
evaluate this integral because
$\DS \left(x-\frac{a+b}{2}\right)$ changes same sign on
$[a,b]$.  But, from
\[
f\left[\frac{a+b}{2},\frac{a+b}{2},x\right] =
\frac{\DS f\left[\frac{a+b}{2},x\right] -
...
\]
The first integral is $0$ because
$\DS \left(x-\frac{a+b}{2}\right)$ is like an odd function
with respect to the line $\DS x=\frac{a+b}{2}$.  We can use
the Mean Value Theorem for Integrals in the second integral
because $\DS \left(x-\frac{a+b}{2}\right)^2$ does not change
...
&= \frac{f''(\xi)}{2!}\,
\frac{\DS \left(x -\frac{a+b}{2}\right)^3}{3}\bigg|_a^b
= \frac{f''(\xi)}{24}\,(b-a)^3 \ .
\end{align*}
The second equality follows from Theorem~\ref{InterpTh} for some
$\xi$ between $a$ and $b$.
...
$\DS \int_a^b f(x)\dx{x}$ if the
interval $[a,b]$ is large.  To get better approximations of
$\DS \int_a^b f(x) \dx{x}$, we divide the interval $[a,b]$
...
approximation of
$\DS \int_{x_{i-1}}^{x_i} f(x) \dx{x}$ for $1 \leq i \leq n$
...
approximation of
$\DS \int_{x_{2i-2}}^{x_{2i}} f(x) \dx{x}$ for $1 \leq i \leq m$
gives an approximation of $\DS \int_a^b f(x) \dx{x}$.
...
$\DS \int_{x_{2i-2}}^{x_{2i}} f(x) \dx{x}$ for
...
\begin{rmk}
We have $\DS \int_a^b f(x)\dx{x} \approx
...
\]
We have $\DS f(x)=e^{x^2}$, $a=0$, $b=1$, $x_j = jh$ and
$h = 1/n$.  Since $\DS |f''(x)|=(2+4x^2)e^{x^2}$,
we have that $|f''(x)|\leq 6e$ for $x \in [0,1]$.  The magnitude of
the truncation error is at most $\DS \frac{e}{2n^2}$.  We
choose $n$ such that $\DS \frac{e}{2n^2} <10^{-4}$; namely, 
...
that $\DS |f^{(4)}(x)| \leq 76e$ on $[0,1]$.  Thus, the
magnitude of the truncation error is at most
$\DS \frac{76e}{180(2m)^4} = \frac{19e}{720 m^4}$.  We
choose $m$ such that $\DS \frac{19e}{720 m^4} < 10^{-4}$;
...
$\DS \int_0^1 e^{x^2}\dx{x}$.
Choose the number of subintervals such that the magnitude of the
truncation error is smaller than $\DS 10^{-4}$.  Compare with
...
\]
We have $\DS f(x)=e^{x^2}$, $a=0$, $b=1$, $x_j = j\,h$ and
$h = 1/n$ where $n=2m$.  Since
$\DS |f''(x)|=(2+4x^2)e^{x^2}$, we have that
$|f''(x)|\leq 6e$ for $x \in [0,1]$.  The magnitude of the
truncation error is at most $\DS \frac{e}{(2m)^2}$.  We
choose $m$ such that $\DS \frac{e}{(2m)^2} < 10^{-4}$;
...
with $\DS L(f) = \int_{a}^{b} f(x)\dx{x}$ and
...
$\tilde{n} = 2n$,
$\DS \tilde{h} = \frac{b-a}{\tilde{n}} = \frac{h}{2}$ and
...
$\DS L_{h/2^k}^{n}(f) \rightarrow L(f)$ as
$k\rightarrow \infty$.
Then,
\begin{align*}
\lim_{k\rightarrow \infty} L_{h/2^k}^{n+1}(f) &=
\frac{\DS 4^{n+1} \lim_{k\rightarrow \infty}L_{h/2^k}^n(f)
- \lim_{k\rightarrow \infty}L_{h/2^{k-1}}^n(f) }
{4^{n+1} - 1} \\
&= \frac{\DS 4^{n+1} \int_a^b f(x) \dx{x} - \int_a^b f(x) \dx{x}}
{4^{n+1} - 1}
= \int_a^b f(x) \dx{x} \ .
\end{align*}
...
$b=x_i$ and $\DS h = h_i = (x_i-x_{i-1})/2$
...
$\DS h = h_i/2 = (x_i-x_{i-1})/4$ in Theorem~\ref{CSR}, we get
...
\end{align*}
for $\DS h_i = (x_i-x_{i-1})/2$ and $\mu_i$ between
$x_{i-1}$ and $x_i$.

If we assume that $\DS f^{4}(x)$ is almost constant on
$[x_{i-1},x_i]$, we may suppose that
$\DS f^{4}(\eta_i) \approx f^{4}(\mu_i)$.  Hence
$\DS R_i \approx 16 \tilde{R}_i$.

If we subtract $I_i = \tilde{S}_i + \tilde{R}_i$ from
$\DS I_i = S_i + R_i = \approx S_i + 16 \tilde{R}_i$, we get
$\DS 0 \approx (S_i - \tilde{S}_i) + 15\tilde{R}_i$.  Thus
$\DS \tilde{R}_i \approx
...
almost equals to
$\DS \frac{1}{15}\left( \tilde{S}_i-S_i \right)$.
...
$\DS \sum_{i=1}^n \frac{1}{15}\left( \tilde{S}_i-S_i\right)$ of the
...
approximation of $\DS \int_a^b \sqrt{x}\dx{x}$ given by the
...
$\DS \tilde{R}_{[0,1]} \approx \frac{1}{15}\left|
...
$\DS \tilde{R}_{[0,0.5]} \approx \frac{1}{15}\left|
...
$\DS \tilde{R}_{[0,0.25]} \approx \frac{1}{15}\left|
...
and\\
$\DS \tilde{R}_{[0.125,0.25]} \approx \frac{1}{15}\left|
...
$\DS \int_{1/8}^{1/4} \sqrt{x}\dx{x}$; namely,\\
$\DS \int_{1/8}^{1/4} \sqrt{x}\dx{x} \approx \tilde{S}_{[0.125,0.25]}
= 0.05387027$.\\
Hence,
$\DS \int_0^{1/4} \sqrt{x}\dx{x} =
...
$\DS \tilde{R}_{[0.25,0.5]} \approx \frac{1}{15}\left|
  \tilde{S}_{[0.25,0.5]} - S_{[0.25,0.5]}\right|
\approx 0.664 \times 10^{-6} < 0.000125$.\\
So, we accept $\tilde{S}_{[0.25,0.5]}$ as an
approximation of $\DS \int_{1/4}^{1/2} \sqrt{x}\dx{x}$;
namely,\\
$\DS \int_{1/4}^{1/2} \sqrt{x}\dx{x} \approx \tilde{S}_{[0.25,0.5]}
= 0.15236815$.\\
Hence,
$\DS \int_0^{1/2} \sqrt{x}\dx{x} =
...
$\DS \tilde{R}_{[0.5,1]} \approx \frac{1}{15}\left|
\tilde{S}_{[0.5,1]} - S_{[0.5,1]}\right|
\approx 0.18773 \times 10^{-5} < 0.00025$.\\
So, we accept $\tilde{S}_{[0.5,1]}$ as an
approximation of $\DS \int_{1/2}^{1} \sqrt{x}\dx{x}$; namely,\\
$\DS \int_{1/2}^{1} \sqrt{x}\dx{x} \approx \tilde{S}_{[0.5,1]} = 
0.43096219$.\\
Hence,
$\DS \int_0^1 \sqrt{x}\dx{x} =
...
$\DS \int_a^b f(x) \dx{x}$ is in improper integral.
...
$\DS \int_a^b\, 1 \dx{x} = c_1$ and
$\DS \int_a^b\, x \dx{x} = c_1 x_1$; namely,
$a+b = c_1$ and $\DS \frac{b^2}{2} - \frac{a^2}{2} = c_1 x_1$.
...
$\DS \int_a^b\, 1 \dx{x} = c_1 + c_2$,
$\DS \int_a^b\, x \dx{x} = c_1 x_1 + c_2 x_2$,
$\DS \int_a^b\, x^2 \dx{x} = c_1 x_1^2 + c_2 x_2^2$ and
$\DS \int_a^b\, x^3 \dx{x} = c_1 x_1^3 + c_2 x_2^3$;
Namely, $b-a = c_1 + c_2$,
$\DS \frac{b^2-a^2}{2} = c_1 x_1 + c_2 x_2$,
$\DS \frac{b^3-a^3}{3} = c_1 x_1^2 + c_2 x_2^2$ and
$\DS \frac{b^4-a^4}{4} = c_1 x_1^3 + c_2 x_2^3$.
The values of $c_1$, $c_2$, $x_1$ and $x_2$ satisfying these four
nonlinear equations are 
$\DS c_1 = c_2 = \frac{b-a}{2}$, $x_1 = z$ and
$x_2 = a+b-z$, where $z$ is the positive root of
$\DS 6z^2 - 6(a+b)z + a^2 + b^2 + 4ab$.
...
$\DS p(x) = \prod_{i=1}^n (x-x_i)^2$, we ask the reader in
...
$\DS q= \sum_{i=0}^{n-1} \, \alpha_i P_i$ for some constants
...
that
$\DS \int_a^b w(x)\dx{x} = \sum_{j=1}^n c_j$.
\label{sumCoeff}
\end{rmk}
...
$\DS \int_{-1}^1 f(x) \dx{x}$ is approximately equal to
$\DS \sum_{j=1}^n c_j f(x_j)$, where the $x_j$'s are the
...
$\DS \int_a^b f(x) \dx{x}$ into an integral of the form
$\DS \int_{-1}^1 g(t) \dx{t}$, one uses the substitution
$\DS t = \frac{x - (a+b)/2}{(b-a)/2}$,
...
$\DS \int_{-1}^1 \frac{p(x)}{\sqrt{1-x^2}}\dx{x}
= c_1 p(x_1) + c_2 p(x_2)$
...
\frac{\epsilon}{\DS 2\int_a^b w(x) \dx{x}} \ .
\]
...
&\qquad \leq \frac{\epsilon}{\DS 2 \int_a^b w(x) \dx{x}} \left(
...
differences \\
$\DS g[x_1,x_2,\ldots,x_n,x_1,x_2,\ldots,x_n,x_j]$
are well defined and
$\DS \prod_{i=1}^n (x_j-x_i)^2 = 0$ for all $j$.

Since $\DS \prod_{i=1}^n (x-x_i)^2 w(x) \geq 0$ for
...
$\DS B_1(x) = x - \frac{1}{2}$,
$\DS B_2(x) = x^2 - x + \frac{1}{6}$ and
$\DS B_3(x) = x^3 - \frac{3}{2}x^2 + \frac{1}{2}x$.
\end{rmk}
...
\item $\DS B_n(x+1) - B_n(x) = n x^{n-1}$ for $n\geq 1$.
\item $\DS B_n(x) = \sum_{j=0}^n \binom{n}{j} B_j(0) x^{n-j}$
for $n \geq 0$.
\item $\DS B_n(1-x) = (-1)^n B_n(x)$ for $n \geq 0$.
\end{enumerate}
\label{BePoProp}
\end{prop}
...
$B_0(x) = 1$ and $\DS B_1(x) = x - \frac{1}{2}$.  We assume
...
$\DS B_n'(x) = \binom{n}{n-1} B_{n-1}(x) = nB_{n-1}(x)$
which proved (1).
...
\end{equation}
If $\DS F(x) = (-1)^n B_n(x) - B_n(1-x)$ for all $x$,
...
$\DS B_j(0) = B_j(1) = (-1)^j B_j(0)$ for all $j\geq 2$.
...
$\eta_2 \in ]\eta,1[$, of
$\DS G_n'(x) = B_{2n}'(x) = 2nB_{2n-1}(x)$ in $]0,1[$.
...
$\DS B_{2k-1}'(x) = (2k-1)B_{2k-2}(x)$ in $]0,1[$.
...
$\DS B_{2k-2}'(x) = (2k-2)B_{2k-3}(x)$ in $]0,1[$.
...
$\DS \dfdx{f[x_0,x_1,x_2,x]}{x} = f[x_0,x_1,x_2,x,x]$.
Since
$\DS f[x_0,x_1,x_2,x] = \frac{1}{3!}\dydxn{f}{x}{3}(\xi)$
and
$\DS f[x_0,x_1,x_2,x,x] = \frac{1}{4!}\dydxn{f}{x}{4}(\eta)$
...
$\DS \frac{1}{3}\dydxn{f}{x}{3}(\xi)\,h^2$.
}
...
$\DS \dfdx{f[x_0,x_1,x_2,x,x]}{x} = 2\,f[x_0,x_1,x_2,x,x,x]$.
Since
$\DS f[x_0,x_1,x_2,x] = \frac{1}{3!}\dydxn{f}{x}{3}(\xi)$,
$\DS f[x_0,x_1,x_2,x,x] = \frac{1}{4!}\dydxn{f}{x}{4}(\eta)$
and
$\DS f[x_0,x_1,x_2,x,x,x] = \frac{1}{5!}\dydxn{f}{x}{5}(\nu)$
...
&= 2\,\left(\frac{\DS\frac{f(a+2h)-f(a+h)}{h} -
...
\sol{\SOL}{
Let $\DS L^0_h(f) = L_h(f)$.  We prove by induction on $n$ that
...
If we subtract (\ref{questB3}) from $\DS 2^{2m+1}$
times (\ref{questB4}) and divide by $\DS 2^{2m+1}-1$, we get that
\begin{align*}
...
\]
for $k \geq n > 0$ with a truncation error of $\DS O(h^{2n+1})$.
}
...
\end{equation}
If we subtract (\ref{questC1}) from $\DS 2^3$ times
(\ref{questC2}) and divide by $\DS 2^3-1$, we get that
\begin{align*}
...
If we subtract (\ref{questC3}) from $\DS 2^{3m+3}$ times
(\ref{questC4}) and divide by $\DS 2^{3m+3}-1$, we get that
\begin{align*}
&L(f) = L_{h/2^{k+1}}^{m+1}(f) \\
...
\]
for $k \geq n > 0$ with a truncation error of $\DS O(h^{3n+3})$.
}
...
accuracy of $\DS 10^{-7}$.  Start with $h=0.8$.
\end{question}
...
$\DS |L_h^i(f) - L_{2h}^{i-1}(f)|$ gets
smaller than $\DS 10^{-7}$ and take
$\DS L_h^i(f)$ as our approximation of
$f'(3)$.  We have also included the ratios defined in
(\ref{checkRich}) to ensure that the approximating values
$\DS L_h^i(h)$ can be trusted.  We have that
$\DS f'(3) \approx L_{0.05}^4(f) \approx 0.15161081$ meets
...
\]
with an accuracy of $\DS 10^{-5}$.  You have to find a number of
subintervals of $[1,3]$ (and so a step size $h$) such that the local
truncation error is smaller than $\DS 10^{-5}$.
\end{question}
...
$\DS - \frac{f''(\xi)(b-a)}{6}\,h^2$ for some
$\xi \in [a,b]$.  We seek a small $m$ for which this 
truncation error will be smaller in absolute value than $\DS 10^{-5}$.
We have
\[
...
$\DS f^{(3)}(x) = -1/x^2 + 1/4$ in the interval $[1,3]$.  Since
...
$\DS \frac{3}{m^2} < 10^{-5}$; namely,
$\DS m > \sqrt{ 3 \times 10^5} \approx 547.72$.
...
$\DS \frac{8}{3^{29/3}\,m^4} < 10^{-5}$; namely,\\
$\DS m > \left( 8 \times 10^5 / 3^{29/3}\right)^{1/4}
\approx 2.102468339$\ .
...
\]
to within $\DS 10^{-5}$ and compute the approximation with
this value of $n$.\\
...
$\DS \left| \frac{f''(\eta)(b-a)}{6} \, h^2  \right|$ for
...
about $\DS 0.82551686 \times 10^{-5}$.

\subQ{c} For the Simpson rule, we choose $n = 2m$ and
$h = (b-a)/n = 1/m$ such that the truncation error
$\DS \left| \frac{f^{(4)}(\eta)(b-a)}{180} \, h^4  \right|$
...
exact for $\DS x^i$ with $i=0$, $1$, $2$ and $3$.  For
$\DS f(x) = x^i$, Simpson's rule becomes
...
\int_2^4 (x+1)^{1/3} \dx{x}
\]
with an accuracy of $\DS 10^{-5}$.  Start with two subdivisions of the
interval $[2,4]$.
\end{question}
...
Note that
$\DS L_{1/2}^1(f) = (4L_{1/2}^0(f)-L_1^0(f))/(4-1)$,
$\DS L_{1/4}^1(f) = (4L_{1/4}^0(f)-L_{1/2}^0(f))/(4-1)$
and
$\DS L_{1/4}^2(f) = (4^2L_{1/4}^1(f)-L_{1/2}^1(f))/(4^2-1)$.

The requested approximation is $3.16734826$ because
$\DS \left| L_{1/2}^1(f) - L_1^0(f) \right| \approx 0.0038295
> 10^{-5}$ and
$\DS \left| L_{1/4}^2(f) - L_{1/2}^1(f) \right| \approx
...
\]
with an accuracy of $\DS 10^{-5}$.  Start with one subdivision of the
interval $[3,5]$.
\end{question}
...
We stopped the procedure when $\DS |L_h^i(f) - L_{2h}^{i-1}(f)|$ got
smaller than $\DS 10^{-5}$ and took
$\DS L_h^i(f)$ as our approximation of the integral.  We have that
\[
...
if the values of $\DS L_h^i(h)$ can be trusted.  However, we
...
diagonal line is smaller than $\DS 10^{-5}$.
\label{diffQ13}
\end{question}
...
$\DS -0.88 \times 10^{-9}$.  This is better than expected.
...
$\DS L_{0.25}^2(f) \approx 6.99862022$ associated to the ratio
$15.17336693$.  The absolute error for this approximation is about
$\DS 0.14891 \times 10^{-5}$.  Despite the fact that the
ratios were not respecting the rule of $\DS 4^i$, we did the
...
\int_3^5 (x-2)^{1/4} \dx{x}
\]
with an accuracy of $\DS 10^{-5}$.  For this purpose and to
...
$\DS \int_a^b (x-2)^{1/4} \dx{x}$ with $m = (b-a)/(2h)$.
The values displayed in the following computations have been rounded
...
\end{array}
\]
$h=0.5$, $\DS T=0.5\times 10^{-5}$ (store for $[4, 5]$),
...
$\DS \int_a^b f(x)\dx{x}$, where $a$ and $b$ are any real
numbers.
\end{question}
...
the monomials $\DS x^i$ for $0 \leq i \leq 4$, it is enough to show
that (\ref{NF01}) is true for $\DS f(x) = x^i$ with
$0\leq i \leq 4$ to
...
\sol{\SOL}{
For $\DS f(x) = x^i$, the formula is
\[
\int_0^1 x^i \dx{x} \approx A\, \left( x_0^i + x_1^i \right) \ .
\]
For $i=0$, we have $\DS 1 = \int_0^1 \dx{x} = 2\,A$.
...
$\DS x_0= \frac{1}{2} + \frac{1}{2\sqrt{3}}$ and
$\DS x_1= \frac{1}{2} - \frac{1}{2\sqrt{3}}$, and
$\DS x_0= \frac{1}{2} - \frac{1}{2\sqrt{3}}$ and
$\DS x_1= \frac{1}{2} + \frac{1}{2\sqrt{3}}$.
...
\sol{\SOL}{
We get $\DS A + B = \int_0^{2\pi} \dx{x} = 2\pi$ when $k=0$
and
$\DS A \cos(0) + B \cos(\pi) = \int_0^{2\pi} \cos(x) \dx{x}
...
Since $\DS f[x_0]= f(x_0) = f\left(\frac{2a+b}{2}\right)$
and
\[
f[x_0,x_1] = 
...
such that $\DS |f^{(3)}(x)|<M$ for all $x\in[a,b]$.
\end{question}
...
$\DS v'(x) = (x-x_2)^2$ for the first integral on the right hand side
...
$\DS \int_a^b f(x)\dx{x}$ and its truncation error.\\
...
\subQ{b} To obtain a formula for $\DS \int_a^b f(x)\dx{x}$ from
...
Since $\DS (x+2h)(x+h)x^2$ is non-negative on the interval
...
$\DS f[-2h,-h,0,0,\eta] = \frac{1}{4!} f^{(4)}(\xi_1)$
and
$\DS f[-2h,-h,0,0] = \frac{1}{3!} f^{(3)}(\xi_2)$.
...
$\DS x= \frac{b+a}{2} + t$ with $\DS h = \frac{b-a}{2}$
to get
\begin{align*}
...
that (\ref{G_quadr}) is true for $\DS p(x) = x^m$ with
$0 \leq m <k$.  Namely, 
\[
...
matrix $A$ above is $\DS \prod_{0<i<j\leq k} (x_j - x_i)$.
}
...
\]
Since $A_1=1$,
$\DS B_1 = \frac{\int_{-1}^1 x^5 \dx{x}}{\int_{-1}^1 x^4 \dx{x}}
= 0$ and
$\DS C_1 = \frac{\int_{-1}^1 x^4 \dx{x}}{\int_{-1}^1 x^2\dx{x}}
=\frac{3}{5}$, we get
\[
...
function.  If
$\DS \max_{a\leq x \leq b} |f^{(n+1)}(x)| < M$ for some
...
If we substitute $\DS x= \frac{a+b}{2} + \frac{b-a}{2}\,t$
...
\dx{t} \ .
\end{align*}
$\DS p\left(\frac{a+b}{2} + \frac{b-a}{2}\,t\right)$ is the
interpolating polynomial of
$\DS f\left(\frac{a+b}{2} + \frac{b-a}{2}\,t\right)$ at the
Chebyshev points
$\DS t_i = \cos\left( \frac{(2i-1)\pi}{2(n+1)}\right)$
for $1\leq i \leq n+1$.
