\chapter{Least Square Approximation (in $\displaystyle L^2$)}\label{chaptApproxA}

To understand the foundation of least square approximation, we first 
need to briefly review $\displaystyle L^2$ spaces.  The reader will notice many
similarities with linear algebra in $\displaystyle \RR^n$.

\section{$\displaystyle L^2$ spaces}
...
$\displaystyle L^2(\Omega)$ be the space of measurable functions
...
\]
Equipped with this norm, $\displaystyle L^2(\Omega)$ is a
...
combinations of elements of $S$ is dense in $\displaystyle L^2(\Omega)$.
...
index set, be a orthonormal basis of $\displaystyle L^2(\Omega)$.  The
...
function $\displaystyle f \in L^2(\Omega)$ with respect to the
...
orthonormal subset of $\displaystyle L^2[a,b]$.  Given
$\displaystyle f\in L^2[a,b]$, we have
\[
...
is the complex number satisfying $\displaystyle i^2=-1$.  Since
\begin{align*}
...
set in the space $\displaystyle L^2[-\pi,\pi]$ with the Lebesgue measure.  If we
replace $\phi_n$ by $\displaystyle (2\pi)^{-1/2} \phi_n$, we get an
orthonormal set.  It can be shown that the set of all finite linear combinations
of elements of $S$ is dense in $\displaystyle L^2[-\pi,\pi]$.  Hence, $S$ is a
complete orthogonal set in $\displaystyle L^2[-\pi,\pi]$.

For $\displaystyle f\in L^2[-\pi,\pi]$, we have that
...
$\displaystyle L^2[a,b]$ is the space of measurable functions
$f:[a,b]\rightarrow \RR$ such
that $\displaystyle \int_a^b f^2(x) w(x) \dx{x}$ is finite.  The scalar
product on $\displaystyle L^2[a,b]$ is
...
\end{equation}
The space $\displaystyle L^2[a,b]$ has a countable orthonormal basis.
Suppose that
\[
S = \{\phi_n : n \in \NN \} \subset L^2[a,b]
\]
is an orthonormal basis for $\displaystyle L^2[a,b]$, the Fourier
...
$\displaystyle L^2$ to $f$ as $N\rightarrow \infty$.
...
orthonormal functions in $\displaystyle L^2[-\pi,\pi]$, where the
...
Hence, for $\displaystyle f\in L^2[-\pi,\pi]$, we have that
...
sense of convergence in $\displaystyle L^2[-\pi,\pi]$; namely,
...
These polynomials can be considered as elements of $\displaystyle L^2[a,b]$.
...
is $\displaystyle c_j \alpha_{n_j,n_j} x^{n_j}$.  Hence
$\displaystyle c_j \alpha_{n_j,n_j}x^{n_j} = 0$
with $\alpha_{n_j,n_j} \neq 0$.  Thus $c_j=0$.  We
therefore get
...
$\displaystyle x^{k+1}$ in $p(x)$.  Since $p$ and $P_{k+1}$ are of
...
density of continuous functions in $\displaystyle L^2[a,b]$, we may
show that the set of all finite linear combinations of elements of the set
$P = \displaystyle \{ P_n : n \in \NN \}$ is dense in
$\displaystyle L^2[a,b]$.   Combined with the linear independence of
$P$, this shows that $P$ is a basis of $\displaystyle L^2[a,b]$.
...
\]
If we consider only the terms in $\displaystyle x^{n_j}$, we get the following
...
\item If the coefficient of $\displaystyle x^k$ in $P_k$ is
$\alpha_{k,k}$, then
\[
P_{k+1}(x) = A_k(x-B_k)P_k(x) - C_k P_{k-1}(x)
\]
for $k \geq 0$, where $P_{-1} = 0$,
$\displaystyle A_k = \frac{\alpha_{k+1,k+1}}{\alpha_{k,k}}$ for $k \geq 0$,\\
$\displaystyle B_k = \frac{\displaystyle \int_a^b xP_k^2(x)w(x) \dx{x}}
{\displaystyle \int_a^b P_k^2(x)w(x)\dx{x}}$ for $k\geq 0$,\\
$C_0 = 0$ and $\displaystyle
...
of $\displaystyle x^n$ in $p$.  The constant $b$ is non-null because $p$ is of
degree exactly $n$.  Similarly, the coefficient $\alpha_{n,n}$ of
$\displaystyle x^n$ in $P_n$ is non null because $P_n$ is of degree
...
$[a,b]=[-1,1]$, the weight function is $\displaystyle w(x)=\sqrt{1-x^2}$,
and the coefficient $\alpha_{k,k}$ of $\displaystyle x^k$ in the
polynomial $P_k$ is $1$ for all $k$.

Due to our assumption on the coefficient of $\displaystyle x^i$ in
...
If, in the fourth item of Theorem~\ref{orthpoly}, we take $a=-1$,
$b=1$, $\displaystyle w(x)=(1-x^2)^{-1/2}$, $\alpha_{0,0} = 1$ and
$\displaystyle \alpha_{k,k} = 2^{k-1}$ for
$k\geq 1$, we get $P_0(x) = 1$, $P_1(x) = x$, $\displaystyle P_2(x) = 2x^2 -1$,
$\displaystyle P_3(x) = 4x^3 -3x$, and in general
\[
...
$a=-1$, $b=1$ and $\displaystyle w(x) = (1-x)^\alpha (1+x)^\beta$ with
$\alpha,\beta>-1$, we get the Jacobi polynomials
$\displaystyle P_k^{[\alpha,\beta]}$ for $k=0$, $1$, $2$, \ldots\   For each
...
$b=\infty$ and $\displaystyle w(t) = x^\alpha e^{-x}$ with $\alpha > -1$,
we get the Laguerre polynomials $\displaystyle l_k^{[\alpha]}$. 
\item If, in the fourth item of Theorem~\ref{orthpoly}, we take
$a=-\infty$, $b=+\infty$ and $\displaystyle w(x) = e^{-x^2}$, we get the 
...
$\displaystyle f\in L^2[a,b]$ by a finite
...
polynomials are $\displaystyle P_n(x) = x^n$ for all $n\in \NN$.  Then
\[
...
$\displaystyle \{ P_k \}_{k=0}^\infty$ such that $P_k$ is of degree $k$ and
\[
...
of the term in $\displaystyle x^n$ is $1$.
\end{question}

\sol{\SOL}{
Suppose that $\displaystyle \{ P_k^{[i]} \}_{k=0}^\infty$ for $i=1$ and
$2$ are two orthogonal families of monic polynomials such that
$\displaystyle \ps{p}{P_k^{[i]}}=0$ for all polynomial $p$ of degree
less than $k$. Given $k > 0$, we have that
$\displaystyle P_k^{[1]} - P_k^{[2]}$ is a polynomial
...
\]
Since $\displaystyle P_k^{[1]} - P_k^{[2]}$ is continuous on $[a,b]$,
we get that $\displaystyle P_k^{[1]}(x) = P_k^{[2]}(x)$ for all $x \in [a,b]$.
}
...
$\displaystyle x^{k+1}$ on both sides of (\ref{apprAQ2equ1}).
