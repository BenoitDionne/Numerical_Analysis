\chapter{Least Square Approximation (in $\DS L^2$)}\label{chaptApproxA}

To understand the foundation of least square approximation, we first 
need to briefly review $\DS L^2$ spaces.  The reader will notice many
similarities with linear algebra in $\DS \RR^n$.

\section{$\DS L^2$ spaces}
...
$\DS L^2(\Omega)$ be the space of measurable functions
$f:\Omega \rightarrow \CC$ such that
...
\]
Equipped with this norm, $\DS L^2(\Omega)$ is a
{\bfseries Hilbert space}\index{Hilbert Space}.

\begin{defn}
A set of functions
$\DS \{ \phi_\alpha \}_{\alpha \in A} \subset L^2(\Omega)$, where
$A$ is some index set, is
{\bfseries linearly independent}\index{$L^2$-Spaces!Linearly Independent}
if, for any finite subset
$\DS \left\{ \alpha_i \right\}_{i=1}^n \subset A$,
$\DS \sum_{i=1}^n c_i \phi_{\alpha_i} = 0$ with $c_i \in \CC$
implies that $c_1=c_2 = \ldots = c_n = 0$.
\end{defn}

\begin{defn}
A set of functions
$\DS S=\{ \phi_\alpha \}_{\alpha \in A} \subset L^2(\Omega)$, where
$A$ is some index set, is
...
\end{cases}
\]
If instead of $\DS \ps{\phi_{\alpha_1}}{\phi_{\alpha_2}} = 1$ for
$\alpha_1 \neq \alpha_2$, we have
$\DS \ps{\phi_{\alpha_1}}{\phi_{\alpha_2}} \neq 0$ for
...
orthonormal set
$\DS S= \{ \phi_\alpha : \alpha \in A \} \subset L^2(\Omega)$,
where $A$ is some index set, such that the set of all finite linear
combinations of elements of $S$ is dense in $\DS L^2(\Omega)$.
...
of functions $\DS \{ \phi_\alpha : \alpha \in A\}$, where $A$
is some index set, is complete if $\ps{f}{\phi_\alpha} = 0$
for all $\alpha \in A$ implies that $f = 0$ almost everywhere on
$\Omega$.

\begin{defn}
Let $\DS S= \{ \phi_\alpha : \alpha \in A \}$, where $A$ is some
index set, be a orthonormal basis of $\DS L^2(\Omega)$.  The
...
function $\DS f \in L^2(\Omega)$ with respect to the
orthogonal basis $S$ is
\[
f \sim \sum_{\alpha \in A} a_\alpha \phi_\alpha \ ,
\]
where
...
J \rightarrow \infty
\]
whatever the ordering $\DS \{ \alpha_j\}_{j=0}^\infty$ of the
...
\begin{theorem}
Let $\DS S = \{\phi_j : 1 \leq j \leq J\}$ be a finite
orthonormal subset of $\DS L^2[a,b]$.  Given
$\DS f\in L^2[a,b]$, we have
\[
...
\begin{egg}
Let $\DS \phi_n(x) = e^{n x i}$ for $n \in \ZZ$, where $i$
is the complex number satisfying $\DS i^2=-1$.  Since
\begin{align*}
...
& \qquad =
\begin{cases}
\DS \frac{1}{(k-j)i} e^{(k-j)x i}\bigg|_{x=-\pi}^{\pi} =
\frac{1}{(k-j)i} \left( e^{(k-j)\pi i} - e^{-(k-j)\pi i}\right) = 0 &
\quad \text{if} \quad k \neq j \\[1em]
\DS x\bigg|_{x=-\pi}^{\pi} = 2\pi & \quad \text{if} \quad k = j
\end{cases}
\end{align*}
The set $\DS S = \{ e^{n x i} : n\in \ZZ\}$ is an orthogonal
set in the space $\DS L^2[-\pi,\pi]$ with the Lebesgue measure.  If we
replace $\phi_n$ by $\DS (2\pi)^{-1/2} \phi_n$, we get an
orthonormal set.  It can be shown that the set of all finite linear combinations
of elements of $S$ is dense in $\DS L^2[-\pi,\pi]$.  Hence, $S$ is a
complete orthogonal set in $\DS L^2[-\pi,\pi]$.

For $\DS f\in L^2[-\pi,\pi]$, we have that
...
$\DS L^2[a,b]$ is the space of measurable functions
$f:[a,b]\rightarrow \RR$ such
that $\DS \int_a^b f^2(x) w(x) \dx{x}$ is finite.  The scalar
product on $\DS L^2[a,b]$ is
...
\end{equation}
The space $\DS L^2[a,b]$ has a countable orthonormal basis.
Suppose that
\[
S = \{\phi_n : n \in \NN \} \subset L^2[a,b]
\]
is an orthonormal basis for $\DS L^2[a,b]$, the Fourier
...
N \rightarrow \infty \ .
\]
We say that $\DS \sum_{n=0}^{N} a_n \phi_n$ converges in
$\DS L^2$ to $f$ as $N\rightarrow \infty$.
...
\end{cases}
\]
Thus $\DS S = \{ \phi_n : n \in \NN \}$ is a set of
orthonormal functions in $\DS L^2[-\pi,\pi]$, where the
weight function is $w(x) = 1$ for all $x$.  It is possible to show
that the set of all linear combination of elements of $S$ is dense in
$\DS L^2[-\pi,\pi]$.

Hence, for $\DS f\in L^2[-\pi,\pi]$, we have that
...
sense of convergence in $\DS L^2[-\pi,\pi]$; namely,
(\ref{ClassFSconv}) is satisfied.  We may not have pointwise
convergence for all $x \in [a,b]$.
...
$\DS P_n(x) = \sum_{j=0}^n \alpha_{n,j}x^j$ be a
polynomial of degree exactly $n$; namely, $\alpha_{n,n} \neq 0$.
These polynomials can be considered as elements of $\DS L^2[a,b]$.

We now prove that for any finite subset $A$ of $\NN$, if
$\DS \sum_{n\in A} c_n P_n = 0$ with $c_n \in \RR$,
...
then $\DS c_1 P_{n_1} = \sum_{j=0}^{n_1} c_1 \alpha_{n_1,j}x^j = 0$
...
$\DS \sum_{i=1}^k c_i P_{n_i} = 0$ with $c_i \in \RR$
implies that $c_i = 0$ for all $i$.  Suppose that
$\DS \sum_{i=1}^{k+1} c_i P_{n_i} = 0$ with $c_i \in \RR$ for
a set $A = \{ n_1, n_2, \ldots , n_k,n_{k+1} \} \subset \NN$ of
cardinality $k+1$.  Let $\DS n_j = \max_{1\leq i \leq k+1}\{ n_i\}$.
The only term of degree $n_j$ in
$\DS \sum_{i=1}^{k+1} c_i P_{n_i} = 0$
is $\DS c_j \alpha_{n_j,n_j} x^{n_j}$.  Hence
$\DS c_j \alpha_{n_j,n_j}x^{n_j} = 0$
with $\alpha_{n_j,n_j} \neq 0$.  Thus $c_j=0$.  We
therefore get
$\DS \sum_{\substack{i=1\\i\neq j}}^{k+1} c_i P_{n_i} = 0$ with
$c_i \in \RR$.
...
$\DS x^{k+1}$ in $p(x)$.  Since $p$ and $P_{k+1}$ are of
degree $k+1$, we have that $c\neq 0$ and $\alpha_{k+1,k+1} \neq 0$.  Thus,
$\DS p - (c/\alpha_{k+1,k+1}) P_{k+1}$ is a polynomial of
...
density of continuous functions in $\DS L^2[a,b]$, we may
show that the set of all finite linear combinations of elements of the set
$P = \DS \{ P_n : n \in \NN \}$ is dense in
$\DS L^2[a,b]$.   Combined with the linear independence of
$P$, this shows that $P$ is a basis of $\DS L^2[a,b]$.
...
\]
If we consider only the terms in $\DS x^{n_j}$, we get the following
...
that $\DS \left\{ P_0, P_1, \ldots , P_k\right\}$ is a
...
a linear combination $\DS p=\sum_{k=0}^n c_k\,P_k$
...
\item If the coefficient of $\DS x^k$ in $P_k$ is
$\alpha_{k,k}$, then
\[
P_{k+1}(x) = A_k(x-B_k)P_k(x) - C_k P_{k-1}(x)
\]
for $k \geq 0$, where $P_{-1} = 0$,
$\DS A_k = \frac{\alpha_{k+1,k+1}}{\alpha_{k,k}}$ for $k \geq 0$,\\
$\DS B_k = \frac{\DS \int_a^b xP_k^2(x)w(x) \dx{x}}
{\DS \int_a^b P_k^2(x)w(x)\dx{x}}$ for $k\geq 0$,\\
$C_0 = 0$ and $\DS
C_k = \frac{\DS A_k\int_a^b P_k^2(x)w(x) \dx{x}}
{\DS A_{k-1} \int_a^b P_{k-1}^2(x)w(x) \dx{x}}$
for $k>0$.
\end{enumerate}
\label{orthpoly}
\end{theorem}
...
of $\DS x^n$ in $p$.  The constant $b$ is non-null because $p$ is of
degree exactly $n$.  Similarly, the coefficient $\alpha_{n,n}$ of
$\DS x^n$ in $P_n$ is non null because $P_n$ is of degree
exactly $n$.  Hence, $p - a_n P_n$ with $a_n = b/\alpha_{n,n}$ is a
polynomial of degree $n-1$ that can therefore be expressed as a linear
combination of the polynomials $P_i$ for $0\leq i < n$ by the
hypothesis of induction.  Namely,
\[
...
\]
because $\DS \int_a^b P_j(x) P_k(x) w(x)\dx{x} =0$ for
...
\stage{4}  We write $\DS P_{k+1}(x) = A_k x P_k(x) + q(x)$, where
$\DS A_k = \alpha_{k+1.k+1}/\alpha_{k,k}$ and $q(x)$ is
...
$[a,b]=[-1,1]$, the weight function is $\DS w(x)=\sqrt{1-x^2}$,
and the coefficient $\alpha_{k,k}$ of $\DS x^k$ in the
polynomial $P_k$ is $1$ for all $k$.

Due to our assumption on the coefficient of $\DS x^i$ in
$P_i$, we have that $P_0(x) = 1$ for all $x$.  For the sake of the
computations. we let $P_{-1}(x)=0$ for all $x$ and $C_0=0$.
We have
\[
...
$\DS P_2(x) = \frac{3}{2}\left(x^2- \frac{1}{3}\right)$,
$\DS P_3(x) = \frac{5}{2}\left(x^3 -\frac{3}{5} x\right)$,
and in general
\[
  P_{k+1}(x) = \frac{1}{k+1}\big( (2k+1)xP_k(x) -k P_{k-1}(x) \big)
\]
for $k=1$, $2$, $3$, \ldots\ These polynomials are said to be
...
If, in the fourth item of Theorem~\ref{orthpoly}, we take $a=-1$,
$b=1$, $\DS w(x)=(1-x^2)^{-1/2}$, $\alpha_{0,0} = 1$ and
$\DS \alpha_{k,k} = 2^{k-1}$ for
$k\geq 1$, we get $P_0(x) = 1$, $P_1(x) = x$, $\DS P_2(x) = 2x^2 -1$,
$\DS P_3(x) = 4x^3 -3x$, and in general
\[
...
$a=-1$, $b=1$ and $\DS w(x) = (1-x)^\alpha (1+x)^\beta$ with
$\alpha,\beta>-1$, we get the Jacobi polynomials
$\DS P_k^{[\alpha,\beta]}$ for $k=0$, $1$, $2$, \ldots\   For each
...
$b=\infty$ and $\DS w(t) = x^\alpha e^{-x}$ with $\alpha > -1$,
we get the Laguerre polynomials $\DS l_k^{[\alpha]}$. 
\item If, in the fourth item of Theorem~\ref{orthpoly}, we take
$a=-\infty$, $b=+\infty$ and $\DS w(x) = e^{-x^2}$, we get the 
...
We want to find the ``best approximation'' of
$\DS f\in L^2[a,b]$ by a finite
linear combination of the polynomials $P_n$.  More precisely, let
$\DS S = \{ P_n : 0 \leq n \leq k \}$.  We are looking for 
...
polynomials are $\DS P_n(x) = x^n$ for all $n\in \NN$.  Then
\[
...
$\DS S = \{ P_n :n \in \NN \}$ of polynomials, where
...
$\DS \{ P_k \}_{k=0}^\infty$ such that $P_k$ is of degree $k$ and
\[
...
of the term in $\DS x^n$ is $1$.
\end{question}

\sol{\SOL}{
Suppose that $\DS \{ P_k^{[i]} \}_{k=0}^\infty$ for $i=1$ and
$2$ are two orthogonal families of monic polynomials such that
$\DS \ps{p}{P_k^{[i]}}=0$ for all polynomial $p$ of degree
less than $k$. Given $k > 0$, we have that
$\DS P_k^{[1]} - P_k^{[2]}$ is a polynomial
...
\]
Since $\DS P_k^{[1]} - P_k^{[2]}$ is continuous on $[a,b]$,
we get that $\DS P_k^{[1]}(x) = P_k^{[2]}(x)$ for all $x \in [a,b]$.
}

\begin{question}
Let $\left\{ P_0,P_1,P_2,\ldots \right\}$ be an orthonornal set of
polynomials on $[a,b]$ with respect to a weight function $w$.
Suppose that $\DS P_k(x) = \sum_{j=0}^k a_{k,j}x^j$ with
$a_{k,k} \neq 0$ for all $k$.
Prove that
...
$\DS A_k = \frac{a_{k+1,k+1}}{a_{k,k}}$,
$\DS B_k = \frac{a_{k,k-1}}{a_{k,k}}- \frac{a_{k+1,k}}{a_{k+1,k+1}} $
and
$\DS C_k = \frac{a_{k+1,k+1}a_{k-1,k-1}}{a_{k,k}^2}$
...
theorem that $\DS A_k = \frac{a_{k+1,k+1}}{a_{k,k}}$ for
$k\geq 0$ and $\DS C_k = \frac{A_k}{A_{k-1}}
= \frac{a_{k+1,k+1}a_{k-1,k-1}}{a_{k,k}^2}$ for $k >0$.  Because of
our choice for $a_{-1,-1}$, we also have $C_0 = 0$ as required.
Note that $\DS A_k = \frac{a_{k+1,k+1}}{a_{k,k}}$ can
be easily proved by comparing the coefficient of
$\DS x^{k+1}$ on both sides of (\ref{apprAQ2equ1}).
