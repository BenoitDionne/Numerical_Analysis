...
$A$ is an invertible \nn matrix and $\DS \VEC{b} \in \RR^n$ is given.
...
\end{pmatrix}  \ .
\]
Let $\DS M(1) = \begin{bmatrix} A & \VEC{b} \end{bmatrix}$.
...
exists $i>k$ such that $\DS a^{[k]}_{i,k}\not=0$ because $A$
is invertible.  We interchange the $\DS k^{th}$ and
$\DS i^{th}$ rows.

To get $M(k+1)$ from $M(k)$, we subtract
$\DS a^{[k]}_{i,k}/a^{[k]}_{k,k}$ times the
$\DS k^{th}$ row from the $\DS i^{th}$ row
and write the result back in the $\DS i^{th}$ row for each
$i > k$.  Namely,
...
We have $\DS a^{[k+1]}_{i,k} = 0$ for $i>k$.  Repeating
...
\begin{rmk}
If $\DS |a^{[k]}_{i,k}| \gg |a^{[k]}_{k,k}|$, then
$\DS a^{[k]}_{i,k} / a^{[k]}_{k,k}$ is very large.  Hence,
...
\]
and interchange the $\DS i^{th}$ and $\DS k^{th}$ rows.
...
such that
\[
\frac{|a^{[k]}_{i,k}|}{\DS \max_{k\leq j \leq n}|a^{[k]}_{i,j}|} \geq
\frac{|a^{[k]}_{s,k}|}{\DS \max_{k\leq j \leq n}|a^{[k]}_{s,j}|}
\]
for $k\leq s \leq n$, and interchange the $\DS i^{th}$
and $\DS k^{th}$ rows.
...
|a^{[k]}_{s,r}|
\]
and interchange the $\DS i^{th}$ and $\DS k^{th}$
rows and the $\DS j^{th}$ and
$\DS k^{th}$ columns.  With this strategy, the indices of
...
because
\[
\frac{|a^{[1]}_{2,1}|}{\DS \max_{1\leq j \leq 2}|a^{[1]}_{2,j}|}
= \frac{1727}{12180} > \frac{1}{5220}
= \frac{|a^{[1]}_{1,1}|}{\DS \max_{1\leq j \leq 2}|a^{[1]}_{1,j}|} \ .
\]
Hence
\[
M(1) = \begin{pmatrix}
...
$\DS L\VEC{y} = \VEC{c} = P^{-1}\VEC{b}$.  Note that $\VEC{c}$ is
...
are respectively given by $\DS u_{i,j}= a^{[i]}_{i,j}$ and
$\DS \ell_{i,j} = a^{[j]}_{i,j}/a^{[j]}_{j,j}$.  Recall that
$\DS a^{[i]}_{i,j} = 0$ and $\ell_{i,j} = 0$ if $j<i$.
...
Gaussian elimination.  Let $\DS P^{-1}$ be the permutation matrix that
performs all the needed row interchanges.  Note that $\DS P=P^{-1}$.
From our previous discussion, we can write
$\DS P^{-1}A$ as $\DS P^{-1}A=LU$
for a lower-triangular matrix $L$ and an upper-triangular matrix $U$.
No row interchange is needed to reduce $\DS P^{-1}A$ to an
upper-triangular matrix using Gaussian elimination.  Hence $A=PLU$.

To solve $A\VEC{x} = \VEC{b}$, we only have to solve
$\DS LU \VEC{x} = P^{-1}A\VEC{x}=P^{-1}\VEC{b}$.
...
$\DS \VEC{p} - \VEC{x}_a = A^{-1}\VEC{r}$.  Thus
...
Since
$\DS A^{-1} = \begin{pmatrix}
10^4 & -10^4 \\
-4999.8\overline{3} & 5000
\end{pmatrix}$, we have
$\DS \| A^{-1}\|_\infty = 2\times 10^4$.
Thus $\DS K(A) = 1.8\times 10^5$ is really large.  $A$ is
ill-conditioned.
...
\begin{proof}
From Proposition~\ref{Banach}, $\DS \Id_n + A^{-1}\dtx{A}$
...
= (\VEC{b} + \dtx{\VEC{b}})
\]
from the left by $\DS A^{-1}$, we get
\[
...
+ \frac{\|\dtx{A}\|}{\|A\|} \right) \ ,
\end{align*}
where we have used $\DS K(A) = \|A||\, \|A^{-1}\|$ and
...
\]
Thus
$\DS 10^t \frac{\|\VEC{y}\|}{\|\VEC{x}_a\|}$ may be used as
...
\item If $\DS 10^t \|\VEC{x_c}\|/\|\VEC{x}_b\| < \epsilon$, the
...
$\DS A^{-1}$ is lower-triangular (resp.\ upper-triangular) if
...
upper-triangular, prove that $\DS U = DL^\top$, where $D$ is
...
ill-conditioned?\\
Hint: the $\DS i^{th}$ column of $\DS A^{-1}$
...
\sol{\SOL}{
From $\DS \Id = A\,A^{-1}$, we get
$\DS 1 = \|\Id\| = \|A\,A^{-1}\| \leq \|A\|\,\|A^{-1}\| = K(A)$.
}
...
where
$\DS \VEC{b}_p = \begin{pmatrix} 3.00001 & 3.00003
\end{pmatrix}^\top$.
Compare $\VEC{x}_p$ with the solution
$\DS \VEC{x}_s = \begin{pmatrix} 1 & 1 \end{pmatrix}^\top$
of the unperturbed system.  Compute the condition number using the
$\DS \ell^\infty$-norm.  Is the system ill-conditioned or
well-conditioned?
\end{question}
...
$\DS \VEC{x}_p = \begin{pmatrix} 2.00001 & 0.5 \end{pmatrix}^\top$.
...
$\DS K(A) = \|A\|_\infty\,\|A^{-1}\|_\infty = 3.00001 \times
...
$\DS K(A) = \|A\|_\infty \, \|A^{-1}\|_\infty$.  Is $A$ well
conditioned?
\end{question}
...
$\DS \sum_{i=1}^k r^i = \frac{1-r^{k+1}}{1-r}$ with $r=2$
and $k=n-2$. 

Hence, $\DS K(A) = n 2^{n-1}$.  For large matrices $A$
...
Without computing $\DS A^{-1}$, can you determine if the system is
ill-conditioned or well-conditioned?
\end{question}

\sol{\SOL}{
We use the relation
$\DS \| \VEC{q} - \VEC{p} \|_1 \leq K(A) \,
...
Since
$\DS \|A\|_1 = \sum_{i=1}^3 |a_{i,3}| = 4.21$, 
$\DS \| \VEC{q} - \VEC{p} \|_1
...
we get\\
$\DS K(A) \geq 0.12 \times 4.21 / 0.3527 \approx 1.43237879$.
