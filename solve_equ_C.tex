...
$A$ is an invertible \nn matrix and $\displaystyle \VEC{b} \in \RR^n$ is given.
...
\end{pmatrix}  \ .
\]
Let $\displaystyle M(1) = \begin{bmatrix} A & \VEC{b} \end{bmatrix}$.
Suppose that, after several row operations, we have the matrix
...
\]
We may assume that
$\displaystyle a^{[k]}_{k,k}\not=0$.  If $a^{[k]}_{k,k} = 0$, there
exists $i>k$ such that $\displaystyle a^{[k]}_{i,k}\not=0$ because $A$
is invertible.  We interchange the $\displaystyle k^{th}$ and
$\displaystyle i^{th}$ rows.

To get $M(k+1)$ from $M(k)$, we subtract
$\displaystyle a^{[k]}_{i,k}/a^{[k]}_{k,k}$ times the
$\displaystyle k^{th}$ row from the $\displaystyle i^{th}$ row
and write the result back in the $\displaystyle i^{th}$ row for each
$i > k$.  Namely,
...
We have $\displaystyle a^{[k+1]}_{i,k} = 0$ for $i>k$.  Repeating
these operations from $k=1$ to $k=n-1$, we get
\[
M(n) = \begin{pmatrix}
...
\begin{rmk}
If $\displaystyle |a^{[k]}_{i,k}| \gg |a^{[k]}_{k,k}|$, then
$\displaystyle a^{[k]}_{i,k} / a^{[k]}_{k,k}$ is very large.  Hence,
when performing (\ref{iminusk}), we may magnify the rounding error.
Moreover, when performing the backward substitution (\ref{backsubst}),
we may also magnify the rounding error if we divide by a small number
$\displaystyle a^{[k]}_{k,k}$.
...
\]
and interchange the $\displaystyle i^{th}$ and $\displaystyle k^{th}$ rows.
...
\]
for $k\leq s \leq n$, and interchange the $\displaystyle i^{th}$
and $\displaystyle k^{th}$ rows.
...
|a^{[k]}_{s,r}|
\]
and interchange the $\displaystyle i^{th}$ and $\displaystyle k^{th}$
rows and the $\displaystyle j^{th}$ and
$\displaystyle k^{th}$ columns.  With this strategy, the indices of
$\VEC{x}$ also have to be permuted.
\end{rmk}
...
$\displaystyle L\VEC{y} = \VEC{c} = P^{-1}\VEC{b}$.  Note that $\VEC{c}$ is
...
are respectively given by $\displaystyle u_{i,j}= a^{[i]}_{i,j}$ and
$\displaystyle \ell_{i,j} = a^{[j]}_{i,j}/a^{[j]}_{j,j}$.  Recall that
$\displaystyle a^{[i]}_{i,j} = 0$ and $\ell_{i,j} = 0$ if $j<i$.
...
Gaussian elimination.  Let $\displaystyle P^{-1}$ be the permutation matrix that
performs all the needed row interchanges.  Note that $\displaystyle P=P^{-1}$.
From our previous discussion, we can write
$\displaystyle P^{-1}A$ as $\displaystyle P^{-1}A=LU$
for a lower-triangular matrix $L$ and an upper-triangular matrix $U$.
No row interchange is needed to reduce $\displaystyle P^{-1}A$ to an
upper-triangular matrix using Gaussian elimination.  Hence $A=PLU$.

To solve $A\VEC{x} = \VEC{b}$, we only have to solve
$\displaystyle LU \VEC{x} = P^{-1}A\VEC{x}=P^{-1}\VEC{b}$.
...
\end{pmatrix}  \ ,
\]
$M=LD$ and $\displaystyle N=D^{-1}U$.  Recall that the elements on the
diagonal of a strictly positive definite matrix are all positive
numbers.  Then $A=MN$, where $M$ is lower-triangular and
$\displaystyle N = M^\top$.

To prove that $\displaystyle M=N^\top$, we use the relation $A=MN$ to get
\begin{align}
...
$\displaystyle \VEC{p} - \VEC{x}_a = A^{-1}\VEC{r}$.  Thus
...
\end{pmatrix}$, we have
$\displaystyle \| A^{-1}\|_\infty = 2\times 10^4$.
Thus $\displaystyle K(A) = 1.8\times 10^5$ is really large.  $A$ is
ill-conditioned.
...
\end{equation}
where $\dtx{A}$ is an \nn matrix near the \nn null
matrix and $\dtx{\VEC{b}}$ is a vector of $\displaystyle \RR^n$
near $\displaystyle \VEC{0}\in \RR^n$.
The next theorem gives an estimate of the difference between the exact
solution of (\ref{unpertub}) and the exact solution of
(\ref{pertub}).

\begin{theorem}
If $\displaystyle \|\dtx{A}\| < \|A^{-1}\|^{-1}$, we have that
...
\begin{proof}
From Proposition~\ref{Banach}, $\displaystyle \Id_n + A^{-1}\dtx{A}$
...
= (\VEC{b} + \dtx{\VEC{b}})
\]
from the left by $\displaystyle A^{-1}$, we get
\[
...
+ \frac{\|\dtx{A}\|}{\|A\|} \right) \ ,
\end{align*}
where we have used $\displaystyle K(A) = \|A||\, \|A^{-1}\|$ and
...
\item If $\displaystyle 10^t \|\VEC{x_c}\|/\|\VEC{x}_b\| < \epsilon$, the
...
If $\displaystyle 10^t \|\VEC{x_c}\|/\|\VEC{x}_b\| \not< \epsilon$.
...
$\displaystyle A^{-1}$ is lower-triangular (resp.\ upper-triangular) if
...
upper-triangular, prove that $\displaystyle U = DL^\top$, where $D$ is
a diagonal matrix whose diagonal is the diagonal of the matrix $U$.\\
...
ill-conditioned?\\
Hint: the $\displaystyle i^{th}$ column of $\displaystyle A^{-1}$
is the solution of $A\VEC{x} = \VEC{e}_i$.
\end{question}
...
\sol{\SOL}{
From $\displaystyle \Id = A\,A^{-1}$, we get
...
where
$\displaystyle \VEC{b}_p = \begin{pmatrix} 3.00001 & 3.00003
\end{pmatrix}^\top$.
Compare $\VEC{x}_p$ with the solution
$\displaystyle \VEC{x}_s = \begin{pmatrix} 1 & 1 \end{pmatrix}^\top$
of the unperturbed system.  Compute the condition number using the
$\displaystyle \ell^\infty$-norm.  Is the system ill-conditioned or
well-conditioned?
\end{question}
...
$\displaystyle \VEC{x}_p = \begin{pmatrix} 2.00001 & 0.5 \end{pmatrix}^\top$.
...
$\displaystyle K(A) = \|A\|_\infty\,\|A^{-1}\|_\infty = 3.00001 \times
2 \times 10^5 = 6.00002\times 10^5$.  The matrix $A$ is ill-conditioned.
}
...
$\displaystyle K(A) = \|A\|_\infty \, \|A^{-1}\|_\infty$.  Is $A$ well
conditioned?
\end{question}
...
the $\displaystyle k^{th}$ row of $B$ is the sum of the
$\displaystyle k^{th}$ row of $\Id_n$ with
...
Hence, $\displaystyle K(A) = n 2^{n-1}$.  For large matrices $A$
(i.e.\ $n$ large), the matrix is not well conditioned.
}
...
Without computing $\displaystyle A^{-1}$, can you determine if the system is
...
we get\\
$\displaystyle K(A) \geq 0.12 \times 4.21 / 0.3527 \approx 1.43237879$.
