...
differentiable functions.  Let $\displaystyle u:\RR^2 \to \RR$ be a sufficiently
...
If we assume that $u$ is of class $\displaystyle C^2$, we have
\[
...
$u(0,t) = t(4-t)$ and $\displaystyle u(2,t) = t(4-t)^2$ for $0 \leq t \leq 4$.
...
$u(x,0) = x(2-x)$ and $\displaystyle u(x,4) = x(2-x)^2$ for
$0 \leq x \leq 2$, and $u(0,y) = y(4-y)$ and
$\displaystyle u(2,y) = y(4-y)^2$ for $0 \leq y \leq 4$.
...
of $R$ is defined as the set $\displaystyle R^\circ = R \setminus \partial R$. 
...
$\displaystyle R^\circ_\Delta = R_\Delta \setminus \partial R$.
...
= F(x_i,y_j)
\end{equation}
for all $(i,j)$ such that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$, where
...
referring to $\displaystyle (x_i,y_j) \in R^\circ_\Delta$.  This is
not really correct.
...
$\displaystyle R^\circ_\Delta$.   The point $(x_i,y_j)$ may be on the boundary as
...
\end{align*}
for all $(i,j)$ such that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$.
...
$\tau_{i,j}$ for $\displaystyle (x_i,y_j) \in R^\circ_\Delta$ though
$(x_i,y_j)$ may not be in $\displaystyle R^\circ_\Delta$.  The formula
(\ref{fdm_fde_tr}) is used to approximate the value of
$u(x_i,y_{j+1})$.  It is $(x_i,y_{j+1})$ which is really in
$\displaystyle R^\circ_\Delta$.   The point $(x_i,y_j)$ may be on the boundary.
...
\]
for all $(i,j)$ such that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$, and
\[
...
\begin{proof}
For every $\displaystyle (x_i,y_j) \in R^\circ_\Delta$, we have
\begin{align*}
...
\subsection{$\displaystyle \ell^2$ Theory} \label{ell2Theory}
...
We consider the space $\displaystyle \ell^2(\ZZ)$ of all functions
$g:\ZZ \to \CC$ such that
\[
...
$\displaystyle Q_\alpha:\ell^2(\ZZ) \to \ell^2(\ZZ)$ is
...
$\displaystyle Q_\alpha = -J^{-1} K$, where
\[
J_{r,s} =
\begin{cases}
...
$\displaystyle \alpha = c \dtx{t} / (2 (\dtx{x})^2)$.
...
$\displaystyle g_j \in \ell^2(\ZZ)$ for all $j \geq 0$ is
{\bfseries $\displaystyle \ell^2$-convergent}
...
with $\displaystyle g_j \in \ell^2(\ZZ)$ for all $j \geq 0$ is
{\bfseries $\displaystyle \ell^2$-stable}\index{Finite Difference
Methods!$\ell^2$-Stable} if there exists a constant $C_\alpha$ such
that $\displaystyle \|Q_\alpha^j\|_2 \leq C$ for $0 \leq j \leq M$ and all $M$.

The index $\alpha$ for $C_\alpha$ is to indicate that there may be a
constraining relation on $\dtx{x}$ and $\dtx{y}$ that must be
satisfied for $\displaystyle \|Q_\alpha^j\|_2 \leq C$ to be satisfied for
...
value does not increase in $\displaystyle \ell^2$ norm.  This can be
...
\end{equation}
where $\displaystyle \delta_j \in \ell^2(\ZZ)$ represents the round off error.
...
\end{align*}
for $0 < j \leq M$.  The error is bounded in $\displaystyle \ell^2$ norm.  In
...
+ \dtx{t}\, \delta_{i,j} \ .
\]
The function $\displaystyle \delta_j \in \ell^2(\ZZ)$ is defined by
$\delta_j(i) = \delta_{i,j}$ for $i \in \ZZ$.
We can reach a similar conclusion with other finite difference schemes.
\end{rmk}

As the reader may know, $\displaystyle \ell^2(\ZZ)$ is a
...
linear mapping on $\displaystyle \RR^n$.   Let $E$ be a Hilbert space
...
of $P$ is the bounded linear mapping $\displaystyle P^\ast$ such that
$\displaystyle \ps{Px}{y} = \ps{x}{P^\ast y}$ for all $x,y \in E$.
The operator $\displaystyle P^\ast$ is the equivalent of the transpose of a \nn
matrix.  A bounded linear operator $P$ is
{\bfseries normal}\index{Linear Mapping!!Normal Operator}
if $\displaystyle P P^\ast = P^\ast P$
\footnote{Symmetric operator (i.e. $\displaystyle P = P^\ast$) are
...
with $\displaystyle g_j \in \ell^2(\ZZ)$ for all $j \geq 0$.  If there exists a
constant $C_\alpha$ such that
$\displaystyle \left\|Q_\alpha \right\|_2 \leq 1 + C_\alpha \dtx{t}$ for all
$\dtx{t}$. then the finite difference scheme is $\displaystyle \ell^2$-stable.
\end{prop}
...
We have used the relation $\displaystyle e^x \geq 1 + x$ for all $x \in \RR$.
\end{proof}

\begin{prop}[von Neumann] \label{vonNNo1}
If a finite difference scheme of the form $g_{j+1} = Q_\alpha(g_j)$
with $\displaystyle g_j \in \ell^2(\ZZ)$ for all
$\displaystyle j \geq 0$ is $\displaystyle \ell^2$-stable, then
there exists a constant $C_\alpha$ such that
$\rho(Q_\alpha) \leq 1 + C_\alpha/M$ for all $M$.
\end{prop}

\begin{proof}
Since the finite difference scheme is $\displaystyle \ell^2$-stable, we have
$\displaystyle \|Q_\alpha^j\|_2 \leq C_\alpha$ for $0\leq j \leq M$.
Hence,
\[
...
$\displaystyle \ell^2$ stability.

\begin{prop}[Lax]
Consider a finite difference scheme of the form $g_{j+1} = Q_\alpha(g_j)$
with $\displaystyle g_j \in \ell^2(\ZZ)$ for all $j \geq 0$.
If $Q_\alpha$ is a normal operator, then the finite difference scheme is
$\displaystyle \ell^2$-stable if and only if there exists a constant
...
$\displaystyle \ell^2$ stability of
the finite difference scheme.  It follows from
Proposition~\ref{vonNNo1} that
$\rho\left(Q_\alpha\right) \leq 1 + C_\alpha/M$ for a constant $C_\alpha$
and all $M$ is a necessary condition for the $\displaystyle \ell^2$
...
Consider a finite difference scheme of the form $g_{j+1} = Q_\alpha(g_j)$
where $\displaystyle g_j \in \ell^2$.   If this finite difference scheme is
$\displaystyle \ell^2$-stable and consistent, then it is
$\displaystyle \ell^2$-convergent. 
\end{theorem}

A proof of this result is given in \cite{WH} (Theorem~6.22).  They
also prove that $\displaystyle \ell^2$-convergence implies
$\displaystyle \ell^2$-stability.  They
provide many more criteria to determine if a finite difference scheme is
$\displaystyle \ell^2$-stable.
...
We consider the space $\displaystyle L^2([0,2\pi])$ of all integrable
...
$\displaystyle \ell^2(\ZZ)$ and the norm
in $\displaystyle L^2([0,2\pi])$.  The reader should be able by the context to
...
\]
for $k\in \ZZ$ and $i \in \CC$ is such that $\displaystyle i^2 =-1$.
...
\]
If we set
$\displaystyle \check{Q}_\alpha = \Phi^{-1}\circ Q_\alpha \circ \Phi$, we get
$\check{g}_{j+1} = \check{Q}_\alpha\left(\check{g}_j\right)$ with
$\displaystyle \check{Q}_\alpha: L^2([0,2\pi]) \to L^2([0,2\pi])$ a
linear mapping.

Since $\displaystyle \Phi : L^2([0,2\pi]) \to \ell^2(\ZZ)$ is an isometry with
inverse $\displaystyle \Phi^{-1}$, we have that they are both of
...
the form $g_{j+1} = Q_\alpha(g_j)$ for $j\geq 0$ is $\displaystyle \ell^2$-stable
if there exists a constant $C$ such that
$\displaystyle \|\check{Q}_\alpha^j\|_2 \leq C$
for all $j\geq 0$.

To determine if a finite difference scheme of the form
$g_{j+1} = Q_\alpha(g_j)$ with $\displaystyle g_j \in \ell^2(\ZZ)$
is $\displaystyle \ell^2$-stable,
we have to prove that there exists a constant $C_\alpha$ such that
$\displaystyle \|Q_\alpha^j \|_2\leq C_\alpha$ or
$\displaystyle \|\check{Q}_\alpha^j \|_2\leq C_\alpha$ for all $j$.
...
$\displaystyle f \in L^2([0,2\pi])$ is just the product $\tilde{Q}_\alpha\,f$.
...
\]
where the last norm is just the $\displaystyle L^2$-norm of the function
$\tilde{Q}_\alpha$.

To use Proposition~\ref{LaxN1} to show that a finite difference scheme
is $\displaystyle \ell^2$-stable, we may simply show that
\[
...
Algorithm~\ref{fdm_sch1S} is $\displaystyle \ell^2$-stable for
$\displaystyle \frac{c^2\dtx{t}}{(\dtx{x})^2} \leq \frac{1}{2}$.
\end{egg}

It could be trickier to use the theory developed above to prove that
an implicit finite difference scheme is $\displaystyle \ell^2$-stable
because we may
...
given by $\displaystyle Q_\alpha = -J^{-1} K$, where $J$ and $K$ are defined in
...
$\displaystyle A_\alpha(g_{j+1}) = B_\alpha(g_j)$ for
$\displaystyle g_j \in \ell^2(\ZZ)$,
where $\displaystyle A_\alpha , B_\alpha:\ell^2(\ZZ) \to \ell^2(\ZZ)$
...
\begin{rmk}
The $\displaystyle \ell^2$ theory above can be expanded to finite
...
Crank-Nicolson scheme is unconstrained $\displaystyle \ell^2$-stable.
\label{CNell2StableNo2}
\end{egg}

\subsection{$\displaystyle L^2$ Stability}\label{L2Stab}
...
$\displaystyle L^2$ notation in this section is not widely used but we
...
by functions $\displaystyle h_j \in L^2([0,2\pi])$ for $0 \leq j \leq M$.
...
{\bfseries $\displaystyle L^2$-stable}\index{Finite Difference
...
is the complex number such that $\displaystyle i^2=-1$.  We have that
...
$\displaystyle |\lambda_k|^j \leq C_\alpha$ for
$0 \leq j \leq M$ and $0 \leq k <N$,
then $\displaystyle |A_{k,j}|^2 \leq C_\alpha^2 |A_{k,0}|^2$ for
...
$\displaystyle |\lambda_k|^j \leq C$ for $0 \leq j \leq M$ and $0 \leq k < N$.
...
$N$ and $M$ because $\displaystyle |\lambda_k|^j \leq C$ may be true only if
...
$\displaystyle \alpha_k \lambda^2 - \beta_k \lambda - \gamma_k$ has two distinct
...
$\displaystyle h_j \in L^2([0,2\pi],\RR^n)$ \footnote{The functions
$\displaystyle h:[0,2\pi] \to \RR^n$ such that each component is in
...
where $\displaystyle A_{k,j} \in \RR^n$, in the previous equation, we find that
...
$\displaystyle \|Q_k^j\|_2 \leq C_\alpha$ for
$0\leq j \leq M$ and $k \in \ZZ$,  we get from Parseval equality that
\[
...
{\bfseries $\displaystyle L^2$-stable} if and only if there exists
$C_\alpha$ such that $\displaystyle \|Q_k^j\|_2 \leq C_\alpha$ for
$0\leq j \leq M$, $k \in \ZZ$ and $N>0$.
\label{QkjCstable}
\end{prop}

\begin{proof}
We have proved above that the condition $\displaystyle \|Q_k^j\|_2 \leq C$ for
$0\leq j \leq M$ and $k \in \ZZ$ was sufficient.   We now prove that
it is necessary.  Suppose that $\displaystyle \|Q_{k_0}^{j_0}\|_2 > C_\alpha$ for
some $k_0$ and $j_0$.  Choose $\displaystyle \VEC{w} \in \RR^n$ such that
$\displaystyle \|Q_{k_0}^{j_0}\VEC{w}\|_2 > C_\alpha \|\VEC{w}\|_2$.
If we consider a boundary value problem
such that $\displaystyle g(x) = \VEC{w} e^{k_0 x\, i}$, then
$\displaystyle h_0(x) = \VEC{A}_{k_0,0} e^{k_0 x\, i}$ with
$\VEC{A}_{k_0,0} = \VEC{w}$.  From (\ref{fdmgfL2G}), we have
$\displaystyle \VEC{A}_{k_0,j} = Q_{k_0}^j\VEC{w}$.   Thus
\[
...
{\bfseries $\displaystyle L^2$-stable}, then there exists a constant
$C_\alpha$ such that $\rho(Q_k) \leq 1 + C_\alpha/M$ for all
$k \in \ZZ$, $N$ and $M$.
\end{prop}

\begin{proof}
Since the finite difference scheme is $\displaystyle L^2$-stable, we get from
Proposition~\ref{QkjCstable} that
$\displaystyle \|Q_k^j\|_2 \leq C_\alpha$ for $0\leq j \leq M$ and $k \in \ZZ$.
...
condition to determine $\displaystyle L^2$ stability.

\begin{prop}
If the matrix $Q_k$ is normal, then the finite difference scheme of
the form (\ref{fdmgfL2D}) is $\displaystyle L^2$-stable if $\rho(Q_k) \leq 1$
for all $k \in \ZZ$, $N$ and $M$.
\end{prop}

\begin{proof}
Since the matrix $Q_k$ is normal, we have $\rho(Q_k) = \|Q_k\|_2$.  Hence,
$\displaystyle \|Q_k^j\|_2 \leq \|Q_k\|_2^j = \rho^j(Q_k) \leq 1$ for
$0\leq j \leq M$, $k \in \ZZ$ and $N>0$.
\end{proof}
...
Suppose that
$P_\Delta(w_{i,j},w_{i,j+1},w_{i+1,j}, \ldots) = F(x_i,t_j)$ for all
$(i,j)$ such that $\displaystyle (x_i,t_j) \in R^\circ_\Delta$ can be expressed
...
$\displaystyle (x_i,t_j) \in R^\circ_\Delta$.
...
$\displaystyle Q_\alpha = -J^{-1}K$ for $K$ given in (\ref{fdm_K}), $J$ given in
(\ref{fdm_J}) and
\[
...
We consider the norm on $\displaystyle \RR^{N-1}$ defined by
\[
  \| \VEC{y} \|_N = \left(\sum_{i=1}^{N-1} y_i^2\,\dtx{x}\right)^{1/2}
\]
for $\displaystyle \VEC{y} \in \RR^{N-1}$.  If $y_i = g(x_i)$ for all $i$, where
...
with $\displaystyle \VEC{w}_j \in \RR^{N-1}$ is
{\bfseries $\displaystyle \ell^2$-convergent}
...
with $\displaystyle \VEC{w}_j \in \RR^{N-1}$ is
{\bfseries $\displaystyle \ell^2$-consistent}
...
$\displaystyle i^{th}$ component of $\VEC{\tau}_j(\dtx{x},\dtx{t},q)$ is
...
$\displaystyle \ell^2$-norm instead of in the uniform norm as $M$ increases.
...
\ $\displaystyle Q_\alpha Q_\alpha^\top = Q_\alpha^\top Q_\alpha$)
and the induced norm on the \nm{(N-1)}{(N-1)} matrices is
the Euclidean norm $\|\cdot\|_2$ on $\displaystyle \RR^{N-1}$, then
...
that $\displaystyle \|Q_\alpha^j\|_N \leq \|Q_\alpha\|_N^j \leq 1$.
...
A finite difference scheme of the form
$\VEC{w}_{j+1} = Q_\alpha\VEC{w}_j + \VEC{B}_j$,
where $\displaystyle \VEC{w}_j \in \RR^{N-1}$ and $Q_\alpha$ is normal, is
$\displaystyle \ell^2$-stable according to Definition~\ref{fdmStableDefNo4}
...
features other than uniform convergence.  So $\displaystyle \ell^2$
...
$\displaystyle \ell^2$ stability may seem to be the next reasonable
...
$v_{n+1} = 0$ implies $\displaystyle \beta (n+1) \rho_1^{n+1} = 0$.
...
$\displaystyle \RR^n$ are $n$ linearly independent eigenvectors for
each matrix $Q_{i,j}$.
...
for $(i,j)$ such that $\displaystyle (x_i,t_j) \in R_\Delta^\circ$
...
Algorithm~\ref{fdm_sch1S} is $\displaystyle \ell^2$-stable if
...
$\displaystyle \dtx{x} < 10^{-2}$, then
$\displaystyle \dtx{t} <  10^{-4}/(2c^2)$.  Thus, a lot of computations
...
If $\displaystyle N = 2^k$ and $\displaystyle M= 2^{2k}S$ for $k>0$
arbitrary, then $\displaystyle \dtx{x} = L/2^k$ and
$\displaystyle \dtx{t} = T/(S2^{2k})$.  We have that
\[
...
We study the $\displaystyle \ell^2$ convergence according to
...
$\displaystyle \ell^2$ consistency according to
...
Crank-Nicolson scheme is $\displaystyle \ell^2$-stable according to
Definition~\ref{fdmStableDefNo2}.  Hence, it follows from
Theorem~\ref{ell2StabConstConv}, that the Crank-Nicolson scheme is
$\displaystyle \ell^2$-convergent according to Definition~\ref{ell2ConvDefN1}.
...
scheme is $\displaystyle \ell^2$-stable according to
Definition~\ref{fdmStableDefNo4}.

\begin{prop}
The Crank-Nicolson scheme in Algorithm~\ref{fdm_sch11S} is
$\displaystyle \ell^2$-stable without constraints on $\dtx{t}$ and $\dtx{x}$.
\end{prop}
...
$\displaystyle Q_\beta = -J^{-1}K$ for $K$ given in (\ref{fdm_K}) and
...
$\displaystyle 1 - 4 \alpha \sin^2\left(k\,\pi/(2N)\right)$.
Thus the eigenvalues of $\displaystyle J^{-1} K$ are
\[
...
Theorem~\ref{StabConstConv} for $\displaystyle \ell^2$-convergence
...
for $\displaystyle J^{-1}$ in the proof of Proposition~\ref{CKell2NConv}}, we
...
is $\displaystyle \ell^2$-convergent according to Definition~\ref{ell2ConvDefN2}
...
$\displaystyle Q = -J^{-1}K$ for $K$ given
...
\end{align*}
and $\displaystyle R^\circ_\Delta = R_\Delta \setminus \partial R_\Delta$.
...
$\displaystyle (x_i,y_j)\in R^\circ_\Delta$, then
\[
...
$\displaystyle (x_{i_0},y_{j_0}) \in R^\circ_\Delta$, (i.e.\ $0 < i_0 < M$
and $0 < j_0 < N$) such that $v_{i_0,j_0} \geq v_{i,j}$ for all
$(i,j)$ with $\displaystyle (x_i,y_j) \in R^\circ_\Delta$, and
...
Suppose that
$\displaystyle \{ w_{i,j}^{[k]} : 0\leq i \leq N \ \text{and} \ 0 \leq j
\leq M\}$
...
with $\displaystyle w_{i,j} = w_{i,j}^{[1]} - w_{i,j}^{[2]}$ for all
$i$ and $j$ is
a solution of $\displaystyle \Delta_\Delta w_{i,j} = 0$ for all $(i,j)$
such that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$, and $w_{i,j} = 0$
for all 
...
\]
Thus $\displaystyle w_{i,j}^{[1]} - w_{i,j}^{[2]} = 0$ for $0\leq i \leq N$ and
$0\leq j \leq M$.
...
\end{align*}
We obviously have that $\displaystyle 0 \leq h(x_i,y_j) \leq a^2/2$ for all
...
\end{align*}
Let $\displaystyle g_{i,j}^{\pm} = g^{\pm}(x_i,y_j) = \pm v_{i,j} + K h_{i,j}$ for
...
$\displaystyle g^{\pm}$.  For $\displaystyle g^+$, we get
\begin{align*}
...
\end{align*}
for all $(i,j)$ such that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$.
For $\displaystyle g^-$, we also get
\begin{align*}
-v_{i,j} & \leq \max_{(x_i,y_j) \in R^\circ_\Delta} -v_{i,j}
\leq \max_{(x_i,y_j) \in R^\circ_\Delta} g^-_{i,j}
\leq \max_{(x_i,y_j) \in \partial R_\Delta} g^-_{i,j}
...
\]
for all $(i,j)$ such that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$.  This
...
that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$,  and 
...
with $\displaystyle C = \max \{ 1, a^2/2\}$.
\end{proof}
...
$(i,j)$ such that $\displaystyle (x_i,y_j) \in R^\circ_\Delta$, and
$w_{i,j} = g(x_i,y_j)$ for all $(i,j)$ such that
$\displaystyle (x_i,y_j) \in R^\circ_\Delta$.
...
knowledge of $\displaystyle L^2$-spaces and Fourier series.
...
roots of the characteristic equation $\displaystyle \lambda^2-k=0$.
...
where $\displaystyle \VEC{v}: \RR^2 \rightarrow \RR^n$ and $A$ is a
...
$\displaystyle \ell^2$-stable when
\[
...
prove $\displaystyle \ell^2$ stability for Algorithm~\ref{fdm_sch3S}.
...
for $0 < k < N$.  
We have that
$\displaystyle p_k(\lambda) = \lambda^2 -2 \delta_k \lambda + 1 = 0$
for
\[
...
Since we have $\displaystyle \ell^2$ consistency and
$\displaystyle \ell^2$ stability, we may 
conclude that the finite difference scheme in
Algorithm~\ref{fdm_sch3S} is $\displaystyle \ell^2$-convergent if (and only if) 
$\dtx{t} / \dtx{x} \leq 1/c$.
...
\begin{enumerate}
\item We may also determine the $\displaystyle L^2$ stability as defined in
...
\]
We get $\displaystyle p_k(\lambda) = \lambda^2 - 2\delta_k \lambda +1$
...
$\displaystyle L^2([0,2\pi])$ is replaced by
$\displaystyle L^2([0,L])$ with the norm
$\displaystyle \|f\|_2 =
...
$\displaystyle \alpha_k \lambda^2 - \beta_k \lambda - \gamma_k$.  These
...
\item Finally, we may study the $\displaystyle \ell^2$ stability as presented in
...
\sol{\SOL}{
\subQ{a} Since $\displaystyle P P^\ast = P^\ast P$, we have that
\[
...
\end{equation}
because $\|x\|=1$ for
$\displaystyle x = \left\|P^\ast P y \right\|^{-1} P^\ast P y$.  Thus
...
is $\displaystyle \ell^2$-stable according to
Definition~\ref{fdmStableDefNo4}.\\
...
$\displaystyle \ell^2$-stable if $0 < \alpha \leq 1/2$.
}
