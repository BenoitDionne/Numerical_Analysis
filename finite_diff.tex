...
differentiable functions.  Let $\DS u:\RR^2 \to \RR$ be a sufficiently
...
$\DS \pdydx{u}{x}$ at the mesh point $(x_i,y_j)$.
If we assume that $u$ is of class $\DS C^2$, we have
\[
...
\end{equation}
Since $\DS \pdydxn{u}{x}{2}$ is continuous on the close set
...
$\DS \frac{1}{2} \pdydxn{u}{x}{2}(\zeta_{i,j},y_j) \dtx{x}$ satisfies
\[
...
derive a finite difference formula for $\DS \pdydx{u}{x}$ at
...
+ \frac{1}{2} \pdydxn{u}{x}{2}(\zeta_{i,j},y_j) \dtx{x} \ .
\end{equation}
As we did above, if $\DS \pdydxn{u}{x}{2}$ is continuous on
...
\]
and, because $\DS \pdydxn{u}{x}{3}$ is continuous, we can
...
\end{align*}
Solving for $\DS \pdydxn{u}{x}{2}(x_i,y_j)$, we get
...
\frac{u(x_{i+1},y_j) - 2u(x_i,y_j) + u(x_{i-1},y_j)}{(\dtx{x})^2}
\end{equation}
and, because $\DS \pdydxn{u}{x}{4}$ is continuous, we can
show as we did before that the truncation error 
$\DS \frac{1}{4!} \left( \pdydxn{u}{x}{4}\left(\zeta_{i,j},y_j\right)
+ \pdydxn{u}{x}{4}\left(\eta_{i,j},y_j\right) \right) (\dtx{x})^2$
satisfies
\[
\frac{1}{4!} \left( \pdydxn{u}{x}{4}\left(\zeta_{i,j},y_j\right)
...
$\DS u_{i,j} = u\left(x_i,t_j\right)$
...
$\DS \alpha = \frac{c^2\dtx{t}}{(\dtx{x})^2}$,
...
$\DS \alpha = \frac{c^2\dtx{t}}{2 (\dtx{x})^2}$,
$w_{0,j} = h_0(t_j)$ and $w_{N,j} = h_L(t_j)$ for $0\leq j \leq M$, and
$w_{i,0} = g(x_i)$ for $0 \leq i \leq N$.
\end{algo}
...
$u(0,t) = t(4-t)$ and $\DS u(2,t) = t(4-t)^2$ for $0 \leq t \leq 4$.
...
$\DS u_{i,j} = u\left(x_i,y_j\right)$
for $0\leq i \leq N$ and $0\leq j \leq M$.
...
$u(x,0) = x(2-x)$ and $\DS u(x,4) = x(2-x)^2$ for
$0 \leq x \leq 2$, and $u(0,y) = y(4-y)$ and
$\DS u(2,y) = y(4-y)^2$ for $0 \leq y \leq 4$.
...
$\DS \pdydx{u}{t}$ is useful here.  The initial condition of
$\DS \pdydx{u}{t}$ may be evaluated with the formula
...
finite difference formula to approximate $\DS \pdydx{u}{t}$
...
used to approximate $\DS \pdydxn{u}{t}{2}$.  We have for
...
of $R$ is defined as the set $\DS R^\circ = R \setminus \partial R$. 
...
$\DS R^\circ_\Delta = R_\Delta \setminus \partial R$.
...
\begin{align*}
\partial R_\Delta
&= \{ (x_i,0) : x_i = i \dtx{x} \ \text{for} \ 0 \leq i \leq N \} \\
& \qquad \cup 
...
= F(x_i,y_j)
\end{equation}
for all $(i,j)$ such that $\DS (x_i,y_j) \in R^\circ_\Delta$, where
...
referring to $\DS (x_i,y_j) \in R^\circ_\Delta$.  This is
not really correct.
...
$\DS R^\circ_\Delta$.   The point $(x_i,y_j)$ may be on the boundary as
...
\[
B\left(u(x,t), \DS \pdydx{u}{x}(x,t), \DS
...
\[
B\left(u(x,y), \DS \pdydx{u}{x}(x,y), \DS
...
\begin{pmatrix} u(x,t) \\ \DS \pdydx{u}{t}(x,t) \end{pmatrix}
...
As before $\DS u_{i,j} = u\left(x_i,y_j\right)$ for
...
\end{align*}
for all $(i,j)$ such that $\DS (x_i,y_j) \in R^\circ_\Delta$.
...
\[
\max_{\substack{(i,j) \text{ such that}\\(x_i,y_j) \in R^\circ_\Delta}}
...
$\tau_{i,j}$ for $\DS (x_i,y_j) \in R^\circ_\Delta$ though
$(x_i,y_j)$ may not be in $\DS R^\circ_\Delta$.  The formula
(\ref{fdm_fde_tr}) is used to approximate the value of
$u(x_i,y_{j+1})$.  It is $(x_i,y_{j+1})$ which is really in
$\DS R^\circ_\Delta$.   The point $(x_i,y_j)$ may be on the boundary.

\begin{rmk}[Warning]
The expression
$\DS P\left(q(x_i,y_j), \pdydx{q}{x}(x_i,y_j), \pdydx{q}{y}(x_i,y_j),
\ldots \right)$ in the definition of
$\DS \tau_{i,j}(\dtx{x},\dtx{y},q)$ may in fact be
...
\]
for all $(i,j)$ such that $\DS (x_i,y_j) \in R^\circ_\Delta$, and
\[
...
\begin{proof}
For every $\DS (x_i,y_j) \in R^\circ_\Delta$, we have
\begin{align*}
...
\subsection{$\DS \ell^2$ Theory} \label{ell2Theory}
...
assume that $\DS \pdydx{u}{t}(x,0) = h(x)$ for a periodic
...
$(i,j)$ such that
\[
(x_i,t_j) \in R^\circ_\Delta = \left\{ (x_i,t_j) : x_i = i \dtx{x} \ \text{for} \
i \in \ZZ \ \text{and} \ t_j = j \dtx{t} \ \text{for} \ 0 < j \leq M \right\}
\]
and $w_{i,j} \approx u_{i,j}$.

We consider the space $\DS \ell^2(\ZZ)$ of all functions
$g:\ZZ \to \CC$ such that
\[
...
$\DS Q_\alpha:\ell^2(\ZZ) \to \ell^2(\ZZ)$ is
...
\end{cases}
\]
for $r,s \in \ZZ$.  Recall that
$\DS \alpha = c \dtx{t} / (2 (\dtx{x})^2)$.
...
$\DS g_j \in \ell^2(\ZZ)$ for all $j \geq 0$ is
{\bfseries $\DS \ell^2$-convergent}
...
\]
where $\DS u_t(i) = u(i \dtx{x}, t)$ for all $i \in \ZZ$.
\end{defn}
...
with $\DS g_j \in \ell^2(\ZZ)$ for all $j \geq 0$ is
{\bfseries $\DS \ell^2$-stable}\index{Finite Difference
Methods!$\ell^2$-Stable} if there exists a constant $C_\alpha$ such
that $\DS \|Q_\alpha^j\|_2 \leq C$ for $0 \leq j \leq M$ and all $M$.

The index $\alpha$ for $C_\alpha$ is to indicate that there may be a
constraining relation on $\dtx{x}$ and $\dtx{y}$ that must be
satisfied for $\DS \|Q_\alpha^j\|_2 \leq C$ to be satisfied for
...
value does not increase in $\DS \ell^2$ norm.  This can be
heuristically justified as it follows.  Suppose that $\tilde{g}_j$ is
the computed value obtained using the finite difference scheme.  We
may assume that
\begin{equation}\label{FdmStabNo2QestMark}
  \tilde{g}_{j+1} = Q_\alpha(\tilde{g}_j) + \dtx{t}\, \delta_j \ ,
\end{equation}
where $\DS \delta_j \in \ell^2(\ZZ)$ represents the round off error.
...
\end{align*}
for $0 < j \leq M$.  The error is bounded in $\DS \ell^2$ norm.  In
...
linear mapping on $\DS \RR^n$.   Let $E$ be a Hilbert space
with the norm $\|\cdot\|$ defined by a scalar product
$\ps{\cdot}{\cdot}$, and let
$P:E \to E$ be a bounded linear mapping.  As in finite dimension, we
have that $\DS \|P^j\| \leq \|P\|^j$ and $\rho(P) \leq \|P\|$,
where $\rho(P)$ is the
{\bfseries spectral radius}\index{Linear Mapping!Spectral Radius} of
$P$.  Moreover, we have that
$\DS \rho(P) = \lim_{k\to \infty} \|P^k\|^{1/k}$ as in
finite dimension.  The
{\bfseries adjoint}\index{Linear Mapping!!Adjoint Operator}
of $P$ is the bounded linear mapping $\DS P^\ast$ such that
$\DS \ps{Px}{y} = \ps{x}{P^\ast y}$ for all $x,y \in E$.
The operator $\DS P^\ast$ is the equivalent of the transpose of a \nn
matrix.  A bounded linear operator $P$ is
{\bfseries normal}\index{Linear Mapping!!Normal Operator}
if $\DS P P^\ast = P^\ast P$
\footnote{Symmetric operator (i.e. $\DS P = P^\ast$) are
obviously normal operators.}.  Again, as in finite dimension, if $P$
is a normal operator, then $\rho(P) = \|P\|$.
...
with $\DS g_j \in \ell^2(\ZZ)$ for all $j \geq 0$.  If there exists a
constant $C_\alpha$ such that
$\DS \left\|Q_\alpha \right\|_2 \leq 1 + C_\alpha \dtx{t}$ for all
$\dtx{t}$. then the finite difference scheme is $\DS \ell^2$-stable.
\end{prop}
...
We have used the relation $\DS e^x \geq 1 + x$ for all $x \in \RR$.
\end{proof}

\begin{prop}[von Neumann] \label{vonNNo1}
If a finite difference scheme of the form $g_{j+1} = Q_\alpha(g_j)$
with $\DS g_j \in \ell^2(\ZZ)$ for all
$\DS j \geq 0$ is $\DS \ell^2$-stable, then
there exists a constant $C_\alpha$ such that
$\rho(Q_\alpha) \leq 1 + C_\alpha/M$ for all $M$.
\end{prop}

\begin{proof}
Since the finite difference scheme is $\DS \ell^2$-stable, we have
$\DS \|Q_\alpha^j\|_2 \leq C_\alpha$ for $0\leq j \leq M$.
Hence,
\[
...
Consider $\DS f(x) = e^{x\ln{C_\alpha}} - 1 - C_\alpha x$ for
$0 \leq x \leq 1$.  Since
$\DS f'(x) = \ln(C_\alpha) e^{x\ln{C_\alpha}} - C_\alpha \leq 0$ for
...
\begin{prop}[Lax]
Consider a finite difference scheme of the form $g_{j+1} = Q_\alpha(g_j)$
with $\DS g_j \in \ell^2(\ZZ)$ for all $j \geq 0$.
If $Q_\alpha$ is a normal operator, then the finite difference scheme is
$\DS \ell^2$-stable if and only if there exists a constant
$C_\alpha$ such that $\rho(Q_\alpha) \leq 1 + C_\alpha/M$ for all $M$.
\end{prop}

\begin{proof}
Since $Q_\alpha$ is normal, we have that
$\DS \left\|Q_\alpha^2 \right\|_2 = \left\|Q_\alpha\right\|^2$.
The reader is asked to prove this result in Exercise~\ref{fdmQ2}.  By
induction, we have
$\DS \left\|Q_\alpha^{2^k} \right\|_2 = \left\|Q_\alpha\right\|^{2^k}$
for all $k\geq 0$.

From $\DS \rho\left(Q_\alpha\right)
...
and all $M$ is a sufficient condition for the
$\DS \ell^2$ stability of
the finite difference scheme.  It follows from
Proposition~\ref{vonNNo1} that
$\rho\left(Q_\alpha\right) \leq 1 + C_\alpha/M$ for a constant $C_\alpha$
and all $M$ is a necessary condition for the $\DS \ell^2$
...
Consider a finite difference scheme of the form $g_{j+1} = Q_\alpha(g_j)$
where $\DS g_j \in \ell^2$.   If this finite difference scheme is
$\DS \ell^2$-stable and consistent, then it is
$\DS \ell^2$-convergent. 
\end{theorem}

A proof of this result is given in \cite{WH} (Theorem~6.22).  They
also prove that $\DS \ell^2$-convergence implies
$\DS \ell^2$-stability.  They
...
We consider the space $\DS L^2([0,2\pi])$ of all integrable
functions $f:[2,\pi] \to \CC$ such that
\[
  \|f\|_2^2 = \frac{1}{2\pi} \int_0^{2\pi} |f(x)|^2 \dx{x} < \infty \ .
\]
We have used the same notation for the norm in
$\DS \ell^2(\ZZ)$ and the norm
in $\DS L^2([0,2\pi])$.  The reader should be able by the context to
...
\]
for $k\in \ZZ$ and $i \in \CC$ is such that $\DS i^2 =-1$.
The function $\hat{f}$ is the Fourier transform of $f$.  We could have
...
\]
If we set
$\DS \check{Q}_\alpha = \Phi^{-1}\circ Q_\alpha \circ \Phi$, we get
$\check{g}_{j+1} = \check{Q}_\alpha\left(\check{g}_j\right)$ with
$\DS \check{Q}_\alpha: L^2([0,2\pi]) \to L^2([0,2\pi])$ a
linear mapping.

Since $\DS \Phi : L^2([0,2\pi]) \to \ell^2(\ZZ)$ is an isometry with
inverse $\DS \Phi^{-1}$, we have that they are both of
induced norm $1$.  Hence,
\[
...
the form $g_{j+1} = Q_\alpha(g_j)$ for $j\geq 0$ is $\DS \ell^2$-stable
if there exists a constant $C$ such that
$\DS \|\check{Q}_\alpha^j\|_2 \leq C$
for all $j\geq 0$.

To determine if a finite difference scheme of the form
$g_{j+1} = Q_\alpha(g_j)$ with $\DS g_j \in \ell^2(\ZZ)$
is $\DS \ell^2$-stable,
we have to prove that there exists a constant $C_\alpha$ such that
$\DS \|Q_\alpha^j \|_2\leq C_\alpha$ or
$\DS \|\check{Q}_\alpha^j \|_2\leq C_\alpha$ for all $j$.
...
$\check{Q}_\alpha$ to be able to determine the
$\DS \ell^2$ stability of
...
\end{align*}
Let $\DS \tilde{Q}_\alpha(x) = \sum_{s\in \ZZ} q_s e^{-sx i}$ for
$x \in \RR$.  The action of $\check{Q}_\alpha$ on a function in
$\DS f \in L^2([0,2\pi])$ is just the product $\tilde{Q}_\alpha\,f$.
...
\]
where the last norm is just the $\DS L^2$-norm of the function
$\tilde{Q}_\alpha$.

To use Proposition~\ref{LaxN1} to show that a finite difference scheme
is $\DS \ell^2$-stable, we may simply show that
\[
...
otherwise, where
$\DS \alpha = \frac{c^2\dtx{t}}{(\dtx{x})^2}$.  Thus
\[
...
Algorithm~\ref{fdm_sch1S} is $\DS \ell^2$-stable for
$\DS \frac{c^2\dtx{t}}{(\dtx{x})^2} \leq \frac{1}{2}$.
\end{egg}

It could be trickier to use the theory developed above to prove that
an implicit finite difference scheme is $\DS \ell^2$-stable
because we may
...
given by $\DS Q_\alpha = -J^{-1} K$, where $J$ and $K$ are defined in
...
$\DS A_\alpha(g_{j+1}) = B_\alpha(g_j)$ for
$\DS g_j \in \ell^2(\ZZ)$,
where $\DS A_\alpha , B_\alpha:\ell^2(\ZZ) \to \ell^2(\ZZ)$
are two bounded linear mapping.  This relation can be rewritten explicitly as
...
\begin{rmk}
The $\DS \ell^2$ theory above can be expanded to finite
difference scheme that are more then one-step
schemes.   \label{ell2TheoryMsteps}
...
\end{equation}
where
$\DS R(x) = \sum_{s\in \ZZ} a_s e^{-sx i}$,
$\DS S(x) = \sum_{s\in \ZZ} b_s e^{-sx i}$ and
$\DS T(x) = \sum_{s\in \ZZ} c_s e^{-sx i}$.
...
\]
for all $x$.  Since
$\DS \left\|\tilde{Q}_\alpha(x)\right\|_\infty \leq 1$
independently of the value of $\alpha$, we have that the
Crank-Nicolson scheme is unconstrained $\DS \ell^2$-stable.
\label{CNell2StableNo2}
\end{egg}

\subsection{$\DS L^2$ Stability}\label{L2Stab}
...
often presented as the von Neumann's method in many books.  The
$\DS L^2$ notation in this section is not widely used but we
use it to distinguish the definition of stability presented in this
section from the definition of stability presented in other sections.
...
by functions $\DS h_j \in L^2([0,2\pi])$ for $0 \leq j \leq M$.
So, $\DS w_{k,j} = h_j(x_k)$ for $0 \leq k \leq N$ and
$0\leq j \leq M$.

We now define the stability as it follows.

\begin{defn} \label{fdmStableDefNo3}
A finite difference scheme is
{\bfseries $\DS L^2$-stable}\index{Finite Difference
...
$\DS h_j(x) = \sum_{k\in \ZZ} A_{k,j} e^{kxi}$, where $i$
is the complex number such that $\DS i^2=-1$.  We have that
$\DS g(x) = h_0(x) = \sum_{k\in \ZZ} A_{k,0} e^{kxi}$
with $\DS A_{k,0} = \frac{1}{2\pi} \int_0^{2\pi} g(x) e^{-kxi}\dx{x}$.
...
ratios $\DS \lambda_k = \beta_k/\alpha_k$ to
determine.  We only need to compute $\lambda_k$ for $0 \leq k < N$.

If there exists a constant $C_\alpha$ such that
$\DS |\lambda_k|^j \leq C_\alpha$ for
$0 \leq j \leq M$ and $0 \leq k <N$,
then $\DS |A_{k,j}|^2 \leq C_\alpha^2 |A_{k,0}|^2$ for
...
$\DS |\lambda_k|^j \leq C$ for $0 \leq j \leq M$ and $0 \leq k < N$.
...
$N$ and $M$ because $\DS |\lambda_k|^j \leq C$ may be true only if
...
$\DS \alpha_k \lambda^2 - \beta_k \lambda - \gamma_k$ has two distinct
...
If we assume that $\DS \VEC{w}_j = h_j(x_k)$ for
$\DS h_j \in L^2([0,2\pi],\RR^n)$ \footnote{The functions
$\DS h:[0,2\pi] \to \RR^n$ such that each component is in
$\DS L^2([0,2\pi])$.},
...
$\DS h_j(x) = \sum_{k\in \ZZ} \VEC{A}_{k,j} e^{k x\, i}$,
where $\DS A_{k,j} \in \RR^n$, in the previous equation, we find that
...
\end{equation}
If there exists $C_\alpha$ such that
$\DS \|Q_k^j\|_2 \leq C_\alpha$ for
$0\leq j \leq M$ and $k \in \ZZ$,  we get from Parseval equality that
\[
...
{\bfseries $\DS L^2$-stable} if and only if there exists
$C_\alpha$ such that $\DS \|Q_k^j\|_2 \leq C_\alpha$ for
$0\leq j \leq M$, $k \in \ZZ$ and $N>0$.
\label{QkjCstable}
\end{prop}

\begin{proof}
We have proved above that the condition $\DS \|Q_k^j\|_2 \leq C$ for
$0\leq j \leq M$ and $k \in \ZZ$ was sufficient.   We now prove that
it is necessary.  Suppose that $\DS \|Q_{k_0}^{j_0}\|_2 > C_\alpha$ for
some $k_0$ and $j_0$.  Choose $\DS \VEC{w} \in \RR^n$ such that
$\DS \|Q_{k_0}^{j_0}\VEC{w}\|_2 > C_\alpha \|\VEC{w}\|_2$.
If we consider a boundary value problem
such that $\DS g(x) = \VEC{w} e^{k_0 x\, i}$, then
$\DS h_0(x) = \VEC{A}_{k_0,0} e^{k_0 x\, i}$ with
$\VEC{A}_{k_0,0} = \VEC{w}$.  From (\ref{fdmgfL2G}), we have
$\DS \VEC{A}_{k_0,j} = Q_{k_0}^j\VEC{w}$.   Thus
\[
...
{\bfseries $\DS L^2$-stable}, then there exists a constant
$C_\alpha$ such that $\rho(Q_k) \leq 1 + C_\alpha/M$ for all
$k \in \ZZ$, $N$ and $M$.
\end{prop}

\begin{proof}
Since the finite difference scheme is $\DS L^2$-stable, we get from
Proposition~\ref{QkjCstable} that
$\DS \|Q_k^j\|_2 \leq C_\alpha$ for $0\leq j \leq M$ and $k \in \ZZ$.
...
Suppose that
$P_\Delta(w_{i,j},w_{i,j+1},w_{i+1,j}, \ldots) = F(x_i,t_j)$ for all
$(i,j)$ such that $\DS (x_i,t_j) \in R^\circ_\Delta$ can be expressed
in vector form as $\VEC{w}_{j+1} = Q_\alpha\VEC{w}_j + \VEC{B}_j$
for $0 \leq j < M$, where
\begin{enumerate}
\item $Q_\alpha$ is a \nm{(N-1)}{(N-1)} matrix,
\item $\DS \VEC{w}_j = \begin{pmatrix}
...
the values $w_{i,j}$ for $(i,j)$ such that
$\DS (x_i,t_j) \in R^\circ_\Delta$.
...
$\DS Q_\alpha = -J^{-1}K$ for $K$ given in (\ref{fdm_K}), $J$ given in
(\ref{fdm_J}) and
\[
...
We consider the norm on $\DS \RR^{N-1}$ defined by
\[
  \| \VEC{y} \|_N = \left(\sum_{i=1}^{N-1} y_i^2\,\dtx{x}\right)^{1/2}
\]
for $\DS \VEC{y} \in \RR^{N-1}$.  If $y_i = g(x_i)$ for all $i$, where
$g:[0,L]\to \RR$ is a continuous function, then
$\DS \|\VEC{y} \|_N \to \left( \int_0^L g^2(x)\dx{x}\right)^{1/2}$
...
\]
where $\DS \VEC{u}_j = \begin{pmatrix}
...
consistency according to the previous definition because the
$\DS i^{th}$ component of $\VEC{\tau}_j(\dtx{x},\dtx{t},q)$ is
...
&\leq L \max_{\substack{(i,j) \text{ such that}\\(x_iy_j) \in R^\circ}}
...
$\VEC{w}_{j+1} = Q_\alpha\VEC{w}_j + \VEC{B}_j$
with $\DS \VEC{w}_j \in \RR^{N-1}$ is
{\bfseries $\DS \ell^2$-stable}
\index{Finite Difference Methods!$\ell^2$-Stable} if there
exists a constant $C_\alpha$ such that
$\DS \|Q_\alpha^j\|_N \leq C_\alpha$
...
$\DS \ell^2$-norm instead of in the uniform norm as $M$ increases.
...
\ $\DS Q_\alpha Q_\alpha^\top = Q_\alpha^\top Q_\alpha$)
and the induced norm on the \nm{(N-1)}{(N-1)} matrices is
the Euclidean norm $\|\cdot\|_2$ on $\DS \RR^{N-1}$, then
...
that $\DS \|Q_\alpha^j\|_N \leq \|Q_\alpha\|_N^j \leq 1$.
...
A finite difference scheme of the form
$\VEC{w}_{j+1} = Q_\alpha\VEC{w}_j + \VEC{B}_j$,
where $\DS \VEC{w}_j \in \RR^{N-1}$ and $Q_\alpha$ is normal, is
$\DS \ell^2$-stable according to Definition~\ref{fdmStableDefNo4}
...
$\DS \left\| \VEC{w}_1- \VEC{w}_2 \right\| \leq
...
Definition~\ref{fdmStableDefNo3} and \\
Definition~\ref{fdmStableDefNo4}.
...
features other than uniform convergence.  So $\DS \ell^2$
convergence and stability may be preferable.
...
Using Definition~\ref{fdmStableDefNo4} to determine
$\DS \ell^2$ stability may seem to be the next reasonable
option but that requires a nice matrix (usually ``near'' diagonal) to
be able to determine the eigenvalues of $Q_\alpha$.  Using
Definitions~\ref{fdmStableDefNo1}
...
$\DS v_j = \alpha \rho_1^j + \beta j \rho_1^j$ for
$0 \leq j \leq n+1$. Since $v_0 = 0$ implies $\alpha=0$, we get that
$v_{n+1} = 0$ implies $\DS \beta (n+1) \rho_1^{n+1} = 0$.
It follows that $\beta=0$.  We find $v_j=0$ for all $j$ which is not
possible for an eigenvector.
...
\]
Thus $\DS \left(\rho_1/\rho_2\right)^{n+1} = 1$.
...
Suppose that $\VEC{v}_1$, $\VEC{v}_2$, \ldots, $\VEC{v}_n$ in
$\DS \RR^n$ are $n$ linearly independent eigenvectors for
each matrix $Q_{i,j}$.
Let $\DS \lambda_{i,j,k}$ be the eigenvalue of $Q_{i,j}$
...
Since $\DS B\left(u(x,y), \ldots \right) = u(x,y)$,
$\DS B_\Delta(w_{i,j},\ldots) = w_{i,j}$ and $w_{i,j} = u(x_i,t_j)$
...
\end{align*}
for $0<i<N$ and $0\leq j < M$,
where $\DS \alpha = \frac{c^2\dtx{t}}{(\dtx{x})^2}$.
...
\]
and
$\DS F = \max_{\substack{0<i<N\\0\leq j<M}} |f(x_i,y_j)|$.
...
$\DS v_{j+1} \leq v_j + F \dtx{t}$ for $0\leq j < M$.
By induction, we get
\[
...
for $(i,j)$ such that $\DS (x_i,t_j) \in R_\Delta^\circ$
...
+ \max_{\substack{(i,j)\ \text{such that}\\(x_i,t_j) \in R_\Delta^\circ}}
...
Algorithm~\ref{fdm_sch1S} is $\DS \ell^2$-stable if
$\DS c^2\dtx{t}/(\dtx{x})^2 \leq 1/2$.
...
to be very small if $\dtx{x}$ is small.  If
$\DS \dtx{x} < 10^{-2}$, then
$\DS \dtx{t} <  10^{-4}/(2c^2)$.  Thus, a lot of computations
...
\]
where $\DS \alpha = \frac{c^2\dtx{t}}{(\dtx{x})^2}$,  Let
$\DS R_j = \max_{0 < i < N}|r_{i,j}|$.  Since we assume that
$w_{i,j} = u(x_i,t_j)$ for $i=0$ and $i=N$, we have in fact that
$\DS R_j = \max_{0 \leq i \leq N}|r_{i,j}|$.
...
$\DS \sum_{m=1}^\infty|\beta_m|$ and
$\DS \sum_{m=1}^\infty\beta_m^2$ converge.  This ensure that
...
have that $\DS b = b_n = 2n\pi/L$ for an integer $n$ that we
may assume positive.
...
$\DS c^2T/L^2\leq S < 2c^2T/L^2$, where we
assume that $\DS 2c^2 T/ L^2 > 1$.
If $\DS N = 2^k$ and $\DS M= 2^{2k}S$ for $k>0$
arbitrary, then $\DS \dtx{x} = L/2^k$ and
$\DS \dtx{t} = T/(S2^{2k})$.  We have that
\[
...
\]
with
$\DS \frac{1}{2} <  \frac{T c^2}{SL^2} \leq 1$.
We get
\[
w_{i,j} = \sum_{m=1}^\infty \beta_m
...
For $m > k$, we have
$\DS \sin^2\left( \frac{2^m\pi}{2^{k+1}} \right)
...
For $m < k - 1$, we have
$\DS \sin^2\left(\frac{\pi}{2^{k+1-m}} \right) \leq \frac{1}{4}$
because
$\DS 0 \leq \frac{2^m\pi}{2^{k+1}} = \frac{\pi}{2^{k+1-m}}
\leq \frac{\pi}{8}$
for $k-m +1 > 2$.  Thus
\[
...
$\DS \beta_m = e^{-2^m} >0$ \footnote{Any sequence
$\DS \{\beta_m\}_{m=0}^\infty$ that preserves the convergence of
$\DS \sum_{m=1}^\infty|\beta_m|$ and
$\DS \sum_{m=1}^\infty\beta_m^2$, and such that
$\DS |\beta_k| e^{2^{2k} S t_j/T} \to \infty$ as $k \to
\infty$ can be used.}.  We have that
$\DS \sum_{m=1}^\infty|\beta_m|$ and
$\DS \sum_{m=1}^\infty\beta_m^2$ converge as required.
...
We study the $\DS \ell^2$ convergence according to
...
$\DS \ell^2$ consistency according to
Definition~\ref{fdm_consist_defN2}
...
\begin{align*}
&P_\Delta\left(w_{i,j}, w_{i,j+1}, w_{i+1,j}, \ldots\right) \\
&\quad = \frac{w_{i,j+1} - w_{i,j}}{\dtx{t}}
- c^2 \left( \frac{w_{i+1,j} - 2w_{i,j} + w_{i-1,j}}{(\dtx{x})^2}
+ \frac{w_{i+1,j+1} - 2w_{i,j+1} + w_{i-1,j+1}}{(\dtx{x})^2} \right)
\end{align*}
...
P\left(q(x_i,t_j), \pdydx{q}{x}(x_i,t_j), \pdydx{q}{y}(x_i,t_j),
\pdydxn{q}{x}{2}(x_i,t_j), \ldots \right) \right. \\
&\qquad \qquad \left. + P\left(q(x_i,t_{j+1}), \pdydx{q}{x}(x_i,t_{j+1}),
\pdydx{q}{y}(x_i,t_{j+1}), \pdydxn{q}{x}{2}(x_i,t_{j+1}), \ldots \right)
\right) \\
&\quad = \left( -\frac{1}{4}\, \pdydxn{q}{t}{3}(x_i,\breve{\xi}_j)
+ \frac{1}{12}\, \pdydxn{q}{t}{3}(x_i,\xi_{i,j})
+ \frac{1}{12}\, \pdydxn{q}{t}{3}(x_i,\tilde{\xi}_{i,j}) \right) (\dtx{t})^2 \\
&\quad \qquad
...
scheme is $\DS \ell^2$-stable according to
Definition~\ref{fdmStableDefNo4}.

\begin{prop}
The Crank-Nicolson scheme in Algorithm~\ref{fdm_sch11S} is
$\DS \ell^2$-stable without constraints on $\dtx{t}$ and $\dtx{x}$.
\end{prop}
...
$\DS Q_\beta = -J^{-1}K$ for $K$ given in (\ref{fdm_K}) and
$J$ given in (\ref{fdm_J}), and 
\[
\VEC{B}_j = J^{-1} \begin{pmatrix}
\alpha \left( w_{0,j+1} + w_{0,j} \right)
+ \big( f(x_1,t_j) + f(x_1,t_{j+1}) \big)\dtx{t}/2 \\
...
Theorem~\ref{StabConstConv} for $\DS \ell^2$-convergence
...
for $\DS J^{-1}$ in the proof of Proposition~\ref{CKell2NConv}}, we
...
is $\DS \ell^2$-convergent according to Definition~\ref{ell2ConvDefN2}
...
Algorithm~\ref{fdm_sch11S}.  We have that
$\DS Q = -J^{-1}K$ for $K$ given
...
\end{align*}
and $\DS R^\circ_\Delta = R_\Delta \setminus \partial R_\Delta$.
...
$\DS (x_i,y_j)\in R^\circ_\Delta$, then
\[
  \max_{(x_i,y_j) \in R^\circ_\Delta} v_{i,j}
...
$\DS (x_{i_0},y_{j_0}) \in R^\circ_\Delta$, (i.e.\ $0 < i_0 < M$
and $0 < j_0 < N$) such that $v_{i_0,j_0} \geq v_{i,j}$ for all
$(i,j)$ with $\DS (x_i,y_j) \in R^\circ_\Delta$, and
...
$\DS (x_i,y_j)\in R^\circ_\Delta$, then
\[
  \min_{(x_i,y_j) \in R^\circ_\Delta} v_{i,j}
...
Suppose that
$\DS \{ w_{i,j}^{[k]} : 0\leq i \leq N \ \text{and} \ 0 \leq j
\leq M\}$
...
with $\DS w_{i,j} = w_{i,j}^{[1]} - w_{i,j}^{[2]}$ for all
$i$ and $j$ is
a solution of $\DS \Delta_\Delta w_{i,j} = 0$ for all $(i,j)$
such that $\DS (x_i,y_j) \in R^\circ_\Delta$, and $w_{i,j} = 0$
for all 
...
\leq \min_{(x_i,y_j) \in R^\circ_\Delta} w_{i,j}
\leq \max_{(x_i,y_j) \in R^\circ_\Delta} w_{i,j}
\leq \max_{(x_i,y_j) \in \partial R_\Delta} w_{i,j} = 0 \ .
\]
Thus $\DS w_{i,j}^{[1]} - w_{i,j}^{[2]} = 0$ for $0\leq i \leq N$ and
$0\leq j \leq M$.
...
\leq \max_{(x_i,y_j) \in \partial R_\Delta} |v_{i,j}|
+ \frac{a^2}{2} \max_{(x_i,y_j) \in R^\circ_\Delta}
...
\end{align*}
We obviously have that $\DS 0 \leq h(x_i,y_j) \leq a^2/2$ for all
...
let
$\DS
K = \max_{(x_i,y_j) \in R^\circ_\Delta} \left| Q_\Delta(v_{i,j})\right|$.
We define two additional functions.
\begin{align*}
  g^{\pm} : R_\Delta & \to \RR \\
  (x_i,y_j) & \mapsto \pm v(x_i,y_j) + K h(x_i,y_j) = \pm v_{i,j} + K h_{i,j}
\end{align*}
Let $\DS g_{i,j}^{\pm} = g^{\pm}(x_i,y_j) = \pm v_{i,j} + K h_{i,j}$ for
...
\]
for all $(i,j)$ such that $\DS (x_i,y_j) \in R^\circ_\Delta$.
Therefore, we may apply Theorem~\ref{LaplaceMaxPrinc} to
$\DS g^{\pm}$.  For $\DS g^+$, we get
\begin{align*}
v_{i,j} & \leq \max_{(x_i,y_j) \in R^\circ_\Delta} v_{i,j}
\leq \max_{(x_i,y_j) \in R^\circ_\Delta} g^+_{i,j}
...
\]
for all $(i,j)$ such that $\DS (x_i,y_j) \in R^\circ_\Delta$.  This
...
that $\DS (x_i,y_j) \in R^\circ_\Delta$,  and 
...
$(i,j)$ such that $\DS (x_i,y_j) \in R^\circ_\Delta$, and
$w_{i,j} = g(x_i,y_j)$ for all $(i,j)$ such that
$\DS (x_i,y_j) \in R^\circ_\Delta$.
...
\leq \frac{a^2}{2} \max_{(x_i,y_j) \in R^\circ_\Delta}
\left|Q_\Delta(r_{i,j})\right|
= \frac{a^2}{2}
\max_{(x_i,y_j) \in R^\circ_\Delta} |\tau_{i,j}(\dtx{x},\dtx{y},u)| \ .
\]
...
knowledge of $\DS L^2$-spaces and Fourier series.
...
roots of the characteristic equation $\DS \lambda^2-k=0$.
...
$\DS B \sin\left(L\sqrt{-k}\right) = 0$.  If we take $B=0$,
we get the trivial solution.  We must therefore have
$\DS \sin\left(L\sqrt{-k}\right) = 0$ with $k\neq 0$.  This implies
that $\DS k = k_n = -\left(n\pi/L\right)^2$ for $n$ a
positive integer.  The boundary value problem (\ref{waveLT5}) has non-trivial
solutions only for $\DS k=k_n=-\left(n\pi/L\right)^2<0$ with
...
(\ref{waveLT4}) with $\DS k=k_n=-\left(n\pi/L\right)^2$;
namely,
\[
...
$\DS \lambda^2 + (c n\pi/L)^2=0$.  The two
...
form $\DS   \VEC{v}_t + A \VEC{v}_x = 0$,
where $\DS \VEC{v}: \RR^2 \rightarrow \RR^n$ and $A$ is a
\nn real matrix, is an {\bfseries hyperbolic system}\index{Partial Differential
...
$\DS u = u(x,y) = g\left(x-cy\right)$.  The function $u$ is
...
compact set
$\DS R = \left\{ (x,t) : 0 \leq x \leq L \ \text{and} \ 0 \leq
...
prove $\DS \ell^2$ stability for Algorithm~\ref{fdm_sch3S}.
...
Since the matrix $\DS \tilde{Q}_k \equiv
...
$\DS \begin{pmatrix} 0 & 1 \\ 1 & \lambda_i\end{pmatrix}$
...
for $0 < k < N$.  
We have that
$\DS p_k(\lambda) = \lambda^2 -2 \delta_k \lambda + 1 = 0$
for
\[
...
Since we have $\DS \ell^2$ consistency and
$\DS \ell^2$ stability, we may 
conclude that the finite difference scheme in
Algorithm~\ref{fdm_sch3S} is $\DS \ell^2$-convergent if (and only if) 
$\dtx{t} / \dtx{x} \leq 1/c$.
...
\begin{enumerate}
\item We may also determine the $\DS L^2$ stability as defined in
...
We now have
$\DS \alpha = \left(\frac{2\pi c \dtx{t}}{L \dtx{z}}\right)^2$
...
\]
We get $\DS p_k(\lambda) = \lambda^2 - 2\delta_k \lambda +1$
as in the proof of Proposition~\ref{CFLCondProp}.  Proceeding as was done in
that proof, we get
\[
...
$\DS L^2([0,2\pi])$ is replaced by
$\DS L^2([0,L])$ with the norm
$\DS \|f\|_2 =
...
$\DS \alpha_k \lambda^2 - \beta_k \lambda - \gamma_k$.  These
characteristic polynomials are given by the coefficients of
$\DS e^{(2\pi k x/L) i}$ after we substitute the expression
of $w_j$ above in (\ref{fdmgfL2Bext}).  So, it suffices to substitute
$\DS w_{n,j} = \lambda^j e^{(2\pi k x_n/L) i}$ in
...
if we substitute $\DS w_{s,j} = \lambda^j e^{(2\pi k x_s/L) i}$
...
\end{align*}
If we divide by $\DS \lambda^{j-1} e^{(2\pi k x_s/L) i}$, we get
\begin{align}
...
$\DS \dtx{z} =  2 \pi \dtx{x}/L$.
\item Finally, we may study the $\DS \ell^2$ stability as presented in
...
\lambda(z) + 1 \ .
\]
Let $\DS \gamma(z) = 1 -2 \alpha \sin^2(z)$.  We find
\[
...
$\DS \frac{c^2\dtx{t}}{2 (\dtx{x})^2} \leq 1$.  This condition is
...
Consider a function $v:R_\Delta \to \RR$.  Let $v_{i,j} = v(x_i,t_j)$
for all $(x_i,t_j) \in R_\Delta$ and let\\
$\DS \frac{1}{2}\left( f(x_i,y_j) + f(x_i,y_{j+1})\right)
...
& \qquad \qquad = \frac{1}{2}\left( f(x_i,y_j) + f(x_i,y_{j+1})\right)
\end{align*}
for $0<i<N$ and $0\leq j < M$.  Thus
\begin{align*}
...
&\qquad \qquad + \frac{1}{2}\left( f(x_i,y_j) + f(x_i,y_{j+1})\right) \dtx{t}
\end{align*}
for $0<i<N$ and $0\leq j < M$, where
$\DS \alpha = \frac{c^2\dtx{t}}{(\dtx{x})^2}$.

Let $\DS v_j = \max_{0<i<N} |v_{i,j}|$ and
$\DS F = \max_{\substack{0<i<N\\0\leq j<N}} \frac{1}{2}
...
|B_\Delta(v_{i,0},v_{i,1},v_{i+1,0},\ldots)|
+ T \max_{\substack{(i,j)\ \text{such that}\\(x_i,t_j) \in R_\Delta^\circ}}
|P_\Delta(v_{i,j},v_{i,j+1},v_{i+1,j},\ldots)|
\end{align*}
for $0\leq j \leq M$ and $0<i<N$.  Since
...
\subQ{a} Prove that
$\DS \left\|P^2 \right\| = \left\|P P^\ast\right\|$. \\
\subQ{b} Use (a) to prove that $\DS \left\|P^2 \right\| = \|P\|^2$.
\label{fdmQ2}
\end{question}

\sol{\SOL}{
\subQ{a} Since $\DS P P^\ast = P^\ast P$, we have that
\[
...
\end{equation}
because $\|x\|=1$ for
$\DS x = \left\|P^\ast P y \right\|^{-1} P^\ast P y$.  Thus
...
= \|P\|^2 \ .
\]
We get from (a) that $\DS \left\|P^2\right\| \geq \|P\|^2$.
But we already know that $\DS \left\|P^2\right\| \leq \|P\|^2$
by a property of bounded linear operators.  Thus
$\DS \left\|P^2\right\| = \|P\|^2$.
}

\begin{question}
Prove that the finite difference scheme in Algorithm~\ref{fdm_sch1S}
is $\DS \ell^2$-stable according to
Definition~\ref{fdmStableDefNo4}.\\
...
Thus, the finite difference scheme is
$\DS \ell^2$-stable if $0 < \alpha \leq 1/2$.
}
