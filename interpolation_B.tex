...
\[
\VEC{b} = \begin{pmatrix}
\DS - z_0 \dtx{x_0} + \frac{6}{\dtx{x_1}}\left(f(x_2)
  -f(x_1) \right) - \frac{6}{\dtx{x_0}} \left(f(x_1) - f(x_0) \right) \\[0.8em]
\DS \frac{6}{\dtx{x_2}}\left(f(x_3) -f(x_2) \right)
- \frac{6}{\dtx{x_1}} \left(f(x_2) - f(x_1) \right) \\[0.8em]
\vdots \\
\DS \frac{6}{\dtx{x_{n-2}}}\left(f(x_{n-1}) -f(x_{n-2}) \right)
...
Let $\DS \VEC{w} \in \RR^3$ be the solution of
$A\VEC{w} = \VEC{b}$, where
\[
A = \begin{pmatrix}
2 (x_2-x_0) & x_2-x_1 & 0 \\
...
\[
\VEC{b} = \begin{pmatrix}
\DS 6\,\frac{f(x_2) - f(x_1)}{x_2-x_1} -
6\,\frac{f(x_1) - f(x_0)}{x_1-x_0} \\[0.8em]
\DS 6\,\frac{f(x_3) - f(x_2)}{x_3-x_2} -
6\,\frac{f(x_2) - f(x_1)}{x_2-x_1} \\[0.8em]
\DS 6\,\frac{f(x_4) - f(x_3)}{x_4-x_3} -
...
\[
\VEC{b} = \begin{pmatrix}
\DS - 6f'(x_0) + \frac{6}{\dtx{x_0}}\left(f(x_1)-f(x_0)\right)
\\[0.8em]
\DS \frac{6}{\dtx{x_1}}\left(f(x_2) -f(x_1) \right)
...
\DS \frac{6}{\dtx{x_{n-2}}}\left(f(x_{n-1}) -f(x_{n-2}) \right)
- \frac{6}{\dtx{x_{n-3}}} \left(f(x_{n-2}) - f(x_{n-3}) \right) \\[0.8em]
\DS \frac{6}{\dtx{x_{n-1}}}\left(f(x_n) -f(x_{n-1}) \right)
- \frac{6}{\dtx{x_{n-2}}} \left(f(x_{n-1}) - f(x_{n-2}) \right) \\[0.8em]
\DS 6f'(x_n) - \frac{6}{\dtx{x_{n-1}}}\left(f(x_n)-f(x_{n-1})\right)
\end{pmatrix}
\]
...
Let $\DS \VEC{z} \in \RR^3$ be the solution of
$A\VEC{z} = \VEC{b}$, where
\[
A = \begin{pmatrix}
2 (x_1-x_0) & x_1-x_0 & 0 \\
...
\[
\VEC{b} = \begin{pmatrix}
\DS - 6f'(x_0) + 6\, \frac{f(x_1) - f(x_0)}{x_1-x_0}\\[0.8em]
\DS 6\,\frac{f(x_2) -f(x_1)}{x_2-x_1}
- 6\,\frac{f(x_1) - f(x_0)}{x_1-x_0}\\[0.8em]
\DS 6f'(x_2) - 6\,\frac{f(x_2)-f(x_1)}{x_2-x_1}
\end{pmatrix}
= \begin{pmatrix}
2.976232722 \\
5.479684254 \\
...
From $\DS \sum_{j=1}^n b_{k,j}x_j = 0$, we get
\[
...
class $\DS C^2$ on the nodes $a = x_0 < x_1 < \ldots < x_n=b$, then
\[
...
$\DS \max_{0\leq i < n} |\Delta x_i|$ is small enough.
...
function $\DS \phi:[a,b] \rightarrow \RR^2$.  The function $\phi$ is
called a
...
$\DS \phi:[a,b]\to \RR^2$ defined by $\phi(x) = (x,f(x))$ is
a parametric representation of the curve $C$.
...
maps of degree three $\DS \phi_i:[0,1] \rightarrow \RR^2$ such
...
The mapping $\DS \phi_i:[0,1]\to \RR^2$ between
...
degree $\DS m \in \NNp$ for a function $f:[0,1]\rightarrow \RR$ is the
polynomial
...
\]
for $0 \leq i < n$, where $\DS \VEC{c}_{i,k} \in \RR^2$ satisfies
\[
...
$\DS \binom{m}{k} \VEC{c}_{i,k}$ for $k=1$, $2$,
...
\]
such that $\DS \lim_{i\rightarrow -\infty} t_i = -\infty$ and
$\DS \lim_{i\rightarrow \infty} t_i = +\infty$.
...
\end{equation}
for $i\in \ZZ$, where
$\DS v_i^k(t) = \frac{t-t_i}{t_{i+k}-t_i}$.
\end{defn}
...
{Clockwise from the top left corner: $\DS B_i^0$,
$\DS B_i^1$ and $\DS B_i^2$.}{Bsplines}
...
piecewise polynomials of degree $k$ and class $\DS C^{k-1}$.
\label{BSplineCkm1}
\end{prop}
...
that they are of class $\DS C^{k-1}$ requires induction and tedious
...
\begin{prop}
$\DS B_i^0(t) > 0$ for $t_i \leq t < t_{i+1}$ and
$\DS B_i^0(t) = 0$ otherwise.
For $k>0$, $\DS B_i^k(t) > 0$ for $t_i < t < t_{i+k+1}$
and $\DS B_i^k(t) = 0$ otherwise.
\label{Bsplienonneg}
\end{prop}
...
\]
for some constants $\DS c_i^k$.  If we set
$\DS C_i^r(t) = c_i^r$, we can use
the method presented in the proposition above to compute $p(t)$.
\end{rmk}

\begin{prop}
$\DS \sum_{i=-\infty}^{+\infty} B_i^k(t) = 1$ for all $t \in \RR$
and $k\geq 0$.
\label{Bsplinesum1}
\end{prop}

\begin{proof}
We use the previous proposition with $\DS C_i^k(t) = 1$ for
all $i$.  We have that
\[
...
$\DS C_i^r(t) = 1$ for all $i$ and all $r$ with
$0 \leq r \leq k$.  Thus,
\[
  \sum_{i=-\infty}^\infty B_i^k(t) = \sum_{i=-\infty}^\infty B_i^0(t) \ .
\]
For $t\in [t_j,t_{j+1}[$, we get that
\[
  \sum_{i=-\infty}^\infty B_i^k(t) = \sum_{i=-\infty}^\infty B_i^0(t)
  = B_j^0(t) = 1 \ .  \qedhere
\]
...
\begin{prop}
The set
$\DS \left\{ B_j^k, B_{j+1}^k, \ldots , B_{j+k}^k \right\}$
is linearly independent on $]t_{j+k},t_{j+k+1}[$.
\end{prop}

We note that the only B-splines $\DS B_i^k$ that are not
trivially null on $]t_{j+k},t_{j+k+1}[$ are those for
$j \leq i \leq j+k$.  The proof of
this proposition is by induction and requires the formula for the
derivative of the B-splines $\DS B_i^k$ that was required
for the proof of Proposition~\ref{BSplineCkm1}.

\begin{prop}
The set of B-splines
$\DS \left\{ B_{-k}^k, B_{-k+1}^k, \ldots , B_{n-1}^k \right\}$
is a basis for the space $\DS S_n^k$ of functions $p$ of
class $\DS C^{k-1}$ on $[t_0,t_n]$ such that
$\DS p\big|_{[t_i,t_{i+1}]}$ is a polynomials of degree at most
$k$ for $0\leq i < n$.
\label{BSplineLinInd}
\end{prop}
...
\begin{proof}
We note that the only B-splines $\DS B_i^k$ that are not
trivially null on $]t_0,t_n[$ are those for $-k \leq i \leq n-1$.

\subI{Linear Independence}
Suppose that $\DS \sum_{i=-k}^{n-1} c_i B_i^k = 0$ on $[t_0,t_n]$.
...
\[
 0 = \sum_{i=-k}^{n-1} c_i B_i^k(t) = \sum_{i=j}^{n-1} c_i B_i^k(t) =
  c_j B_j^k(t) \ .
\]
Since $\DS B_j^k(t) > 0$, we get $c_j = 0$.  This is a
contradiction that $c_j \neq 0$.  So, there is no such $j$ between $0$
and $n$ such $c_j \neq 0$.

In particular, this proves that $\DS S_n^k$ is at least of dimension
$n+k$. 

\subI{Generating set}
We prove that all functions $\DS p \in S_n^k$ can be
expressed as a linear combination over $\RR$ of the following $n+k$
functions in $\DS S_n^k$:
$\DS t^j$ for $0\leq j \leq k$ and
$\DS H(t-t_j)(t-t_j)^k$ for
...
\[
  ...
  \end{cases}
\]
This will prove that $\DS S_n^k$ is at most of dimension
$n+k$.  Combined with what we proved in the first part of the proof,
this shows that $\DS S_n^k$ is of dimension $n+k$ and that\\
$\DS \left\{ B_{-k}^k, B_{-k+1}^k, \ldots , B_{n-1}^k \right\}$
is a basis of $\DS S_n^k$.

Given $\DS p \in S_n^k$, we have that
$p_0 = p\big|_{[t_0,t_1]}$ is a polynomial of degree at most $k$.  So
$\DS p_0(t) = \sum_{i=0}^k a_i t^i$ for some $a_i \in \RR$.
...
most $k$.  Since $p$ is of class $\DS C^{k-1}$, we have that
\[
...
of the $n+k$ functions $\DS t^j$ for $0\leq j \leq k$ and
...
such that $\DS x_j < x_{j+1}$ for $1\leq j < n+k$, and
...
\nm{m}{m} matrix $Q$ with the entries
$\DS Q_{j,i} = B_{i+q}^k(x_j)$ for
$1 \leq i,j \leq m$.  Then, $Q$ is invertible if and only if
$Q_{j,j} \neq 0$ for $1 \leq j \leq m$.
\end{theorem}

To find the $\DS c_i^k$ required to satisfy
(\ref{BsplineFormCoeff}), we
have to solve the linear system $Q \VEC{z} = \VEC{y}$, where
$\DS Q_{j,i} = B_{i-k-1}^k(x_j)$ and $z_i = c_{i-k-1}$ 
...
only if $\DS Q_{j,j} = B_{j-k-1}^k(x_j) \neq 0$ for $1\leq j \leq n+k$;
...
We need $j-4 \leq i < j$ to possibly have that
$\DS B_i^3(x_j)$ is non null.
...
summation because $\DS B_i^3(x_j) = B_i^3(t_{j-1}) = 0$ for
these values of $i$.  There are $n$ equations with $n+2$ variables $c_i$ for
...
determine the $n+2$ variables $\DS c_i^3$ for $-3 \leq i \leq n-2$.
\end{egg}
...
\leq \left| f(t) - f(t_j)\right| + \left| f(t_j) - f(t_{j-1})\right| + \ldots \\
&\qquad + \left| f(t_{j-s+3}) - f(t_{j-s+2})\right|
...
Since every element of $\DS S_n^k$ is a linear combination of
$\DS B_{j}^k$ for $-k \leq j <n$, we get the following result from the
previous theorem.
...
$\DS b_0(t) = \frac{t^3}{2} - t^2 + \frac{2}{3}$,
$\DS
...
$\DS \sum_{j=-1}^2 b_j(t) = 1 $ for all $t$.
...
\DS 6\,\frac{f(x_5) -f(x_4)}{x_5-x_4}
- 6\,\frac{f(x_4) - f(x_3)}{x_4-x_3}\\[0.8em]
\DS 6f'(x_5) - 6\,\frac{f(x_5)-f(x_4)}{x_5-x_4}
\end{pmatrix}
= \begin{pmatrix}
6 \\
-6 \\
