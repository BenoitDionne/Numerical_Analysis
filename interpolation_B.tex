...
Let $\displaystyle \VEC{w} \in \RR^3$ be the solution of
$A\VEC{w} = \VEC{b}$, where
\[
A = \begin{pmatrix}
2 (x_2-x_0) & x_2-x_1 & 0 \\
...
Let $\displaystyle \VEC{z} \in \RR^3$ be the solution of
$A\VEC{z} = \VEC{b}$, where
\[
A = \begin{pmatrix}
2 (x_1-x_0) & x_1-x_0 & 0 \\
...
class $\displaystyle C^2$ on the nodes $a = x_0 < x_1 < \ldots < x_n=b$, then
\[
...
function $\displaystyle \phi:[a,b] \rightarrow \RR^2$.  The function $\phi$ is
called a
...
$\displaystyle \phi:[a,b]\to \RR^2$ defined by $\phi(x) = (x,f(x))$ is
...
maps of degree three $\displaystyle \phi_i:[0,1] \rightarrow \RR^2$ such
...
The mapping $\displaystyle \phi_i:[0,1]\to \RR^2$ between
...
degree $\displaystyle m \in \NNp$ for a function $f:[0,1]\rightarrow \RR$ is the
polynomial
...
\]
for $0 \leq i < n$, where $\displaystyle \VEC{c}_{i,k} \in \RR^2$ satisfies
\[
...
{Clockwise from the top left corner: $\displaystyle B_i^0$,
$\displaystyle B_i^1$ and $\displaystyle B_i^2$.}{Bsplines}
...
piecewise polynomials of degree $k$ and class $\displaystyle C^{k-1}$.
\label{BSplineCkm1}
\end{prop}
...
that they are of class $\displaystyle C^{k-1}$ requires induction and tedious
...
\begin{prop}
$\displaystyle B_i^0(t) > 0$ for $t_i \leq t < t_{i+1}$ and
$\displaystyle B_i^0(t) = 0$ otherwise.
For $k>0$, $\displaystyle B_i^k(t) > 0$ for $t_i < t < t_{i+k+1}$
and $\displaystyle B_i^k(t) = 0$ otherwise.
\label{Bsplienonneg}
\end{prop}
...
\]
for some constants $\displaystyle c_i^k$.  If we set
$\displaystyle C_i^r(t) = c_i^r$, we can use
...
\begin{proof}
We use the previous proposition with $\displaystyle C_i^k(t) = 1$ for
all $i$.  We have that
\[
...
$\displaystyle C_i^r(t) = 1$ for all $i$ and all $r$ with
$0 \leq r \leq k$.  Thus,
\[
...
We note that the only B-splines $\displaystyle B_i^k$ that are not
...
derivative of the B-splines $\displaystyle B_i^k$ that was required
for the proof of Proposition~\ref{BSplineCkm1}.

\begin{prop}
The set of B-splines
$\displaystyle \left\{ B_{-k}^k, B_{-k+1}^k, \ldots , B_{n-1}^k \right\}$
is a basis for the space $\displaystyle S_n^k$ of functions $p$ of
class $\displaystyle C^{k-1}$ on $[t_0,t_n]$ such that
...
\[
 0 = \sum_{i=-k}^{n-1} c_i B_i^k(t) = \sum_{i=j}^{n-1} c_i B_i^k(t) =
  c_j B_j^k(t) \ .
\]
Since $\displaystyle B_j^k(t) > 0$, we get $c_j = 0$.  This is a
contradiction that $c_j \neq 0$.  So, there is no such $j$ between $0$
and $n$ such $c_j \neq 0$.

In particular, this proves that $\displaystyle S_n^k$ is at least of dimension
$n+k$. 

\subI{Generating set}
We prove that all functions $\displaystyle p \in S_n^k$ can be
expressed as a linear combination over $\RR$ of the following $n+k$
functions in $\displaystyle S_n^k$:
$\displaystyle t^j$ for $0\leq j \leq k$ and
$\displaystyle H(t-t_j)(t-t_j)^k$ for
...
\[
  ...
  \end{cases}
\]
This will prove that $\displaystyle S_n^k$ is at most of dimension
$n+k$.  Combined with what we proved in the first part of the proof,
this shows that $\displaystyle S_n^k$ is of dimension $n+k$ and that\\
$\displaystyle \left\{ B_{-k}^k, B_{-k+1}^k, \ldots , B_{n-1}^k \right\}$
is a basis of $\displaystyle S_n^k$.

Given $\displaystyle p \in S_n^k$, we have that
...
most $k$.  Since $p$ is of class $\displaystyle C^{k-1}$, we have that
\[
...
$\displaystyle C^{k-1}$, we have that
\[
...
of the $n+k$ functions $\displaystyle t^j$ for $0\leq j \leq k$ and
...
only if $\displaystyle Q_{j,j} = B_{j-k-1}^k(x_j) \neq 0$ for $1\leq j \leq n+k$;
...
$\displaystyle B_i^3(x_j)$ is non null.
...
summation because $\displaystyle B_i^3(x_j) = B_i^3(t_{j-1}) = 0$ for
...
determine the $n+2$ variables $\displaystyle c_i^3$ for $-3 \leq i \leq n-2$.
\end{egg}
...
Since every element of $\displaystyle S_n^k$ is a linear combination of
$\displaystyle B_{j}^k$ for $-k \leq j <n$, we get the following result from the
previous theorem.
