...
These lecture notes cover the material normally presented in a
two-term course on numerical analysis.  We start with the basic
concept normally presented in a first course on numerical analysis.
We end with topics that are more appropriated for a first course in
numerical analysis for differential equations.

These lecture notes can be used by two different groups of students.  If the
focus is on the algorithms and the theory is ignored, then the lecture
notes can be used for an introduction to numerical analysis for engineering
and applied science students.  The lecture notes can also be used as an
...
Most of the numerical methods presented in these lecture notes are
accompanied by a code in MATLAB.

The background for these lecture notes is a two-term course in linear algebra,
...
equation for the last part of these lecture notes.

These lecture notes are divided into several parts.
...
The {\em second part} on {\bfseries polynomial interpolation}
...
The {\em third part} on {\bfseries approximation of functions} is
composed of three short chapters: Chapter~\ref{chaptApproxA} on
continuous least square approximation (i.e.\ in $\displaystyle L^2$), 
...
approximation (i.e.\ in $\displaystyle \ell^2$).
...
functional analysis to be properly covered.  To keep these lecture
notes accessible the undergraduate students (as much as possible), no
knowledge of functional analysis is assumed. 

The second and third part of these lecture notes are related and even
...
There is a strong emphasis in these lecture notes on differential equations.
This is only a reflection of the principal interest of the author.
...
are not real life applications of numerical analysis in these lecture
notes.  It would be nice (in the future) to add some realistic
projects to illustrate each topics.
...
In these lecture notes, we use the following notation for some standard
sets of numbers.
...
\begin{defn*} Let $\displaystyle f:\RR^n \rightarrow \RR$ and
