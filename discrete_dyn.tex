...
$\displaystyle f:\RR^k \rightarrow \RR^k$ with $k>1$.
...
$\displaystyle f^{-1}:\RR\rightarrow \RR$, then the
...
$\displaystyle \OO_x = \OO_x^+ \cup \OO_x^-$.
\end{defn}
...
$\displaystyle f^n(p) = p$.  If $n$ is the smallest positive integer
such that $\displaystyle f^n(p)=p$, then we say that 
...
$\displaystyle \OO_p^+$ is called a
{\bfseries periodic orbit}\index{Periodic Orbit}.  If $f$ has a
continuous inverse $\displaystyle f^{-1}:\RR\rightarrow \RR$, then
the orbit $\displaystyle \OO_p$ is called a
...
$\displaystyle W^s(p)$.  There are similar definitions for
{\bfseries backward asymptotic}\index{Backward Asymptotic} when
$f$ has a continuous inverse $\displaystyle f^{-1}:\RR\rightarrow \RR$.
\end{defn}
...
such that $\displaystyle f^i(V) \subset U$ for all $i > 0$.  Fixed
...
periodic orbit $\displaystyle \OO_p^+$ is
{\bfseries stable}\index{Stable} if $p$ is a stable
fixed point of $\displaystyle f^n$.  We say that the periodic orbit is
{\bfseries asymptotically stable}\index{Asymptotically Stable} if $p$
is an asymptotically stable fixed point of $\displaystyle f^n$.
\end{defn}

\begin{rmk}
If $f:\RR\to \RR$ is a continuous function with a continuous inverse
$\displaystyle f^{-1}:\RR\to \RR$, then the previous definition of
...
for $\displaystyle f^n$ and let $\displaystyle q=f^k(p)$ be another
point on the orbit $\displaystyle \OO_p^+$. 
If $U_q$ is an open neighbourhood of $q$, then
$\displaystyle f^{-k}(U_q)$ is an open neighbourhood of $p$.  Since
$p$ is a stable fixed point for $\displaystyle f^n$, there exists an
open neighbourhood $V_p$ of $p$ such that
$\displaystyle f^{ni}(V_q) \subset f^{-k}(U_q)$ for all $i>0$. 
Hence $\displaystyle f^{ni}(f^k(V_p)) = f^{ni+k}(V_q) \subset U_q$ for all $i>0$,
where $\displaystyle f^k(V_p)$ is an open neighbourhood of $q$.  This
proves that $q$ is a stable fixed point of $\displaystyle f^n$.

Suppose furthermore that $p$ is an asymptotically stable fixed point
of $\displaystyle f^n$.  Then there exists an open neighbourhood $W_p$
of $p$ such that $\displaystyle \lim_{i\to \infty}f^{ni}(x) = p$ for
all $x \in W_p$.  Thus
$\displaystyle \lim_{i\to \infty}f^{ni}(f^k(x)) = f^{k}(p) = q$
for all $x \in W_p$; namely,
$\displaystyle \lim_{i\to \infty}f^{ni}(y) = q$ for all
$y$ in the open neighbourhood $\displaystyle f^k(W_p)$ of $q$.  This proves that
$q$ is an asymptotically stable fixed point of $\displaystyle f^n$.
\end{rmk}
...
This implies that $\displaystyle f^j(x) \not\in I$ for infinitely many
...
$\displaystyle p_\mu  = \frac{\mu-1}{\mu}$ is a sink
...
stability of the fixed point $p$ of $\displaystyle f^n$.
Proposition~\ref{fp_stability} holds for a periodic point $p$ of period $n$ if
$f$ is replaced by $\displaystyle f^n$.
...
$\displaystyle x_{i+1} = f_\mu^2(x_i) = f_\mu(f_\mu(x_i))$ for
$i=0$, $1$, $2$, \ldots

\mathF{discrete_dyn/logistic3}{7cm}{Period Two}{Period Two}{figPT}

From the graph of $\displaystyle f_\mu^2$ given in Figure~\ref{figPT},
...
Consider a nice function $\displaystyle f:\RR^2 \rightarrow \RR$.  It defines a
...
Let $\displaystyle f:\RR^2 \rightarrow \RR$ be a smooth function and let
...
\bigg|_{\mu=\mu_0,x=0} \not=0$,
\end{enumerate}
where $\displaystyle f^2_\mu(x) \equiv f(f(x,\mu),\mu)$.  Then, there exist
an interval $I$ about $0$ and a mapping $q:I\rightarrow \RR$ such that
$q(0) = \mu_0$ and $f_{q(x)}(x) \neq x$ but $\displaystyle f^2_{q(x)}(x) = x$
for $x \in I$.
\end{theorem}
...
$\displaystyle \OO^+ \subset [0,1]$ of $f_\mu$ is of period $n$ with
$n$ very large, it is reasonable to expect that
$\displaystyle \OO^+$ will ``almost cover'' some segments of the
...
$\displaystyle \lim_{j\rightarrow \infty} d_j/d_{j+1} =
...
j > n \right\} \; .
\]
It can be shown that $A_n$ consists of $\displaystyle 2^n$ distinct
...
$\displaystyle \left| f^k(x) - f^k(y) \right| > \delta$.
\end{defn}
