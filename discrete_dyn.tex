...
complexity is even greater if
$\DS f:\RR^k \rightarrow \RR^k$ with $k>1$.
...
\right\} \  .
\]
If $f$ has a continuous inverse
$\DS f^{-1}:\RR\rightarrow \RR$, then the
...
$\DS \OO_x = \OO_x^+ \cup \OO_x^-$.
\end{defn}

\begin{defn}
Consider a continuous function $f:\RR \rightarrow \RR$.  The point
$p \in \RR$ is a {\bfseries periodic point}\index{Periodic Point} of
$f$ if there exists a positive integer $n$ such that
$\DS f^n(p) = p$.  If $n$ is the smallest positive integer
such that $\DS f^n(p)=p$, then we say that 
$p$ is of {\bfseries period}\index{Period} $n$.  When $n=1$, the point
$p$ is called a {\bfseries fixed point}\index{Fixed Point}.
We denote by $\Per_n(f)$ the set of all periodic points of $f$ of
period $n$.  In particular, $\Fix(f) = \Per_1(f)$ is the set of fixed
points of $f$.  If $p$ is a periodic point of $f$, then the forward orbit
$\DS \OO_p^+$ is called a
{\bfseries periodic orbit}\index{Periodic Orbit}.  If $f$ has a
continuous inverse $\DS f^{-1}:\RR\rightarrow \RR$, then
the orbit $\DS \OO_p$ is called a
{\bfseries periodic orbit}\index{Periodic Orbit}.
\end{defn}

\begin{egg}
Consider the {\bfseries logistic mapping}\index{Logistic Mapping}
$f_\mu:[0,1] \to \RR$ defined by
\[
f_\mu(x) = \mu x(1-x)
\]
for $0\leq x \leq 1$.  For $0 \leq \mu \leq 4$, we have that
$f_\mu:[0,1]\rightarrow [0,1]$.  Moreover, $f_\mu$ has two fixed
points: $p_0 = 0$ and
$\DS p_{\mu} = \frac{\mu-1}{\mu}$ for $\mu > 0$.

For $\mu=3.4$, we have that
$p_{3.4} = 0.45195878844045\ldots$ is a periodic point of
...
For $\mu=3.5$, we have that
$p_{3.5} = 0.87499726360246\ldots$ is a periodic point of
$f_\mu$ of period $4$.  The orbit of period four is
\[
\begin{split}
\big\{ & 0.87499726360246, 0.38281968301732, 0.82694070659144,
  0.50088421030722, \\
& 0.87499726360246, \ldots \big\} \ ,
\end{split}
\]
where the values have been chopped
...
\]
The set of all points forward asymptotic to $p$ is denoted by
$\DS W^s(p)$.  There are similar definitions for
{\bfseries backward asymptotic}\index{Backward Asymptotic} when
$f$ has a continuous inverse $\DS f^{-1}:\RR\rightarrow \RR$.
\end{defn}
...
such that $\DS f^i(V) \subset U$ for all $i > 0$.  Fixed
points that are not stable are called
{\bfseries unstable}\index{Unstable}.   A fixed point $p$ of
$f$ is {\bfseries asymptotically stable}\index{Asymptotically Stable} if it is
stable and there exists an open neighbourhood $W$ of $p$ such that
$\DS \lim_{i\rightarrow \infty} f^{i}(x) = p$ for all
$x\in W$.

If $p$ is a period point of period $n$ for $f$, we say that the
periodic orbit $\DS \OO_p^+$ is
{\bfseries stable}\index{Stable} if $p$ is a stable
fixed point of $\DS f^n$.  We say that the periodic orbit is
{\bfseries asymptotically stable}\index{Asymptotically Stable} if $p$
is an asymptotically stable fixed point of $\DS f^n$.
\end{defn}

\begin{rmk}
If $f:\RR\to \RR$ is a continuous function with a continuous inverse
$\DS f^{-1}:\RR\to \RR$, then the previous definition of
stability and asymptotic stability for a period orbit is independent
of the point $p$ of the orbit used to determine the stability or the
asymptotic stability \footnote{The result is also true if
$f:\RR\to \RR$ is differentiable and $f'(p) \neq 0$.  The proof is
similar but a little bit more technical.}.

Suppose that $p$ is a stable periodic point of period $n>1$
for $\DS f^n$ and let $\DS q=f^k(p)$ be another
point on the orbit $\DS \OO_p^+$. 
If $U_q$ is an open neighbourhood of $q$, then
$\DS f^{-k}(U_q)$ is an open neighbourhood of $p$.  Since
$p$ is a stable fixed point for $\DS f^n$, there exists an
open neighbourhood $V_p$ of $p$ such that
$\DS f^{ni}(V_q) \subset f^{-k}(U_q)$ for all $i>0$. 
Hence $\DS f^{ni}(f^k(V_p)) = f^{ni+k}(V_q) \subset U_q$ for all $i>0$,
where $\DS f^k(V_p)$ is an open neighbourhood of $q$.  This
proves that $q$ is a stable fixed point of $\DS f^n$.

Suppose furthermore that $p$ is an asymptotically stable fixed point
of $\DS f^n$.  Then there exists an open neighbourhood $W_p$
of $p$ such that $\DS \lim_{i\to \infty}f^{ni}(x) = p$ for
all $x \in W_p$.  Thus
$\DS \lim_{i\to \infty}f^{ni}(f^k(x)) = f^{k}(p) = q$
for all $x \in W_p$; namely,
$\DS \lim_{i\to \infty}f^{ni}(y) = q$ for all
$y$ in the open neighbourhood $\DS f^k(W_p)$ of $q$.  This proves that
$q$ is an asymptotically stable fixed point of $\DS f^n$.
\end{rmk}
...
interval $I$ containing $p$ such that, for each $x \in I$ with
$x \neq p$, one can find $j \in \NN$ such that $f^j(x) \not\in I$.
This implies that $\DS f^j(x) \not\in I$ for infinitely many
values of $j$. \label{fp_stability}
\end{prop}
...
\begin{egg}
For the logistic mapping $f_\mu$ defined by $f_\mu(x) = \mu x(1-x)$,
the origin is a source if $\mu>1$, and
$\DS p_\mu  = \frac{\mu-1}{\mu}$ is a sink
if $1<\mu<3$.  This follows from
$\DS \pdfdx{f_\mu(x)}{x}\bigg|_{x=0} = \mu$ and
$\DS \pdfdx{f_\mu(x)}{x}\bigg|_{x=p_\mu} = 2-\mu$.
\label{EggLogMap2}
\end{egg}
...
if $\DS \left| \dydx{f^n}{x}(p) \right| \neq 1$.
\end{defn}

A periodic point $p$ of $f$ of period $n$ is a fixed point of
$\DS f^n$.
The stability of the periodic point $p$ of $f$ is determined by the
stability of the fixed point $p$ of $\DS f^n$.
Proposition~\ref{fp_stability} holds for a periodic point $p$ of period $n$ if
$f$ is replaced by $\DS f^n$.

\begin{egg}
To study the stability of the periodic points of period $2$ for the
logistic mapping $f_\mu$ with $\mu = 3.4$, we consider the iterative system
$\DS x_{i+1} = f_\mu^2(x_i) = f_\mu(f_\mu(x_i))$ for
$i=0$, $1$, $2$, \ldots

\mathF{discrete_dyn/logistic3}{7cm}{Period Two}{Period Two}{figPT}

From the graph of $\DS f_\mu^2$ given in Figure~\ref{figPT},
we see that the periodic point\\ $0.45195878844045\ldots$ of period
$2$ is a sink.  \label{EggLogMap3}
\end{egg}

\subsection{Bifurcation}

Consider a nice function $\DS f:\RR^2 \rightarrow \RR$.  It defines a
...
\begin{theorem} \label{IFTforDS}
Let $\DS f:\RR^2 \rightarrow \RR$ be a smooth function and let
...
$\DS \left| \pdfdx{f_{\mu}(x)}{x}\right| = 1$.  We use the
...
\begin{enumerate}
\item $\DS f_{\mu_0}(0)=0$\ ,
\item $\DS \pdfdx{f_{\mu}(x)}{x}\bigg|_{\mu=\mu_0,x=0} = 1$ \ ,
\item $\DS \pdfdxn{f_{\mu}(x)}{x}{2}\bigg|_{\mu=\mu_0,x=0} \neq 0$
\quad and
\item $\DS \pdfdx{f_\mu(x)}{\mu}\bigg|_{\mu=\mu_0,x=0} \neq 0$ \ .
\end{enumerate}
Then, there exist an interval $I$ about $0$ and a mapping
$q:I\rightarrow \RR$ such that $q(0) = \mu_0$ and $f_{q(x)}(x) = x$
for $x \in I$.  Moreover, $q'(0) = 0$ and $q''(0) \neq 0$.
\end{theorem}
...
sinks.  However,
\[
q''(0) = \frac{\DS -\pdfdxn{f_{\mu}(x)}{x}{2}\bigg|_{\mu=\mu_0,x=0}}
{\DS \pdfdx{f_{\mu}(x)}{\mu}\bigg|_{\mu=\mu_0,x=0}}
\]
...
\pdfF{discrete_dyn/saddle_node}{Fold bifurcation of discrete mappings}
{A typical fold bifurcation diagram for a discrete mapping.}{figSNBDM}

\begin{theorem}[Period doubling or flip bifurcation]  \label{ThePDTforM}
Let $\DS f:\RR^2 \rightarrow \RR$ be a smooth function and let
$f_\mu(x)= f(x,\mu)$.  Suppose that
\begin{enumerate}
...
and
\item $\DS \pdfdxnm{f_{\mu}^2(x)}{\mu}{x}{2}{}{}
\bigg|_{\mu=\mu_0,x=0} \not=0$,
\end{enumerate}
where $\DS f^2_\mu(x) \equiv f(f(x,\mu),\mu)$.  Then, there exist
an interval $I$ about $0$ and a mapping $q:I\rightarrow \RR$ such that
$q(0) = \mu_0$ and $f_{q(x)}(x) \neq x$ but $\DS f^2_{q(x)}(x) = x$
for $x \in I$.
\end{theorem}
...
points of period two are represented by the curve.   For $\mu > \mu_0$ fixed,
there is a periodic orbit of period two where the iterations alternate
between the lower and upper branches of the curve.  The fixed point
$x=0$ on the dashed straight line is a source.  For $\mu < \mu_0$,
there are no periodic orbit of period $2$ and the fixed point $x=0$ on
the continuous straight line is a sink.  Moreover,
\[
  q''(0) = \frac{\DS
\left(\pdfdxn{f_{\mu}(x)}{x}{2}\bigg|_{\mu=\mu_0,x=0}\right)^2
+\frac{2}{3} \pdfdxn{f_{\mu}(x)}{x}{3}\bigg|_{\mu=\mu_0,x=0}}
{\DS \pdfdxnm{f^2_{\mu}(x)}{x}{\mu}{2}{}{}\bigg|_{\mu=\mu_0,x=0}}
\]
...
discrete mappings}{A typical period doubling bifurcation diagram for a
discrete mapping.}{figPDB}

\begin{rmk}
In the previous theorem, the condition $f_{\mu}(0)=0$ for all $\mu$
near $\mu_0$ is not necessary.  Suppose that
\begin{enumerate}
\item $\DS f_{\mu_0}(x_0) = x_0$\ ,
\item $\DS \pdfdx{f_{\mu}(x)}{x}\bigg|_{\mu=\mu_0,x=x_0} = -1$\ ,
\item $\DS \frac{1}{2}
...
$p$ defined in an open interval $J$ about $\mu_0$ such that
...
&\quad = \frac{1}{2} \left(\pdydxn{f}{x}{2}(0+p(\mu_0),\mu_0) \right)^2
...
= \pdfdx{\left( \pdydx{f}{x}(f(x+p(\mu),\mu),\mu)
...
this discrete dynamical systems may have, we now present some results
about the logistic mapping $f_\mu$ without giving the proofs.  A good
reference is \cite{D}.  Most of the results about the logistic mapping
...
{Bifurcation diagram for the logistic mapping}
{Bifurcation diagram for the logistic mapping.}{figBDLM}
...
$\DS \OO^+ \subset [0,1]$ of $f_\mu$ is of period $n$ with
$n$ very large, it is reasonable to expect that
$\DS \OO^+$ will ``almost cover'' some segments of the
interval $[0,1]$.  This explains the shaded area. Figure~\ref{figLPO}
illustrates the attracting periodic orbit for $\mu=3.6$.  The
corresponding histogram with 300 subintervals and 10 millions
iterations is given in Figure~\ref{figHLPO}.

\mathF{discrete_dyn/logistic8}{7cm}{Cobweb of a periodic orbit with a large
period}{Cobweb of a periodic orbit of the logistic mapping $f_{\mu}$
for $\mu = 3.6$. The period of this stable periodic orbit is very
large.}{figLPO}

\mathF{discrete_dyn/histogram2}{7cm}{Histogram of a periodic orbit with a
large period}{Histogram of a periodic orbit of the logistic mapping
$f_{\mu}$ for $\mu = 3.6$.  The period of this stable periodic orbit
is very large.}{figHLPO}

Period doubling accumulates to $\mu = 3.5699456\ldots$.  This number
is known as the {\bfseries Feigenbaum point}\index{Feigenbaum Point}.
Moreover, let $\mu_0=3$, $\mu_1=3.236\ldots$, $\mu_2 =\ldots$, and so
on be the values of $\mu$ for which the logistic mapping undergoes
period doubling and let $d_j = \mu_{j+1}-\mu_j$ for $j \geq 0$.
It has been shown that
$\DS \lim_{j\rightarrow \infty} d_j/d_{j+1} =
...
$x_{n+1} = g_\mu(x_n)$ where the graph of $g_\mu$ looks like the
graph of $f_\mu$.
...
prime number.  If $f$ has a periodic orbit of period $k$, then $f$ has
a periodic orbit of period $m$ for all $m \ll k$.
\end{theorem}

\begin{egg}
For $\mu = 3.839\ldots$, we have that
\[  
\left\{0.14988539433432, 0.48917380192271, 0.95930024021836,
  0.14988539433432, \ldots \right\}
\]
...
j > n \right\} \; .
\]
It can be shown that $A_n$ consists of $\DS 2^n$ distinct
open subintervals of $[0,1]$.  We have that any iteration that starts
with $x_0 \in A_0$ (i.e.\ such that $f_\mu(x_0)>1$) eventually
converges to $-\infty$ (Figure~\ref{figA0}).  We also have that any
iteration that starts with $x_0 \in A_1$ (i.e. such that
$f_\mu(x_0) \in A_0$) eventually
...
\]
$\Delta_\mu$ contains all the points $x$ such that
$\DS f_\mu^j(x)$ stay
...
\begin{defn}
Let $I$ be a closed subset of $\RR$ and $f:I\rightarrow I$ be a
continuous function.  The function $f$ is
...
\begin{defn}
Let $I$ be a closed subset of $\RR$ and $f:I\rightarrow I$ be a
continuous function.  The function $f$ has
...
$\DS \left| f^k(x) - f^k(y) \right| > \delta$.
\end{defn}

\begin{defn}[Chaos]
Let $I$ be a closed subset of $\RR$ and $f:I\rightarrow I$ be a
continuous function.  The function $f$ is said to be
...
\begin{egg}
The logistic mapping $f_{\mu}: \Delta_\mu\to \Delta_\mu$ is chaotic
for $\mu > 4$.  It can also be proved that $f_\mu$ has a dense orbit in
$\Delta_\mu$.
\end{egg}
...
$\DS \left| \dydx{f^n}{x}(x)  \right| < 1$
(respectively $>1$) for all $n \geq N$ and $x \in V$.
\end{enumerate}
\end{defn}

\begin{egg}
It can be proved that $\Delta_\mu$ is a repelling hyperbolic set for
the logistic mapping $f_\mu$ for $\mu>2+\sqrt{5}$. 
\end{egg}
