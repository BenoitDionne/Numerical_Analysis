...
\begin{rmk}
Three important norms on $\displaystyle V=\RR^n$ are the
{\bfseries Euclidean or $\displaystyle \mathbf{\ell^2}$
norm}\index{Norms!Euclidean or $\mathbf \ell^2$}
\[
N(\VEC{x}) \equiv \|\VEC{x} \|_2 = \sqrt{\sum_{i=1}^{n} x_i^2} \ ,
\]
the {\bfseries maximum or $\displaystyle \mathbf{\ell^\infty}$
norm}\index{Norms!Maximum or $\mathbf{\ell^\infty}$} 
\[
N(\VEC{x}) \equiv \| \VEC{x} \|_{\infty} = \max_{1\leq i \leq n} |x_i|
\]
and the {\bfseries $\displaystyle \mathbf{\ell^1}$
...
\begin{defn}
Let $\|\cdot\|$ be any norm on $\displaystyle \RR^n$.  The
{\bfseries distance}\index{Distance}
between two vectors $\VEC{x}$ and $\VEC{y}$ in $\displaystyle \RR^n$,
...
$\displaystyle \{\VEC{x}_k\}_{k=1}^\infty$ in
$\displaystyle \RR^n$ {\bfseries converges}\index{convergence} to a
vector $\VEC{p}$ in $\displaystyle \RR^n$ if
$\displaystyle \lim_{k\to \infty} \| \VEC{x}_k - \VEC{p} \| = 0$.
\end{defn}
...
\]
for all vector $\displaystyle \VEC{x} \in \RR^n$.
...
$\displaystyle \{x_{k,j}\}_{k=0}^\infty$
converges to $x_j$ for $1\leq j \leq n$, where $x_j$ is the
$\displaystyle j^{th}$ component of the vector $\VEC{x}$ and $x_{k,j}$ is the
$\displaystyle j^{th}$ component of the vector $\VEC{x}_k$.
\end{enumerate}
\end{rmkList}

\begin{defn}
Let $\|\cdot\|$ be any norm on $\displaystyle \RR^n$ and
$A$ be an \nn matrix.  The
...
$\displaystyle \RR^{n^2}$ and so is of finite dimension.
\item It is easy to see that $\|A\VEC{x}\| \leq \|A\|\|\VEC{x}\|$ for
all $\displaystyle \VEC{x} \in \RR^n$.  This shows that the mapping
$\displaystyle \phi:\RR^n \to \RR^n$ defined by
$\phi(\VEC{x}) = A\VEC{x}$ for all
$\VEC{x}$ is a continuous mapping. \label{MatrNormP1}
\item Since $S = \{ \VEC{x} : \|\VEC{x}\| = 1\}$ is a compact subset of
$\displaystyle \RR^n$, the continuous mapping $\phi$ defined in the
...
\begin{proof}
For $\displaystyle \VEC{x} \in \RR^n$ satisfying
...
\]
Define $\displaystyle \VEC{x} \in \RR^n$ by
\[
x_j = \begin{cases}
...
\begin{rmk}
If $A$ is an \nn matrix, let $\displaystyle A^\ast$ be the transpose complex
...
\end{cases}
\]
Since $\displaystyle u_{i,j} = \epsilon^{i-j} t_{i,j} \rightarrow 0$
...
$\displaystyle Q_\epsilon A Q_\epsilon^{-1} = D + S_\epsilon$, where $D$ is a
...
defines a norm on $\displaystyle \RR^n$.  The induced norm of $A$ with
...
\item [(ii)] $\rho (A) < 1$.
\item [(iii)] Given any $\displaystyle \VEC{x} \in \RR^n$, the sequence
$\displaystyle \left\{ A^k \VEC{x}\right\}_{k=0}^\infty$ converges to
$\displaystyle \VEC{0} \in \RR^n$.
\end{itemize}
\end{theorem}
...
\]
as $k\rightarrow \infty$ for all $\displaystyle \VEC{x}\in \RR^n$.
...
Given a vector $\displaystyle \VEC{x}_{0} \in \RR^n$, the goal is to generate a
sequence $\displaystyle \{\VEC{x}_{k}\}_{k=1}^\infty$ that converges to
...
$\displaystyle \VEC{x}_{0} \in \RR^n$, the goal is to generate a sequence
$\displaystyle \{\VEC{x}_{k}\}_{k=1}^\infty$ that converges to the
...
$\displaystyle \{x_k\}_{k=0}^\infty$ that converges faster to the solution of
...
can be rewritten as
$\displaystyle \VEC{x}_{k+1} = D^{-1}(L + U) \VEC{x}_{k} + D^{-1}\VEC{b}$.
We have that $\VEC{x}$ is a solution of
$\displaystyle \VEC{x} = D^{-1}(L + U) \VEC{x} + D^{-1}\VEC{b}$ if and only if
...
$\displaystyle \VEC{x}_{k+1} = (D-L)^{-1} U \VEC{x}_{k} + (D-L)^{-1}\VEC{b}$.
Again, $\VEC{x}$ is a solution of
$\displaystyle \VEC{x} = (D-L)^{-1} U \VEC{x} + (D-L)^{-1}\VEC{b}$
...
For the Jacobi iterative method, $\displaystyle T = D^{-1}(L + U)$ and
$\displaystyle \VEC{c} = D^{-1}\VEC{b}$.  For the Gauss-Seidel iterative method,
$\displaystyle T = (D-L)^{-1} U$ and
$\displaystyle \VEC{c} = (D-L)^{-1}\VEC{b}$.
...
\begin{proof}
Let $\displaystyle S_k = \Id_n + T + T^2 + \ldots + T^k$.  We have
...
\begin{proof}
From $\displaystyle \Id_n=(\Id_n-T)(\Id_n-T)^{-1}$, we get
\begin{align*}
...
From $\displaystyle (\Id_n-T)^{-1} = \Id_n + T (\Id_n - T)^{-1}$, we get
\[
...
Since $\displaystyle P^{-1}$ is invertible, we get the $Q$ is invertible.

Moreover, we get from Corollary~\ref{Banach_cor} with
$\displaystyle T=(P-Q)P^{-1}$ that
\[
...
$\displaystyle \{\VEC{x}_{k}\}_{k=0}^\infty$ defined by
(\ref{formula7}) converges for all $\displaystyle \VEC{x}_{0} \in \RR^n$ to the
...
has a unique solution given by $\displaystyle \VEC{p} = (\Id_n-T)^{-1}\VEC{c}$.
...
\stage{$\Rightarrow$} Let $\VEC{x}_{0}$ be any vector in $\displaystyle \RR^n$.
We show by induction that
\[
...
= \VEC{0}
\]
because $\displaystyle \{\VEC{x}_{k}\}_{k=0}^\infty$ converges to $\VEC{p}$
by hypothesis.  Since $\VEC{p} - \VEC{x}_{0}$ can be any vector in
$\displaystyle \RR^n$ by the arbitrary status of $\VEC{x}_0$, we get that
...
\]
because $\displaystyle \{\VEC{x}_{m}\}_{m=0}^\infty$ converges to
...
generate sequences $\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$ which
...
the form (\ref{formula7}), where $\displaystyle T = D^{-1}(L + U)$ and
$\displaystyle \VEC{c} = D^{-1}\VEC{b}$.
...
the form (\ref{formula7}), where $\displaystyle T = (D-L)^{-1} U$ and
$\displaystyle \VEC{c} = (D-L)^{-1}\VEC{b}$.
...
\]
Finally, multiplying by $\displaystyle (D-\omega L)^{-1}$ from the left on both
...
With $\displaystyle T = (D-\omega L)^{-1}((1-\omega)D + \omega U)$ and
$\displaystyle \VEC{c} = \omega (D-\omega L)^{-1}\VEC{b}$, the equation
(\ref{rmlongform}) becomes $\VEC{x} = T\VEC{x} + \VEC{c}$.  If the
matrix $T$ satisfies Theorem~\ref{convth}, the sequence
$\displaystyle \{\VEC{x}_{k}\}_{k=1}^\infty$ defined by
...
\]
for all non-zero vector $\displaystyle \VEC{x} \in \CC^n$, where
...
$\displaystyle \RR^n$, then the relaxation method given by (\ref{formula8})
...
$\displaystyle T = (D-\omega L)^{-1}((1-\omega)D + \omega U)$.

Let $\lambda \in \CC$ be an eigenvalue of $T$ and
$\displaystyle \VEC{x} \in \CC^n$
...
\]
we get that $\displaystyle T = \Id - Q^{-1}A$, where
$\displaystyle Q = \frac{1}{\omega}(D-\omega L)$.
Hence, $\displaystyle \Id - T = Q^{-1}A$ and
$\displaystyle (D-\omega L)Q^{-1} \VEC{y} = \omega \VEC{y}$ for all $\VEC{y}$.
We get
\[
...
\end{equation}
Moreover, from $\displaystyle T = \Id - Q^{-1}A$, we also have that
$Q(\Id - T) = A$.  Hence
\[
(1-\lambda) QT\VEC{x}
...
\ps{\VEC{x}}{A\VEC{x}} \ .
\end{equation}
Since $\displaystyle A = A^\ast$, we have that
...
\ .
\]
Since $2-\omega > 0$, $\displaystyle |1-\lambda|^2>0$,
...
$\displaystyle T_j = D^{-1}(L+U)$ (Jacobi iterative method) and
$\displaystyle T_{gs} = (D-L)^{-1}U$ (Gauss-Seidel iterative method).  Then
$\displaystyle \rho(T_{gs}) = (\rho(T_j))^2 < 1$ and
$\displaystyle \omega = 2/(1+\sqrt{1-\rho(T_{gs})})
...
$\displaystyle A=D-C-C^\ast$, where $D$ is strictly positive definite.
...
For the standard scalar product on $\displaystyle \RR^n$,
\[
...
$\displaystyle t^2$ in $q(t)$ is positive, we have a quadratic
...
$\displaystyle \VEC{u} \in \RR^n$.
The only vector orthogonal to all vectors in
$\displaystyle \RR^n$, in particular to itself, is $\VEC{0}$.
...
vectors in $\displaystyle \RR^n$; namely, $\ps{\VEC{u}_i}{A\VEC{u}_j} = 0$ for
...
$\displaystyle \RR^n$, the previous equations shows that
$A \VEC{x}_{n+1} = \VEC{b}$.
\end{proof}
...
\item Stop if
$\displaystyle \| \VEC{r}_{k+1} \|_2^2 < \epsilon$, where $\epsilon$ is given.
\item Compute
\[
s_k =
...
Since $\displaystyle A\VEC{u}_0 = t_0^{-1}(\VEC{r}_0 - \VEC{r}_1)$,
...
where $\displaystyle \tilde{A} = T^\top A T$,
$\displaystyle \tildev{x} = T^{-1}\VEC{x}$ and
$\displaystyle \tildev{b} = T^\top \VEC{b}$ for an invertible matrix $T$.
...
$\displaystyle \tilde{A} =T^\top A T$ and
$\displaystyle \tildev{b} = T^{\top}\VEC{b}$.

To compare the conjugate gradient algorithm applied to both systems
$A\VEC{x} = \VEC{b}$ and $\tilde{A} \tildev{x} = \tildev{b}$, we let
$\displaystyle \tildev{x}_k = T^{-1}\VEC{x}_k$ and
$\displaystyle \tildev{u}_k = T^{-1}\VEC{u}_k$.
...
$\displaystyle T\,T^\top$ is an invertible matrix.  If
$\displaystyle Q^{-1} = T\,T^\top$, then
$\displaystyle Q^{-1}$ is a strictly positive definite matrix.
...
$\displaystyle T^{-1}\VEC{x}_{k+1} = T^{-1}\VEC{x}_k + \tilde{t}_k
T^{-1} \VEC{u}_k$.
...
we get
$\displaystyle T^\top\VEC{r}_{k+1} = T^\top\VEC{r}_k
+ \tilde{t}_k (T^\top A T)(T^{-1} \VEC{u}_k)$.  Multiplying both sides of
this equality by $\displaystyle (T^{-1})^\top$ from the left, we get
...
$\displaystyle T^{-1}\VEC{u}_{k+1} = T^\top \VEC{r}_{k+1} +
...
given, compute $\displaystyle \| \VEC{r}_{k+1} \|_2^2$.  Stop if this last
...
defines a norm on $\displaystyle \RR^n$.
\end{question}
...
null, $\displaystyle 2^{-i}|x_i| = 0$ for all $i$ and therefore $x_i =
0$ for all $i$.
...
\end{align*}
\item For any $\VEC{x}$ and $\VEC{y}$ in $\displaystyle \RR^n$, we have
\begin{align*}
\|\VEC{x}+\VEC{y}\|
...
\begin{question}
If $\|\cdot\|$ is a norm on $\displaystyle \RR^n$, show that
...
\begin{question}
Let $\|\cdot\|$ be a norm on $\displaystyle \RR^n$.  Show that the
...
\sol{\SOL}{
Let $\VEC{x}$ be a vector of $\displaystyle \ell^1$-norm $1$; namely,
...
\end{equation}
we prove that there exists $\displaystyle \VEC{x} \in \RR^n$ of
$\displaystyle \ell^1$-norm $1$
...
$\displaystyle \ell^\infty$-norm.
}

\begin{question}
Let $\|\cdot\|$ be a norm on $\displaystyle \RR^n$ and $A$ be an \nn matrix.
Prove that $\|A\VEC{x} \| \leq \|A\| \, \|\VEC{x}\|$ for all
$\displaystyle \VEC{x} \in \RR^n$.  Moreover, prove that $\|A\|$ is the smallest
...
$\displaystyle \VEC{x} \in \RR^n$.  Since
$\|A\VEC{x} \| \leq C$ for all $\displaystyle \VEC{x} \in \RR^n$ such that
$\|\VEC{x}\|=1$, we get that
...
$\displaystyle 10^{-5}$ if the
infinite norm is used.  Start with
$\displaystyle \VEC{x}_0 = \VEC{0} \in \RR^3$.
\end{question}
...
$\displaystyle \|\VEC{x}_n-\VEC{x}_{n-1}\| < 10^{-5}$ is for $n=10$.  We get
$\displaystyle
...
system of forces to within $\displaystyle 10^{-3}$.\\
...
of forces to within $\displaystyle 10^{-3}$.
...
\begin{enumerate}
\item For the Jacobi iterative method, $\displaystyle T=D^{-1}(L+U)$ and the
...
\item For the Gauss-Seidel iterative method,
  $\displaystyle T=(D-L)^{-1}U$ and the
...
$\displaystyle \VEC{x}_0 = (1 \ 1 \ 1 \ 1 \ 1 \ 1 \ 1 \ 1)^\top$.
...
$A\VEC{x} = \VEC{b}$ with an accuracy of $\displaystyle 10^{-5}$.\\
...
$A\VEC{x} = \VEC{b}$ with an accuracy of $\displaystyle 10^{-5}$.  You
...
$\displaystyle \VEC{x}_0 = \begin{pmatrix} 1 & 1 & 1 & 1 \end{pmatrix}^\top$.
...
$\displaystyle T = (D-\omega L)^{-1}((1-\omega)D + \omega U)$ and 
$\displaystyle \VEC{c} = \omega (D-\omega L)^{-1}\VEC{b}$ with
\[
...
$\displaystyle \{ \VEC{x}_k \}_{k=0}^\infty$ does not converge to a solution of
...
However, $\displaystyle \{ \lambda^k \VEC{u} \}_{k=0}^\infty$ does not
converge to $\VEC{0}$.  If $|\lambda| > 1$, we have
that $\displaystyle \|\lambda^k \VEC{u}\| = |\lambda|^k \|\VEC{u}\| \to \infty$
as $k \to \infty$.  If $|\lambda| = 1$, we have
that $\displaystyle \|\lambda^k \VEC{u}\| = \|\VEC{u}\| > 0$ for all
$k$.  Again, $\displaystyle \|\lambda^k \VEC{u}\| \not\to 0$ as $k\to \infty$.
...
$\VEC{p} = T\VEC{p} - \VEC{c}$, then $\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$
...
$\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$ does not converge for all
$\VEC{x}_0$.
}
...
$\displaystyle \VEC{b}\in \RR^n$, show that a finite number of
...
where $\displaystyle \VEC{c}=D^{-1}\VEC{b}$ and
$\displaystyle T=D^{-1}(L+U)=D^{-1}U$ is
...
upper-triangular matrix satisfies $\displaystyle T^n = 0$.  Thus
\[
...
\begin{question}
Let $A$ be an \nn upper-triangular matrix and
$\displaystyle \VEC{b}\in \RR^n$, show
...
where $\displaystyle \VEC{c}=(D-L)^{-1}\VEC{b}= D^{-1}\VEC{b}$ and
$\displaystyle T=(D-L)^{-1}U=D^{-1}U$ is a strictly upper-triangular
...
and $\VEC{x}_0$ is any vector in $\displaystyle \RR^2$.\\
\subQ{a} Show that the sequence
$\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$ generated by the relaxation method
...
$\VEC{x}_0$ for which the sequence $\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$
...
$\displaystyle T=(D-\omega\,L)^{-1}\left((1-\omega)D + \omega U\right)$ and
$\displaystyle \VEC{c} = \omega(D-\omega\, L)^{-1} \VEC{b}$.  Since
\begin{align*}
...
$\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$ generated by the relaxation
...
the sequence $\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$ does not converge
for all
$\VEC{x}_0$ if there is no solution to $\VEC{x} = T \VEC{x} + \VEC{c}$.
Moreover, if there is a solution $\VEC{p}$ to $\VEC{x} = T \VEC{x} + \VEC{c}$,
then the sequence $\displaystyle \{\VEC{x}_k\}_{k=0}^\infty$ does not
...
$\displaystyle \VEC{e} = A^{-1}\VEC{b} - \VEC{x}$ is positive unless
$A\VEC{x} = \VEC{b}$.
\end{question}
...
> 0
\]
as long as $\displaystyle A^{-1}\VEC{b} - \VEC{x} \neq \VEC{0}$;
