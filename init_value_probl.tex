...
$\displaystyle y''(\xi_i)h^2/2$ is much smaller than
...
Discretization Error} for the Euler's method is $\displaystyle y''(\xi_i)h^2/2$.
\end{enumerate}
\end{rmkList}
...
easy to find the exact solution $\displaystyle y(t) = e^{0.1t^2 -0.1}$ of
...
easy to find the exact solution $\displaystyle y(t) = e^{t^2 -1}$ of
...
e^{L(t_i-t_0)} - 1 \right) + |\delta_0|\,e^{L(t_i-t_0)} \ ,
\end{align*}
where we have used $\displaystyle (1+x)^n \leq e^{nx}$ for $x > 0$ in the second
...
\begin{rmk} Since $\displaystyle |h^p|$ is a decreasing function of
...
that $\displaystyle |y^{(n+1)}(t)| \leq M$ for all $t\in [t_0,t_f]$, then
$\displaystyle |\tau_{i+1}(h)| \leq \tau(h) \equiv M|h|^n/(n+1)!$ for
all $i$ and $\displaystyle \tau(h)  = O(h^n)$ near the origin.  This
...
order two.  Up to $\displaystyle O(h^2)$, we replace the function
$\phi(t,y)$ in the
...
Taylor method of order $2$) such that $\displaystyle \tau(h) = O(h^2)$ near the
...
follows.  We assume that all the mixed partial derivatives of
$f:[t_0,t_f]\times \RR \rightarrow \RR$ exist and are continuous up to
order four.  Up to $\displaystyle O(h^4)$, we replace the function
...
in the previous definition is $\displaystyle O(h^4)$ as its name suggests.
...
problem in $\displaystyle \RR^n$; namely,
$\displaystyle f:[t_0,t_f]\times \RR^n \to \RR^n$ with
...
Let $\displaystyle \VEC{y}:[a,b] \rightarrow \RR^n$ be the solution of
\begin{align*}
\VEC{y}'(t) &= f(t,\VEC{y}(t)) \quad, \quad t_0 \leq t \leq t_f \\
\VEC{y}(t_0) & = \VEC{y}_0
\end{align*}
where the function $\displaystyle f:[t_0,t_f]\times \RR^n \to \RR^n$
is continuously differentiable.  Suppose that
$\displaystyle \VEC{v}:[t_0,t_f]\rightarrow \RR^n$ is
...
y(t_0) &= y_0
\end{align*}
Its solution is $\displaystyle y(t) = e^{a(t-t_0)}y_0$ for $t_0 \leq t \leq t_f$.
...
u(s) &= w
\end{align*}
Its solution is $\displaystyle u(t) = e^{a(t-s)}w$ for $t \geq s$.
...
Since $\displaystyle E_i = O(h^{k+m})$ near the origin for all $i$, we
...
Suppose that $\displaystyle q(t) = t^2 -t + 1/6$, the Gauss-Legendre
...
$\displaystyle f:[t_o,t_f]\times \RR^n \to \RR^n$ with $n>1$.  Namely, we will
consider the initial value problem
...
\end{split}
\end{equation}
where $\displaystyle f:[t_0,t_f]\times\RR^n \rightarrow \RR^n$ and $n>1$.
...
problems with $\displaystyle f:[t_0,t_f]\times \RR^n \to \RR^n$ and
...
order in $\RR$ than in $\displaystyle \RR^n$ with $n>1$.
...
Let $\displaystyle \LL^{m}(\RR^n,\RR^n)$ be the space of multilinear
mappings from
$\displaystyle \RR^n \times \RR^n \times \cdots \times \RR^n$ ($m$
times) to $\displaystyle \RR^n$.

\begin{defn}
The {\bfseries Frechet derivative}\index{Frechet Derivative}
of degree $n$ of $\displaystyle f:\RR^n \rightarrow \RR^n$ is the mapping
$\displaystyle \fdiff{f}{m} : \RR^n \to \LL^{m}(\RR^n,\RR^n)$ defined by
\begin{align*}
...
\end{align*}
where $\displaystyle \VEC{k}_i = \begin{pmatrix}
k_{i,1} & k_{i,2} & \ldots&\ k_{1,n} \end{pmatrix}^\top$.
\end{defn}

\begin{egg}
If $\displaystyle f:\RR^2 \rightarrow \RR^2$,
\[
...
of $\displaystyle f:\RR^n \rightarrow \RR^n$ and their
{\bfseries order}\index{Runge-Kutta!Order of Elementary Differentials}
are defined recursively.
\begin{enumerate}
\item $\displaystyle f:\RR^n \rightarrow \RR^n$ is the only elementary
differential of order $1$.
\item if $\displaystyle g_1:\RR^n\rightarrow \RR^n$,
$\displaystyle g_2:\RR^n\rightarrow \RR^n$, \ldots,
$\displaystyle g_r:\RR^n\rightarrow \RR^n$
are $r$ elementary differentials of $f$ of order $m_1$, $m_2$,
\ldots, $m_r$ respectively, then 
$\displaystyle \fdiff{f}{r}(\cdot)(g_1(\cdot),g_2(\cdot), \ldots,g_r(\cdot))
...
\end{align*}
for $\displaystyle \VEC{y} \in \RR^n$, is an elementary differential of order
...
\begin{egg}
The elementary differential of $\displaystyle f:\RR^n \to \RR^n$ of order $2$ is
...
$\displaystyle f:\RR^n \to \RR^n$ of order $2$:\\
...
$\displaystyle \tau_f = $ is $\{\{\{f\ \{f\ f\}\}\}\} = f^2_{yy} \,f^2_y\,f^3$.
...
not depend on the time.  Thus, $\displaystyle f:\RR^n \to \RR^n$ and
...
and $\VEC{W}_i$ have identical terms in $\displaystyle z^q$ for $q\leq p$.
We thus have that
\[
...
$\displaystyle f:\RR^n\rightarrow\RR^n$ with $n>1$.
...
coefficient of $\displaystyle z^q$ in $\VEC{Y}_i(z)$ was equal to the
coefficient of $\displaystyle z^q$ in $\VEC{W}_i(z)$.  However, for
...
order four.  This ensures that the coefficient of $\displaystyle z^4$ in
$\VEC{Y}_i$ is still equal to the coefficient of
$\displaystyle z^4$ in $\VEC{W}_i$.
...
When $\displaystyle f:\RR^n \rightarrow \RR^n$ with $n>1$, the relation
$F(\tau_1)= F(\tau_2)$ is not necessary true and so the condition
$\alpha(\tau_1)\gamma(\tau_1)\Psi(\tau_1) +
\alpha(\tau_2)\gamma(\tau_2)\Psi(\tau_2) = \alpha(\tau_1) +
\alpha(\tau_1)$ may not guarantee that the coefficient of
$\displaystyle z^4$ in $\VEC{Y}_i$ is equal to the coefficient of
$\displaystyle z^4$ in $\VEC{W}_i$.
...
\end{split}
\end{equation}
where $\displaystyle f:\RR^2 \rightarrow \RR$.   We can rewrite this
...
$\displaystyle f:\RR^n \rightarrow \RR^n$ and $n>1$.  Note that $f$ does
not depend on time.
...
$qh$ where
$\displaystyle q = \left|\epsilon h /(\tilde{w}_{i+1} - w_{i+1})\right|^{1/4}$.
...
\approx \tilde{\tau}_{i+1}(h) +
 \frac{1}{h}\left( \tilde{w}_{i+1} -w_{i+1} \right) \ .
\end{align*}
Since $\displaystyle \tau_{i+1}(h) = O(h^4)$ and
$\displaystyle \tilde{\tau}_{i+1}(h) = O(h^5)$,
...
Since $\displaystyle \tau_{i+1}(h) = O(h^4)$, we may assume that
$\displaystyle \tau_{i+1}(h)\approx Ch^4$ for some constant $C$ and
...
$\displaystyle q^4\left| (\tilde{w}_{i+1} - w_{i+1})/h \right| < \epsilon$ or
...
$\displaystyle |\tau_{i+1}(h) | \leq \tau(h) = O(h^p)$ near the origin
...
$\displaystyle |y^{(3)}(t)|$ is continuous on $[t_0,t_f]$ and reaches
is maximum at a point on the interval $[t_0,t_f]$.  Let $K$ be the
maximum of $\displaystyle |y^{(3)}(t)|$ on $[t_0,t_f]$, then
\[
...
\displaystyle \frac{r(r-1)(r-2)\ldots(r-j+1)}{j!} & \text{ if $j>0$}
  \end{cases}
\]
for $r \in \RR$, and $\displaystyle \nabla^k g_i$ for $k \in \NN$ is
the $\displaystyle k^{th}$
...
$\displaystyle (251/720)\,y^{(5)}(\xi_i)\,h^5$; namely,
\begin{align*}
...
$\displaystyle (251/720)\,y^{(5)}(\xi_i)\,h^5$.
...
$\displaystyle (251/720)\,y^{(5)}(\xi_i)\,h^4$ for some
...
$\displaystyle -(19/720) \,y^{(5)}(\xi_i)\,h^5$.
...
\end{align*}
The local truncation error $\tau_{i+1}(h)$ is
$\displaystyle -(19/720) \,y^{(5)}(\xi_i)\,h^4$ for some
...
\end{equation}
then it is unique.  Suppose that $w$ and $\displaystyle w^\ast$ are two distinct
...
Moreover, if we set $\displaystyle w=e^x$, we get
\begin{align*}
...
satisfied.  Recall that $\displaystyle x = \ln(w) = (w-1) + O((w-1)^2)$
for $w$ near $1$.
\end{proof}
...
statement.  We have $\displaystyle p(w) = w^3-w^2$ and
\[
...
$\displaystyle p(w) = w^3- w^2$, let $v=w-1$.  Then
\begin{align*}
\frac{p(w)}{\ln(w)}
...
$\displaystyle p(w) = w^2-w$, let $v=w-1$.  Then
\begin{align*}
...
\begin{rmk}
If we consider $\displaystyle p(w) = w^{m-1}(w-1)$,
...
If we consider $\displaystyle p(w) = w^{m-2}(w^2-1)$, the explicit
...
$\displaystyle q(w) = b_{-1} w^{m+1}$ for some non-zero constant
$b_{-1} \in \RR$.  
...
multiply it by $\displaystyle v^{m+1}$, we get
\[
...
of the form $\displaystyle (w-1)^k$ with $k\geq m+2$ will not affect
...
Finally, if we assume that $\displaystyle y^{(5)}(t)$ is almost constant on
...
\]
where $\displaystyle Y \approx y^{(5)}(\xi) \approx y^{(5)}(\eta)$.

Thus $\displaystyle Y \approx
\frac{8}{3}\left(\frac{w_{i+1}^{[1]} - w_{i+1}^{[0]}}{h^5}\right)$.
If we substitute $\displaystyle y^{(5)}(\xi)$ by $Y$ in
...
If we assume that $\displaystyle y^{(p+1)}(t)$ is almost constant on an interval
$[t_{i-m},t_{i+1}]$, we may choose $K$ such that
$\displaystyle y^{(p+1)}(t_i) \approx K$.  Hence,
\[
...
\]
If we ignore the small error of order $\displaystyle h^{p+2}$, we get
\[
...
$Q = 5K/12$ and $K$ is the maximum of $\displaystyle y^{(3)}(t)$ on $[t_0,t_f]$.
...
$h < h_0$, any solution $\displaystyle \{u_i^{[j]}\}_{i=0}^N$
of (\ref{MSM_GFpertub}) with
$\displaystyle \delta_i = \delta_i^{[j]}$ for $j=1$ and $2$, then
\[
| u_i^{[1]} - u_i^{[2]} | < S \epsilon
\]
for $i = 0$, $1$, \ldots, $N$ whenever
$\displaystyle | \delta_i^{[1]} - \delta_i^{[2]}  | < \epsilon$ for
$i = 0$, $1$, \ldots, $N$.
\end{defn}
...
\end{equation}
Choose an increasing sequence $\displaystyle \{N_j\}_{j=1}^\infty$ of positive
...
A sequence $\displaystyle \{ u_i \}_{i=0}^\infty$ that satisfies
...
$\displaystyle u_i = u_i^{[1]} - u_i^{[2]}$ for all $i\geq 0$ is a solution of
(\ref{basicFDP}) with $C_i = 0$ for $i\geq s$ and $u_i = 0$ for
$0 \leq i < s$.  It follows from (\ref{solFDP}) that
$\displaystyle u_i = u_i^{[1]} - u_i^{[2]} = 0$ for $i\geq 0$.
\end{proof}
...
independence of solutions $\displaystyle \{ u_i^{[j]} \}_{i=0}^\infty$ of
...
\end{pmatrix} \ .
\]
Since $\displaystyle \{ u_i^{[k]} \}_{i=0}^\infty$ for $0\leq k < s$ are
...
$\displaystyle \{ u_i \}_{i=0}^\infty$ with $\displaystyle u_i = r^i$
...
with $\displaystyle u_i = i^kr^i$ and $0\leq k <m$
...
$\displaystyle p(r) = p'(r) = \ldots = p^{(m-1)}(r) = 0$ and
$\displaystyle p^{(m)}(r) \neq 0$.
Let $\displaystyle q(z) = z^{i-s}p(z)$ for some $i \geq s$.  We have
\[
  q^{(k)}(z) = \sum_{j=0}^k \binom{k}{j}\left(\pdfdxn{z^{i-s}}{z}{j}\right)
  p^{(k-j)}(z) \ .
\]
Hence, $\displaystyle q(r) = q'(r) = \ldots = q^{(m-1)}(r) = 0$
and $\displaystyle q^{(m)}(r) = r^{i-s} p^{(m)}(r) \neq 0$.  Therefore,
\[
...
$\displaystyle \{ v_i^{[k]} \}_{i=0}^\infty$ with
$\displaystyle v_i = i(i-1)\ldots(i-k+1)r^i$ and
...
$\displaystyle \{ u_i^{[0]} \}_{i=0}^\infty$ with $\displaystyle u_i^{(0)} = r^i$,
$\displaystyle \{ u_i^{[1]} \}_{i=0}^\infty$ with
$\displaystyle u_i^{[1]} = v_i^{[1]} = i r^i$,
and in general
$\displaystyle \{ u_i^{[k]} \}_{i=0}^\infty$ with $1<k<m$ and
\[
...
$\displaystyle \{ u_i \}_{i=0}^\infty$ of (\ref{homFDP}) is given by
\[
...
$\displaystyle z^3 -9z^2 +24z -20 = (z-2)^2(z-5)$.
...
The solution $\displaystyle \{u_i\}_{i=0}^\infty$ is given by
...
$\displaystyle \{ u_i^{[k]} \}_{i=0}^\infty$ be the solution of the
...
$\displaystyle \{ u_i \}_{i=0}^\infty$ of (\ref{basicFDP}) is given by
...
$\displaystyle u_{i-j-k-1}^{[s-1]} = 0$.  To simplify the notation, we
assume that $\displaystyle u_i^{[s-1]} = 0$ for $i<0$.  Hence,
\[
...
$\displaystyle \{ u_i^{(s-1)} \}_{i=0}^\infty$ is a solution of the homogeneous
...
If we substitute $\displaystyle \lambda^i$ for $w_i$ in this expression, we get
$\displaystyle \lambda^{i+1} = \sum_{k=0}^m a_k \lambda^{i-k}$. 
If we multiply both sides of this equation by
$\displaystyle \lambda^{m-i}$, we get $p(\lambda) = 0$ for
...
$\displaystyle p(\lambda) = - \lambda^4 + \lambda^3$.  $1$ is a root of 
...
$\displaystyle p(\lambda) = -\lambda^2 + 1$.  $1$ and $-1$  are the
...
Again, $\displaystyle \{u_i\}_{i=0}^N$ is a solution of (\ref{zzFDP}) if we set
...
$\displaystyle \{N_j\}_{j=0}^\infty$ of positive
...
following claims:
\begin{enumerate}  
\item $\displaystyle \{|c^j|/j\}_{j=1}^\infty$ does not converge to $0$ if
$c\in \RR$ satisfies $|c|>1$.
\item $\displaystyle \{|c^j+\bar{c}^j|/j\}_{j=1}^\infty$ does not
converge to $0$ if $c\in \CC \setminus \RR$ satisfies $|c|>1$.
\item $\displaystyle \{|c^j|\}_{j=1}^\infty$ does not converge to $0$
if $c\in \RR$ satisfies $|c| = 1$.
\item $\displaystyle \{|c^j+\bar{c}^j|\}_{j=1}^\infty$ does not
converge to $0$ if $c\in \CC \setminus \RR$ satisfies $|c|=1$.
\end{enumerate}

The two cases where $c \in \RR$ are easy to prove because
$\displaystyle |c|^j/j \to \infty$ as $j \to \infty$ when $|c| >1$, and
$\displaystyle |c|^j = 1$ for all $j$ when $|c|=1$.

If $c \in \CC \setminus \RR$, we have
$\displaystyle c = |c| e^{i\theta}$ for some
...
$\displaystyle \{j_k\}_{k=1}^\infty$ of positive integers and a constant $C>0$
...
need to use the fact that $\displaystyle \{ e^{j\theta} \}_{j=0}^\infty$
is dense on the unit circle.  Thus, there exist an infinite strictly
increasing sequence $\displaystyle \{j_k\}_{k=0}^\infty$ such that
...
$\displaystyle \{u_i\}_{i=0}^N$ is a solution of
...
increasing sequences $\displaystyle \{N_j\}_{j=0}^\infty$ of positive integers
such that $\displaystyle \{|u_{N_j}|\}_{j=0}^\infty$ converges to  $\infty$
...
$\displaystyle \delta(h) = O(h^2)$ in the statement of Theorem~\ref{Dahl_conv}.
...
that $\displaystyle | u_i^{[k]} | \leq Q$ for all $i$ and $k$.  Recall that all
...
form $\displaystyle e_i = i^n c^i$, where $c$ is a root of the
...
Let $\displaystyle \delta_i^{[j]}(h) = h \sigma_i^{[j]}(h)$ for $j=1$ and $2$.
Moreover, let
\[
...
$\displaystyle y(t) = e^{\mu (t-t_0)}y_0$ of the
...
$z \in \CC$ such that $\displaystyle 1 + z + z^2/2$ is on the unit circle
is the black curve shown in Figure~\ref{reg_abs_stab_RK2}.
To draw this black curve, we may use the fact that
$\displaystyle 1 + z + z^2/2 = e^{i\theta}$ is a quadratic equation whose
solutions are given by $\displaystyle z = -1 \pm \sqrt{-1+2 e^{i \theta}}$.
The number $i$ in the previous sentence is the complex number such
that $\displaystyle i^2 = -1$ and not the index $i$ in the Runge-Kutta method.
...
$\displaystyle \left| 1 + z + z^2/2 \right| = 1.2$, the red curve in
Figure~\ref{reg_abs_stab_RK2}, and the curve generated by the set of
points $z$ such that
$\displaystyle \left| 1 + z + z^2/2 \right| = 0.8$, the blue
...
$\displaystyle \left|1 + z + z^2/2 \right|=1.2$ and $0.8$ (red and blue curves
...
It is enough to show that $\displaystyle \VEC{c}^\top(\Id-zB)^{-1}\VEC{u}$ is a
...
the {\bfseries cofactor} $\displaystyle (-1)^{i+j} \det A_{i,j}$,
where $A_{i,j}$ is obtained from $\Id - zB$ by removing the
$\displaystyle i^{th}$ row and $\displaystyle j^{th}$ column.
The cofactors are polynomials in $z$ of degree at most $s-1$.
Hence, $\displaystyle \VEC{c}^\top(\Id-zA)^{-1}\VEC{u}$ is the quotient
...
order two, we have $\displaystyle r(h\mu) = 1 + h\mu + (h\mu)^2/2$.  Then
\begin{align*}
1 - |r(h\mu)|
...
\]
So
$\displaystyle B= \begin{pmatrix} 1/4 & -1/4 \\ 1/4 & 5/12 \end{pmatrix}$
and
$\displaystyle \VEC{c}= \begin{pmatrix} 1/4 \\ 3/4 \end{pmatrix}$.  Thus,
$\displaystyle \Id-z B
...
$|r(z)| < 1$ for $\displaystyle z=\rho e^{i\theta}$ with $\rho>0$ and
...
$\displaystyle 2\rho (1+\rho^2/9) >0$, $\cos(\theta) <0$ and
$\displaystyle 2\rho^2 \cos^2(\theta)/3 + \rho^4/36 > 0$), the
...
\]
are the roots of $\displaystyle 1-2z/3+z^2/6$; namely,
...
of order $p$, then $\displaystyle r(z) = e^z +O(z^{p+1})$ as $z \rightarrow 0$.
\label{exp_approx}
\end{lemma}

\begin{proof}
By definition of order,
$\displaystyle y(t_{i+1}) = y(t_i) + h\,\phi(t_i,y(t_i)) + O(h^{p+1})$ where
...
of (\ref{RKThe_equ}) is $\displaystyle y(t) = e^{t\mu}w_0$.  Thus,
\[
...
$\displaystyle w_i = (r(h\lambda))^iw_0$ for $i\geq 0$,
...
$\displaystyle e^z$ of order $2k$.  From Wanner-Hairer-Norsett
...
\]
If we subtract $\displaystyle \lambda^{i+1}$ from both sides of this equality
and multiply them by $\displaystyle \lambda^{m-i}$, we get
\[
...
\]
for $m \leq i < N$.  Since $\displaystyle y(t) = A e^{\mu\, t}$ is the
...
$\displaystyle e^{\mu h}$ for $h$ small.  This root is called the
...
solutions $\displaystyle \{w_i\}_{i=0}^\infty$ of the difference
...
$\displaystyle y(t) = e^{\mu(t-t_0)}y_0$ of the differential equation
(\ref{RKThe_equ}) satisfy $\displaystyle \lim_{t\to \infty} y(t) = 0$.
\end{rmk}
...
\]
are the roots of $\displaystyle \lambda^2 + 4h\lambda -1 =0$, and
...
$\displaystyle w_i =c_1\lambda^i + c_2\lambda_2^i$ is the general solution of
...
Similarly, $\displaystyle (1+2h)^i \approx e^{2 t_i}$ for $h$ very small.
...
mathematically but the theory assumes that $\displaystyle \delta_i(h) = O(h^2)$
...
$\displaystyle \{w_i\}_{i=0}^\infty$ of this finite difference
equation is a linear combination of solutions of the form
$\displaystyle \{i^n\lambda^i\}_{i=0}^\infty$,
...
if and only if any non-trivial solution $\displaystyle \{w_i\}_{i=0}^\infty$ of
...
solution is $\displaystyle y(t) = y_0\,e^{\mu t}$.  The approximation $w_i$ of
...
\end{align*}
Thus, $\displaystyle w_i = (1+h\mu)^iy_0$ for $i\geq 0$.
...
$\displaystyle (1+h\mu)^i y_0 + (1+h\mu)^i \delta_0$ which differ from the
unperturbed value of $w_i$ by $\displaystyle (1+h\mu)^i \delta_0$.  To have this
...
we may assume that $\displaystyle \lambda = e^{\theta i}$ for some
...
$\displaystyle \lambda = r e^{\theta i}$
...
$\displaystyle \lambda = e^{i\theta}$ (black curve), for
$\displaystyle \lambda = 1.1 e^{i\theta}$
and $\displaystyle \lambda = 1.5 e^{i\theta}$ (red curves) and
$\displaystyle \lambda = 0.8 e^{i\theta}$ (blue curve).  The region of absolute
...
$z = - p(\lambda)/q(\lambda)$ for $\displaystyle \lambda = e^{i\theta}$ with
$0\leq \theta < 2\pi$.   For the Adams-Bashforth method
of order four $\displaystyle p(\lambda) = -\lambda^4 + \lambda^3$ and
$\displaystyle q(\lambda) = (55\lambda^3-59\lambda^2+37\lambda - 9)/24$.
...
$\displaystyle \lambda = e^{i\theta}$ with $0\leq \theta < 2\pi$.
For the Adams-Moulton method of order four
$\displaystyle p(\lambda) = -\lambda^3 + \lambda^2$ and
$\displaystyle q(\lambda) = (9\lambda^3+19\lambda^2-5\lambda + 1)/24$.
...
$\displaystyle \lambda = 1.2 e^{i\theta}$ and
$\displaystyle \lambda = 0.8 e^{i\theta}$ with
$0\leq \theta < 2\pi$ for the two methods.  The points $z$ on the
curves in red associated to $\displaystyle \lambda = 1.2 e^{i\theta}$ correspond
...
in blue associated to $\displaystyle \lambda = 0.8 e^{i\theta}$ correspond to
...
$z = - p(\lambda)/q(\lambda)$ for $\displaystyle \lambda = e^{i\theta}$ (black
curve), for $\displaystyle \lambda = 1.2 e^{i\theta}$ (red curve) and
$\displaystyle \lambda = 0.8 e^{i\theta}$ (blue curve).  The region to
...
$\displaystyle p(\lambda) = -\lambda^2 + \lambda$ and
$\displaystyle q(\lambda) = (\lambda^2 + \lambda)/2$.
...
$\displaystyle \lambda = e^{i\theta}$ (black curve), for
$\displaystyle \lambda = 1.2 e^{i\theta}$
(red curve) and $\displaystyle \lambda = 0.8 e^{i\theta}$ (blue
...
$\displaystyle y(t) = 1-t^2+C e^{100t}$.  The initial condition $y(0)=1$ implies
...
number of subinterval $N=10$, the tolerance $\displaystyle T=10^{-5}$ and the
...
Consider the quadratic equation $\displaystyle p(z) = az^2+bz+c$,
...
$|a| \geq|c|$ and
$\displaystyle \big| |a|^2-|c|^2\big| \geq |a\overline{b}-b\overline{c}|$
\label{CSCrit}
\end{lemma}
...
$\displaystyle \{ z \in \CC : z = \rho\,e^\theta \ \text{with}\ \rho>0
...
\end{split}
\end{equation}
where $\displaystyle f:[t_0,t_f]\times\RR^n \rightarrow \RR^n$.
...
\end{split}
\end{equation}
where $\displaystyle \VEC{y}:\RR\rightarrow \RR^2$,
$\displaystyle \VEC{y}_0 =  \begin{pmatrix} y_{0,1} \\ y_{0,2}
\end{pmatrix} \in \RR^2$, and
$\displaystyle A = \begin{pmatrix} -\lambda & 1 \\
0 & -0.1 \end{pmatrix}$ for a large positive number $\lambda$.

Since $\displaystyle A = Q B Q^{-1}$, where
$\displaystyle Q=\begin{pmatrix} 1 & 1 \\
0 & \lambda -0.1 \end{pmatrix}$ and
$\displaystyle B=\begin{pmatrix} -\lambda & 0 \\
...
Suppose that
$\displaystyle \VEC{y}_0 = \begin{pmatrix} 1 \\ \lambda-0.1
...
$\displaystyle \VEC{y}(t) = e^{-0.1 t} \VEC{y}_0$.
...
\]
Since $\displaystyle A = Q B Q^{-1}$ for
$\displaystyle Q=\begin{pmatrix} 1 & 1 \\
0 & \lambda -0.1 \end{pmatrix}$ and
$\displaystyle B=\begin{pmatrix} -\lambda & 0 \\
...
\\
0 & \left(\displaystyle \frac{1- h/20}{1+ h/20}\right)^{i+1}
...
\sol{\SOL}{
\subQ{a}
Let $\displaystyle f(t,y) = t^2 \sin(y) + y$.  The function $f$ is
...
\]
Since $\displaystyle |t^2 \cos(y) + 1| \leq 2$ for all
...
have magnitude less than $\displaystyle 10^{-8}$. \\
...
magnitude less than $\displaystyle 10^{-8}$.\\
...
has a global minimum for $h>0$ at $\displaystyle h=\sqrt{2}/10^4$.
\[
\begin{array}{c|c|c|c}
...
\end{array}
\]
Thus, $\displaystyle h = \sqrt{2}/10^4$ minimizes the error bound for the Euler
Method.

\subQ{c} We use the Euler Method with $t_0=1$, $t_f= 2$,
$y_0 = 0$ and $f(t,y) = 1 + y/t$.  Since
$\displaystyle (t_f-t_0)/h = 10^4/\sqrt{2} \approx 7071.0678$
...
$\displaystyle |e_{7072}| = |u_{7072} - y_{7072}| \approx 0.707 \times 10^{-4}$.
...
$i\geq 0$ with  $w_0  = 1$,
where
$\displaystyle K_1 = 1 +(t_i-w_i)^2$,
$\displaystyle K_2 = 1 +\big((t_i+0.05) - (w_i +0.05 K_1)\big)^2$,
$\displaystyle K_3 = 1 +\big((t_i + 0.05) - (w_i+ 0.05 K_2)\big)^2$ and
$\displaystyle K_4 = 1 +\big(t_{i+1}- (w_i + 0.1 K_3) \big)^2$.
...
\end{align*}
because
$\displaystyle (\Id - \beta h A)^{-1} A = A (\Id - \beta h A)^{-1}$.
Thus
\begin{align*}
...
$\displaystyle \tau_{i+1}(h) = O(h^2)$.  The method is consistent because
\[
...
$\displaystyle t^2 - t + 1/6$, then the $2$-stage Runge-Kutta Method
...
If $\displaystyle q(t) = t^{n-1}$ with $1 \leq n \leq k$, then
(\ref{GradQ7A}) yields
\[
...
For $1 \leq i \leq k$, Let $\displaystyle \VEC{w}^{[i]}$ be
the vector with components
$\displaystyle w^{[i]}_j = \beta_{i,j} - \int_0^{\alpha_i} \ell_j(t) \dx{t}$
for $1 \leq j \leq k$.  Moreover, let $\displaystyle \VEC{w}^{[k+1]}$
be the vector with the components 
$\displaystyle w^{[k+1]}_j = \gamma_j - \int_0^1 \ell_j(t) \dx{t}$
for $1 \leq j \leq k$.

We have that (\ref{GradQ7C}) with $\displaystyle q(t) = t^n$ for $0 \leq n < k$
yields the linear equations $\displaystyle A \VEC{w}^{[i]}  = \VEC{0}$ for
$0 \leq i \leq k$.  Similarly, (\ref{GradQ7D}) with
$\displaystyle q(t) = t^n$ for $0 \leq n < k$ yields the
linear equations $\displaystyle A \VEC{w}^{[k+1]}  = \VEC{0}$.  Since $A$ is
...
\]
we have that $\displaystyle \VEC{c}^\top(\Id-zA)^{-1}\VEC{u}$ is the quotient
...
polynomial $\displaystyle q(t) = t^2 -t + 1/6$.  See
...
\]
where $\displaystyle r(z) = 1 + z\VEC{c}^\top (\Id - zB)^{-1} \VEC{u}$ with
\[
...
$\displaystyle w_i = A i^2 + B i$ for some constants $A$ and $B$.\\
...
$\displaystyle w_i = r^i$, we get $\displaystyle r^{i+1} = r^i$.  The
...
\]
for all $i$.  Thus, $\displaystyle 2A-h^2 = 0$ and
$\displaystyle A + B - 5h^2/12 = 0$.  Solving for $A$ and $B$,
we find $\displaystyle A = h^2/2$ and $\displaystyle B = -h^2/12$.  Hence,
...
$\displaystyle p(w) = \lambda^2 - \lambda$.  It has two roots, $\lambda = 0$ and
...
\subQ{a} We consider $\displaystyle p(w) = w^3-1$ and set $m=3$.  Using the
...
\subQ{b} We consider $\displaystyle p(w) = w^3-1$ and set $m=3$.  Using the
substitution $w=v+1$, we get
\begin{align*}
\frac{p(w)}{\ln(w)} &= \frac{w^3-1}{\ln(w)}
= \frac{(v+1)^3-1}{\ln(v+1)}
...
(we set the coefficient of $\displaystyle y^{(3)}(t_i) h^2$ to $0$), we
get that $\displaystyle a_1 = 5$.

We have found that all the terms in $\displaystyle h^{k}$ for $k<3$ vanish if
$a_0 = -4$, $a_1 = 5$. $b_0 = 4$ and $b_1 = 2$.
With these values of $a_1$ and $b_1$, we have that
$\displaystyle \frac{1}{24} - \frac{a_1}{24} + \frac{b_1}{6}
= \frac{1}{6} \neq 0$ (the coefficient of $\displaystyle y^{(4)}h^3$).  Hence,
