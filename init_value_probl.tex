...
$0\leq i < N$ satisfy
$\DS \max_{0\leq i <N}\,|h_i| < K \min_{0\leq i <N}\,|h_i|$
for a constant $K$.
...
$\DS y''(\xi_i)h^2/2$ is much smaller than
$y_i + f(t_i,y_i)\,h$ for all
...
Discretization Error} for the Euler's method is $\DS y''(\xi_i)h^2/2$.
\end{enumerate}
\end{rmkList}
...
easy to find the exact solution $\DS y(t) = e^{0.1t^2 -0.1}$ of
...
easy to find the exact solution $\DS y(t) = e^{t^2 -1}$ of
...
e^{L(t_i-t_0)} - 1 \right) + |\delta_0|\,e^{L(t_i-t_0)} \ ,
\end{align*}
where we have used $\DS (1+x)^n \leq e^{nx}$ for $x > 0$ in the second
...
\]
where the factor
$\DS \frac{Mh}{2} + \frac{\delta}{h}$ goes to infinity as
$h$ goes to $0$.
\end{enumerate}
\end{rmkList}
...
integer $k$ (as large as possible) such that
$\DS \tau(h) = O(h^k)$ near
...
\begin{rmk} Since $\DS |h^p|$ is a decreasing function of
$p$ for $|h|<1$ fixed, the local truncation error is generally smaller
for high order methods than for low order methods.
\end{rmk}
...
\end{equation}
for some $\xi_i$ between $t_i$ and $t_{i+1}$.  If we assume that
$\DS \frac{y^{(n+1)}(\xi_i)}{(n+1)!}\,h^{n+1}$ is small
...
that $\DS |y^{(n+1)}(t)| \leq M$ for all $t\in [t_0,t_f]$, then
$\DS |\tau_{i+1}(h)| \leq \tau(h) \equiv M|h|^n/(n+1)!$ for
all $i$ and $\DS \tau(h)  = O(h^n)$ near the origin.  This
justifies the name Taylor method of order $n$.
...
$\DS w_{i+1} = w_i + h \sum_{k=1}^s\,\gamma_k K_k$, where
$\DS \sum_{k=1}^s\,\gamma_k K_k$ is a weighted average of
...
order two.  Up to $\DS O(h^2)$, we replace the function
$\phi(t,y)$ in the
...
$\DS h\, \pdydx{f}{t}(t,y)$ and 
$\DS h f(t,y)\, \pdydx{f}{y}(t,y)$ in 
...
Taylor method of order $2$) such that $\DS \tau(h) = O(h^2)$ near the
...
w_0 & = y_0
\end{align*}
where $\DS K_1 = f(t_i,w_i)$,
$\DS K_2 = f(t_i+ h/2,w_i+ h K_1/2)$,
$\DS K_3 = f(t_i+h/2,w_i+ h K_2/2)$ and
$\DS K_4 = f(t_{i+1},w_i+ h K_3)$.
\end{defn}
...
follows.  We assume that all the mixed partial derivatives of
$f:[t_0,t_f]\times \RR \rightarrow \RR$ exist and are continuous up to
order four.  Up to $\DS O(h^4)$, we replace the function
$\phi(t,y)$ in the Taylor method of order $4$
(Definition~\ref{Tayloralgo}) by an
...
in the previous definition is $\DS O(h^4)$ as its name suggests.
\item Note that $\DS \sum_{j=1}^s \gamma_j = 1$ is a
...
problem in $\DS \RR^n$; namely,
$\DS f:[t_0,t_f]\times \RR^n \to \RR^n$ with
...
Let $\DS \VEC{y}:[a,b] \rightarrow \RR^n$ be the solution of
\begin{align*}
\VEC{y}'(t) &= f(t,\VEC{y}(t)) \quad, \quad t_0 \leq t \leq t_f \\
\VEC{y}(t_0) & = \VEC{y}_0
\end{align*}
where the function $\DS f:[t_0,t_f]\times \RR^n \to \RR^n$
is continuously differentiable.  Suppose that
$\DS \VEC{v}:[t_0,t_f]\rightarrow \RR^n$ is
...
y(t_0) &= y_0
\end{align*}
Its solution is $\DS y(t) = e^{a(t-t_0)}y_0$ for $t_0 \leq t \leq t_f$.
...
u(s) &= w
\end{align*}
Its solution is $\DS u(t) = e^{a(t-s)}w$ for $t \geq s$.
...
\begin{theorem}
Let $\DS q(t) = \prod_{j=1}^k\,(t-\alpha_j)$, where the
...
\end{align*}
for $0\leq i <N$.  Let
$\DS \phi(t_i,w_i) = \sum_{j=1}^k\gamma_j K_j$ be the
...
Since $\DS E_i = O(h^{k+m})$ near the origin for all $i$, we
may assume that there exists an interval $[c,d]$ such that $w_i$ and
$w_i-E_i$ are in $[c,d]$ for all $i$ and all small $h$ (i.e.\ for all
partitions $a \leq t_0 < t_1 < \dots < t_N = b$ with $t_{i+1}-t_i=h$ small
enough).  Since $\phi$ is composed of $f$ and some of its partial
derivatives, there exists a constant $M$ such that
$\left|\DS \pdydx{\phi}{w}(t,w)\right| \leq M$ for all
$(t,w) \in [a,b]\times [c,d]$.
Hence
\[
...
Suppose that $\DS q(t) = t^2 -t + 1/6$, the Gauss-Legendre
polynomial of degree $2$.  $q(t) = (t-\alpha_1)(t-\alpha_2)$, where
...
None of the results from graph theory will be proved.  There are many
excellent introductory books on graph theory that the reader may consult.
Moreover, we will consider initial value problems like (\ref{ODE}) where
$\DS f:[t_o,t_f]\times \RR^n \to \RR^n$ with $n>1$.  Namely, we will
consider the initial value problem
...
\end{split}
\end{equation}
where $\DS f:[t_0,t_f]\times\RR^n \rightarrow \RR^n$ and $n>1$.
...
problems with $\DS f:[t_0,t_f]\times \RR^n \to \RR^n$ and
$n>1$.  But there are some differences.  One property that is influenced by the
...
order in $\RR$ than in $\DS \RR^n$ with $n>1$.
...
Let $\DS \LL^{m}(\RR^n,\RR^n)$ be the space of multilinear
mappings from
$\DS \RR^n \times \RR^n \times \cdots \times \RR^n$ ($m$
times) to $\DS \RR^n$.

\begin{defn}
The {\bfseries Frechet derivative}\index{Frechet Derivative}
of degree $n$ of $\DS f:\RR^n \rightarrow \RR^n$ is the mapping
$\DS \fdiff{f}{m} : \RR^n \to \LL^{m}(\RR^n,\RR^n)$ defined by
\begin{align*}
...
\end{align*}
where $\DS \VEC{k}_i = \begin{pmatrix}
k_{i,1} & k_{i,2} & \ldots&\ k_{1,n} \end{pmatrix}^\top$.
\end{defn}

\begin{egg}
If $\DS f:\RR^2 \rightarrow \RR^2$,
\[
...
of $\DS f:\RR^n \rightarrow \RR^n$ and their
{\bfseries order}\index{Runge-Kutta!Order of Elementary Differentials}
are defined recursively.
\begin{enumerate}
\item $\DS f:\RR^n \rightarrow \RR^n$ is the only elementary
differential of order $1$.
\item if $\DS g_1:\RR^n\rightarrow \RR^n$,
$\DS g_2:\RR^n\rightarrow \RR^n$, \ldots,
$\DS g_r:\RR^n\rightarrow \RR^n$
are $r$ elementary differentials of $f$ of order $m_1$, $m_2$,
\ldots, $m_r$ respectively, then 
$\DS \fdiff{f}{r}(\cdot)(g_1(\cdot),g_2(\cdot), \ldots,g_r(\cdot))
...
\end{align*}
for $\DS \VEC{y} \in \RR^n$, is an elementary differential of order
$\DS 1 +\sum_{i=1}^r\,m_i$ of $\VEC{f}$.
\end{enumerate}
...
\begin{egg}
The elementary differential of $\DS f:\RR^n \to \RR^n$ of order $2$ is
...
Here are two elementary differentials of
$\DS f:\RR^n \to \RR^n$ of order $2$:\\
...
$\DS \tau_f = $ is $\{\{\{f\ \{f\ f\}\}\}\} = f^2_{yy} \,f^2_y\,f^3$.
...
not depend on the time.  Thus, $\DS f:\RR^n \to \RR^n$ and
...
$\DS \psi_j(\tau) \equiv \sum_{k=1}^s\,\beta_{j,k}$
for $1\leq j \leq s+1$.
...
\end{picture}
&
$\DS \sum_{j=1}^s\,\beta_{s+1,j} = \sum_{j=1}^s\,\gamma_j = 1$
\\
\begin{picture}(2,1)
\put(1,0){\circle*{0.1}}
\put(1,0){\line(0,1){1}}
...
and $\VEC{W}_i$ have identical terms in $\DS z^q$ for $q\leq p$.
We thus have that
\[
...
$\DS 1 = \sum_{j=1}^3 \gamma_j$.

For the rooted tree $\tau$ of order two, we get from
$\Psi(\tau) = 1/\gamma(\tau)$ that
$\DS \frac{1}{2} = \sum_{j=2}^3\,\alpha_j\,\gamma_j$ since
$\alpha_1 = 0$.
...
$\DS f:\RR^n\rightarrow\RR^n$ with $n>1$.
...
coefficient of $\DS z^q$ in $\VEC{Y}_i(z)$ was equal to the
coefficient of $\DS z^q$ in $\VEC{W}_i(z)$.  However, for
$n=1$, the condition $\Psi(\tau)\gamma(\tau) = 1$ for all rooted trees
$\tau$ of order $q$ is not always necessary to satisfy (\ref{YWorderCond}).
...
order four.  This ensures that the coefficient of $\DS z^4$ in
$\VEC{Y}_i$ is still equal to the coefficient of
$\DS z^4$ in $\VEC{W}_i$.
...
\end{split}
\end{equation}
where $\DS f:\RR^2 \rightarrow \RR$.   We can rewrite this
initial value problem as a system
\begin{align*}
\VEC{z}'(t) & = \tilde{f}(\VEC{z}(t)) \quad , \quad  t_0 \leq s \leq t_f \\
\VEC{z}(t_0) &= \VEC{z}_0
\end{align*}
where
$\DS \tilde{f}(\VEC{z}) = \begin{pmatrix}
f(z_2,z_1) \\ 1 \end{pmatrix}$ and
$\DS \VEC{z}_0 = \begin{pmatrix}
y_0 \\ t_0 \end{pmatrix}$.
Hence,
\begin{align*}
...
\begin{description}
\item[$A$] The method is of order $p$ with
$\DS f:\RR^n \rightarrow \RR^n$ and $n>1$.  Note that $f$ does
not depend on time.
...
$qh$ where
$\DS q = \left|\epsilon h /(\tilde{w}_{i+1} - w_{i+1})\right|^{1/4}$.
...
\approx \tilde{\tau}_{i+1}(h) +
 \frac{1}{h}\left( \tilde{w}_{i+1} -w_{i+1} \right) \ .
\end{align*}
Since $\DS \tau_{i+1}(h) = O(h^4)$ and
$\DS \tilde{\tau}_{i+1}(h) = O(h^5)$,
...
Since $\DS \tau_{i+1}(h) = O(h^4)$, we may assume that
$\DS \tau_{i+1}(h)\approx Ch^4$ for some constant $C$ and
$h$ small.  Let $q$ be a positive constant and suppose that
(\ref{RKMfour}) is used to approximate $y(t_i+qh)$.
...
$\DS q^4\left| (\tilde{w}_{i+1} - w_{i+1})/h \right| < \epsilon$ or
...
$\DS |\tau_{i+1}(h) | \leq \tau(h) = O(h^p)$ near the origin
for all $i$, we say that the
...
$\DS |y^{(3)}(t)|$ is continuous on $[t_0,t_f]$ and reaches
is maximum at a point on the interval $[t_0,t_f]$.  Let $K$ be the
maximum of $\DS |y^{(3)}(t)|$ on $[t_0,t_f]$, then
\[
...
$[t_0,t_f]\times \RR$, then $|y''(t)|$ and $\DS |y^{(3)}(t)|$ are
continuous on $[t_0,t_f]$ and reaches their maximum at a point on the
interval $[t_0,t_f]$.  Let $M_2$ be the maximum of $|y''(t)|$ on
$[t_0,t_f]$ and $M_3$ be the maximum of $\DS |y^{(3)}(t)|$
on $[t_0,t_f]$.  For $\theta = 1/2$, we have
\[
|\tau_{i+1}(h)|
...
\binom{r}{j} = \begin{cases}
  1 & \text{ if $j=0$} \\
\DS \frac{r(r-1)(r-2)\ldots(r-j+1)}{j!} & \text{ if $j>0$}
  \end{cases}
\]
for $r \in \RR$, and $\DS \nabla^k g_i$ for $k \in \NN$ is
the $\DS k^{th}$
...
to get the discretization error
$\DS (251/720)\,y^{(5)}(\xi_i)\,h^5$; namely,
\begin{align*}
&\int_0^1 (-1)^4 \binom{-s}{4} g^{(4)}(t_i+\eta_i(s)h) \dx{s} \\
&\qquad =
\int_0^1 (-1)^4 \frac{(-s)(-s-1)(-s-2)(-s-3)}{4!}
y^{(5)}(t_i+\eta_i(s)h) \dx{s} \\
&\qquad = \frac{1}{24} \int_0^1 \underbrace{s(s+1)(s+2)(s+3)}_{\geq 0} \,
y^{(5)}(t_i+\eta_i(s)h) \dx{s} \\
&\qquad
...
$\DS (251/720)\,y^{(5)}(\xi_i)\,h^4$ for some
...
\end{align*}
The local truncation error $\tau_{i+1}(h)$ is
$\DS -(19/720) \,y^{(5)}(\xi_i)\,h^4$ for some
...
$\DS w_{i+1}^{[0]}$ of $w_{i+1}$ is given
...
(\ref{Lipschitz}), then
$\DS \left\{ w_{i+1}^{[k]}\right\}_{k=0}^\infty$ converges
...
\end{equation}
then it is unique.  Suppose that $w$ and $\DS w^\ast$ are two distinct
...
$\DS \left\{ w_{i+1}^{[k]} \right\}_{k=0}^\infty$
...
$\DS \left|w_{i+1}^{[r]} - w_{i+1}^{[s]} \right|< \epsilon$ whenever 
$r,s \geq N$.
...
$\DS \left\{ w_{i+1}^{[k]} \right\}_{k=0}^\infty$.  We show that
...
Moreover, if we set $\DS w=e^x$, we get
\begin{align*}
...
satisfied.  Recall that $\DS x = \ln(w) = (w-1) + O((w-1)^2)$
for $w$ near $1$.
\end{proof}
...
statement.  We have $\DS p(w) = w^3-w^2$ and
\[
...
polynomial $\DS p(w)= w^{m+1} - \sum_{j=0}^m\,a_j\,w^{m-j}$
...
$\DS \frac{v}{\ln(v+1)}$ can be defined at $v=0$ by the
extension
\[
g(v) = \begin{cases}
\DS \frac{v}{\ln(v+1)} & \quad \text{if} \quad v \neq 0 \\
...
\begin{egg}
To construct an implicit method of order $4$ from
$\DS p(w) = w^3- w^2$, let $v=w-1$.  Then
\begin{align*}
\frac{p(w)}{\ln(w)}
...
\begin{egg}
To construct an explicit method of order $1$ from
$\DS p(w) = w^2-w$, let $v=w-1$.  Then
\begin{align*}
...
\end{align*}
Thus $\DS q(w) = -\frac{1}{2} + \frac{3}{2} w$ and
...
\begin{rmk}
If we consider $\DS p(w) = w^{m-1}(w-1)$,
...
If we consider $\DS p(w) = w^{m-2}(w^2-1)$, the explicit
methods (of order $m$) are called
...
$\DS b_{-1} = \left(\sum_{j=1}^{m+1}\,\frac{1}{j} \right)^{-1}$
and the characteristic polynomial is
$\DS p(w) = b_{-1}\sum_{j=1}^{m+1}\,\frac{1}{j}\,w^{m+1-j}(w-1)^j$.
\end{prop}

\begin{proof}
We will use Theorem~\ref{link_order_char}.  We have by hypothesis that
$\DS q(w) = b_{-1} w^{m+1}$ for some non-zero constant
$b_{-1} \in \RR$.  
...
multiply it by $\DS v^{m+1}$, we get
\[
...
of the form $\DS (w-1)^k$ with $k\geq m+2$ will not affect
(\ref{backwardParg}), we may assume that
\[
p(w) = b_{-1} \sum_{j=1}^{m+1}\,\frac{1}{j}\,(w-1)^j\,w^{m+1-j} \ .
\]
To get $p$ of the form
$\DS p(w) = w^{m+1} - \sum_{j=0}^m a_j w^{m-j}$, we need
$\DS 1 = b_{-1} \sum_{j=1}^{m+1}\,\frac{1}{j}$.
\end{proof}
...
\item ($\mathrm{P}$) Suppose that we have found the approximation $w_i$ of $y_i$
...
\item[$\mathrm{E}$:] Evaluation
\[
f_{i+1} = f(t_{i+s},w_{i+s})
\]
\end{description}
This predictor-corrector method is named
$\DS \mathrm{P(EC)}^{\nu}\mathrm{E}$.
In general, the number of iterations $\nu$ should be small.
...
\label{LE_Corr}
\end{align}
for some $\eta_{i,k}$ between $y(t_{i+1})$ and
$\DS w_{i+1}^{[k]}$, where
\[
...
< \epsilon \  ,
\]
we accept $\DS w_{i+1} = w_{i+1}^{[1]}$, $w_i$, $w_{i-1}$
...
in (\ref{Q}).  The value of $q$ should be less than $1$.  If $w_j$ for
$i-3 \leq j \leq i$ have already been accepted as good approximations
(i.e. $w_i$ comes from the Adams-Moulton method of order four),
...
$\DS y(t_{i+1}) \approx w_{i+1}^{[0]}$, we get
...
Finally, if we assume that $\DS y^{(5)}(t)$ is almost constant on
...
\]
where $\DS Y \approx y^{(5)}(\xi) \approx y^{(5)}(\eta)$.

Thus $\DS Y \approx
\frac{8}{3}\left(\frac{w_{i+1}^{[1]} - w_{i+1}^{[0]}}{h^5}\right)$.
If we substitute $\DS y^{(5)}(\xi)$ by $Y$ in
(\ref{AMTSM_tau}), we get that
\[
...
same estimate for $Y$), we get that
\[
...
If we assume that $\DS y^{(p+1)}(t)$ is almost constant on an interval
$[t_{i-m},t_{i+1}]$, we may choose $K$ such that
$\DS y^{(p+1)}(t_i) \approx K$.  Hence,
\[
...
\]
If we ignore the small error of order $\DS h^{p+2}$, we get
\[
K h^{p+1} \approx \frac{\DS \tilde{w}_{i+1} - w_{i+1}}
{\DS C_p - \tilde{C}_p}
\]
...
\[
\bigg| \frac{\DS C_p}{\DS C_pc-\tilde{D}_p}
...
\[
\bigg| \frac{\DS C_p}{\DS C_pc-\tilde{C}_p}
...
\end{align*}
for $\DS R = L \max_{-1 \leq j \leq m} |b_j|$.
...
\begin{align*}
\left| K_j^{[1]}  - K_j^{[2]}  \right|
&\leq \|\VEC{K}\|_1 \leq \frac{L}{1-L h\|B\|_1} \|\VEC{W}\|_1
\leq 2 L \|\VEC{W}\|_1 \\
...
$\DS \max_{0\leq i \leq N} |\delta_i(h)| \to 0$ as $h\to 0$.
...
$\DS \max_{0\leq i \leq N} |\delta_i(h)|$ very small,
...
\[
\max_{0\leq i \leq N} |y_i - w_i| \leq
\frac{Mh}{2L}\left(e^{L(t_f -t_0)} -1 \right) \rightarrow 0
\]
...
$\DS \max_{0\leq i \leq N} |\delta_i(h)| = O(h^2)$ to be
...
$Q = 5K/12$ and $K$ is the maximum of $\DS y^{(3)}(t)$ on $[t_0,t_f]$.
...
where
$\DS \VEC{y}
= \begin{pmatrix} y_{i-m} & \ldots & y_{i-1} & y_i & y_{i+1}
...
&\quad = \frac{\DS \big(y(t_i) + h y'(t_i) + \frac{h^2}{2} y''(t_i) +
...
\]
where
$\DS M = \max_{t_0 \leq t \leq t_f} |y'''(t)|$.
...
$h < h_0$, any solution $\DS \{u_i^{[j]}\}_{i=0}^N$
of (\ref{MSM_GFpertub}) with
$\DS \delta_i = \delta_i^{[j]}$ for $j=1$ and $2$, then
\[
| u_i^{[1]} - u_i^{[2]} | < S \epsilon
\]
for $i = 0$, $1$, \ldots, $N$ whenever
$\DS | \delta_i^{[1]} - \delta_i^{[2]}  | < \epsilon$ for
$i = 0$, $1$, \ldots, $N$.
\end{defn}
...
\end{equation}
Choose an increasing sequence $\DS \{N_j\}_{j=1}^\infty$ of positive
...
$\DS h = h_j \equiv \frac{t_f-t_0}{N_j} \to 0$,
...
\]
We get $\DS 1-\sum_{k=0}^m\,a_k  = 0$ from the first equation and,
...
$\DS \max_{0\leq i \leq N} |w_i -y_i| \to 0$ as $h \to 0$, then
$\DS 1 = \sum_{i=0}^m\,a_i$.
\end{rmk}
...
A sequence $\DS \{ u_i \}_{i=0}^\infty$ that satisfies
(\ref{basicFDP}) is called a
...
$\DS \left\{ u_i^{[1]} \right\}_{i=0}^\infty$ and
$\DS \left\{u_i^{[2]} \right\}_{i=0}^\infty$ of
(\ref{basicFDP}).  Then,
$\DS \left\{ u_i \right\}_{i=0}^\infty$ with
$\DS u_i = u_i^{[1]} - u_i^{[2]}$ for all $i\geq 0$ is a solution of
(\ref{basicFDP}) with $C_i = 0$ for $i\geq s$ and $u_i = 0$ for
$0 \leq i < s$.  It follows from (\ref{solFDP}) that
$\DS u_i = u_i^{[1]} - u_i^{[2]} = 0$ for $i\geq 0$.
\end{proof}
...
$\DS \left\{ u_i^{[k]} \right\}_{i=0}^\infty$ be the solution of
\begin{align*}
\sum_{j=0}^s\,a_j\,u_{i-j} &= 0 \quad, \quad i \geq s \\
...
$\DS \left\{ u_i^{[0]} \right\}_{i=0}^\infty$,
$\DS \left\{ u_i^{[1]} \right\}_{i=0}^\infty$,
\ldots, $\DS \left\{ u_i^{[s-1]} \right\}_{i=0}^\infty$
...
\begin{theorem}
Let $\DS \{ u_i^{[k]} \}_{i=0}^\infty$ for $0 \leq k < s$ be
...
$\DS \left\{ u_i^{[k]} \right\}_{i=0}^\infty$ for $0 \leq k < s$.
\end{theorem}
...
\end{pmatrix} \ .
\]
Since $\DS \{ u_i^{[k]} \}_{i=0}^\infty$ for $0\leq k < s$ are
...
Suppose that $\DS u_i = z^i$ for $i \geq 0$ is a solution of
...
$\DS \{ u_i \}_{i=0}^\infty$ with $\DS u_i = r^i$
is a solution for (\ref{homFDP}).  We note that $r \neq 0$ because we
...
$\DS \left\{ u_i^{[k]} \right\}_{i=0}^\infty$
with $\DS u_i = i^kr^i$ and $0\leq k <m$
...
$\DS p(r) = p'(r) = \ldots = p^{(m-1)}(r) = 0$ and
$\DS p^{(m)}(r) \neq 0$.
Let $\DS q(z) = z^{i-s}p(z)$ for some $i \geq s$.  We have
\[
  q^{(k)}(z) = \sum_{j=0}^k \binom{k}{j}\left(\pdfdxn{z^{i-s}}{z}{j}\right)
  p^{(k-j)}(z) \ .
\]
Hence, $\DS q(r) = q'(r) = \ldots = q^{(m-1)}(r) = 0$
and $\DS q^{(m)}(r) = r^{i-s} p^{(m)}(r) \neq 0$.  Therefore,
\[
...
$\DS \{ v_i^{[k]} \}_{i=0}^\infty$ with
$\DS v_i = i(i-1)\ldots(i-k+1)r^i$ and
...
&\qquad + \underbrace{\left( (-1)^N\prod_{n_1=1}^N a_{n_1}\right)}_{=A_N} i \ ,
\end{align*}
we have that
\[
...
instance, $\DS A_1 = - \sum_{n=1}^{k-1} n$.
Using the fact that a linear combination of solutions of
(\ref{homFDP}) is a solution of (\ref{homFDP}), we have that
$\DS \{ u_i^{[0]} \}_{i=0}^\infty$ with $\DS u_i^{(0)} = r^i$,
$\DS \{ u_i^{[1]} \}_{i=0}^\infty$ with
$\DS u_i^{[1]} = v_i^{[1]} = i r^i$,
and in general
$\DS \{ u_i^{[k]} \}_{i=0}^\infty$ with $1<k<m$ and
\[
...
$\DS \{ u_i \}_{i=0}^\infty$ of (\ref{homFDP}) is given by
\[
...
The solution $\DS \{u_i\}_{i=0}^\infty$ is given by
$\DS u_i = -\left(\frac{1}{9}+\frac{1}{6}\,i\right)2^i +
\frac{1}{9}\,5^i$ for $i=0$, $1$, $2$, \ldots
\end{egg}

\begin{theorem}
Suppose that (\ref{basicFDP}) is of order $s$ and let
$\DS \{ u_i^{[k]} \}_{i=0}^\infty$ be the solution of the
homogeneous finite difference equation 
\begin{align*}
\sum_{j=0}^s\,a_j\,u_{i-j} &= 0 \quad, \quad i \geq s \\
u_i &= \delta_{k,i} \quad, \quad 0 \leq i < s
\end{align*}
for $0 \leq k < s$.  Then, the solution
$\DS \{ u_i \}_{i=0}^\infty$ of (\ref{basicFDP}) is given by
...
w_i =  \begin{cases}
\DS \frac{1}{a_0}\,\sum_{k=0}^{i-s}\,C_{s+k}\,u_{i-k-1}^{[s-1]} &
...
$\DS u_{i-j-k-1}^{[s-1]} = 0$.  To simplify the notation, we
assume that $\DS u_i^{[s-1]} = 0$ for $i<0$.  Hence,
\[
...
equality comes from 
$\DS \sum_{j=0}^s\,a_j\,u_{(i-k-1)-j}^{(s-1)} = 0$ 
for $i-k-1 \geq s$ because
$\DS \{ u_i^{(s-1)} \}_{i=0}^\infty$ is a solution of the homogeneous
...
$\DS w_{i+1} = \sum_{k=0}^m a_k w_{i-k}$.
If we substitute $\DS \lambda^i$ for $w_i$ in this expression, we get
$\DS \lambda^{i+1} = \sum_{k=0}^m a_k \lambda^{i-k}$. 
If we multiply both sides of this equation by
$\DS \lambda^{m-i}$, we get $p(\lambda) = 0$ for
$\DS p(\lambda) = - \lambda^{m+1} + \sum_{k=0}^m a_k \lambda^{m-k}$.
...
$\DS p(\lambda) = - \lambda^4 + \lambda^3$.  $1$ is a root of 
...
$\DS p(\lambda) = -\lambda^2 + 1$.  $1$ and $-1$  are the
two roots of this polynomial.  The method is weakly stable.
\end{enumerate}
\end{eggList}
...
We have that $\DS \{u_i\}_{i=0}^N$ is a solution of
(\ref{zzFDP}) if we set $\delta_i = u_i$ for $0\leq i \leq m$.
For $c \in \CC \setminus \RR$,
$\DS \{u_i\}_{i=0}^N$ is linear combination of the two solutions,
$\DS \{c^i\}_{i=0}^N$ and
$\DS \{\overline{c}^i\}_{i=0}^N$.
However,
\[
|u_N| =
\begin{cases}
...
Again, $\DS \{u_i\}_{i=0}^N$ is a solution of (\ref{zzFDP}) if we set
$\delta_i = u_i$ for $0\leq i \leq m$.
For $c \in \CC \setminus \RR$, $\DS \{u_i\}_{i=0}^N$ is linear
combination of the two solutions: 
$\DS \{i c^i\}_{i=0}^N$ and
$\DS \{i \overline{c}^i\}_{i=0}^N$ (Proposition~\ref{rootHO}).
However,
\[
|u_N| =
\begin{cases}
...
If we select an increasing sequence
$\DS \{N_j\}_{j=0}^\infty$ of positive
...
following claims:
\begin{enumerate}  
\item $\DS \{|c^j|/j\}_{j=1}^\infty$ does not converge to $0$ if
$c\in \RR$ satisfies $|c|>1$.
\item $\DS \{|c^j+\bar{c}^j|/j\}_{j=1}^\infty$ does not
converge to $0$ if $c\in \CC \setminus \RR$ satisfies $|c|>1$.
\item $\DS \{|c^j|\}_{j=1}^\infty$ does not converge to $0$
if $c\in \RR$ satisfies $|c| = 1$.
\item $\DS \{|c^j+\bar{c}^j|\}_{j=1}^\infty$ does not
converge to $0$ if $c\in \CC \setminus \RR$ satisfies $|c|=1$.
\end{enumerate}

The two cases where $c \in \RR$ are easy to prove because
$\DS |c|^j/j \to \infty$ as $j \to \infty$ when $|c| >1$, and
$\DS |c|^j = 1$ for all $j$ when $|c|=1$.

If $c \in \CC \setminus \RR$, we have
$\DS c = |c| e^{i\theta}$ for some
...
$\DS \{j_k\}_{k=1}^\infty$ of positive integers and a constant $C>0$
depending on $\theta$ such that $|\cos(j_k \theta)|\geq C$ for all $k$.

If $\DS \theta = \frac{m \pi}{n}$ for two positive integer
...
If $\DS \theta = \frac{m \pi}{2}$ for $m=1$ or $3$, then
...
need to use the fact that $\DS \{ e^{j\theta} \}_{j=0}^\infty$
is dense on the unit circle.  Thus, there exist an infinite strictly
increasing sequence $\DS \{j_k\}_{k=0}^\infty$ such that
$j_k \theta$ is between $\pi/6$ and $\pi/3$ modulo $2\pi$.  We then have
\[
  |\cos(j_k \theta)|
...
\]
Since $\DS \lim_{k\to \infty} \frac{|c|^{j_k}}{j_k} = \infty$
because $|c|>1$, we get that
$\DS \lim_{k\to \infty} \frac{|c^j_k+\bar{c}^j_k|}{j_k} = \infty$.
...
$\DS \{|c^{j_k}+\bar{c}^{j_k}|\}_{k=0}^\infty$ does not
converge to $0$.
\end{enumerate}
\label{cjdiverges}
\end{rmkList}
...
$\DS \{ \tilde{u}_i \}_{i=0}^N$, where $\tilde{u}_i = 0$ for
$0 \leq i \leq N$.
...
\end{cases}
\]
for $0 \leq i \leq N$.  We have that
$\DS \{u_i\}_{i=0}^N$ is a solution of
...
|u_N| =
\begin{cases}
\DS \delta\,|c|^N  & \quad \text{if}\quad c \in \RR \\
\DS \delta\,|c^N + \bar{c}^N| &
\quad \text{if}\quad c \in \CC\setminus \RR
\end{cases}
\]
does not converge to $0$ as $N \to \infty$.  There are strictly
increasing sequences $\DS \{N_j\}_{j=0}^\infty$ of positive integers
such that $\DS \{|u_{N_j}|\}_{j=0}^\infty$ converges to  $\infty$
...
$\DS \max_{0 \leq i \leq N} | \delta_i(h) | \leq
...
$\DS \delta(h) = O(h^2)$ in the statement of Theorem~\ref{Dahl_conv}.

Suppose that $\DS \left\{u_i\right\}_{i=0}^N$ is a
solution of
\[
...
make is that $\DS \max_{0 \leq i \leq N} | \sigma_i(h) | \leq
...
Let $\DS \left\{ u_i^{[k]} \right\}_{i=0}^\infty$ be the solution of
\begin{align*}
...
that $\DS | u_i^{[k]} | \leq Q$ for all $i$ and $k$.  Recall that all
...
form $\DS e_i = i^n c^i$, where $c$ is a root of the
...
|e_i| & \leq
\begin{cases}
\DS Q(m+1)\,\max_{0\leq j \leq m} |e_j| & \\
\DS \qquad + Q(i-m+1)\,h \bigg( R\,(m+2)\,\max_{0\leq j \leq i} |e_j|
+ \sigma(h) + \tau(h) \bigg) & \quad \text{if}\quad  m< i \leq N\\[1em]
\DS Q(m+1)\,\max_{0\leq j \leq m} |e_j| & \quad \text{if}
\quad 0\leq i \leq m
\end{cases}  \nonumber \\
& \leq
\begin{cases}
\DS Q(m+1)\,\max_{0\leq j \leq m} |e_j| & \\
\DS \qquad + Qih \bigg( R\,(m+2)\,\max_{0\leq j \leq i} |e_j|
+ \sigma(h) + \tau(h) \bigg) & \quad \text{if}\quad  m< i \leq N\\[1em]
\DS Q(m+1)\,\max_{0\leq j \leq m} |e_j| & \quad \text{if}
\quad 0\leq i \leq m
...
$\DS \max_{0\leq j \leq i} |e_j|$, we get
...
$\DS I_k = [t_0+k/(2M), t_0+(k+1)/(2M)]$ for $1 \leq k \leq K$
with the initial conditions at $t_j$ given by $e_j$ for
$\DS i_k-m \leq j \leq i_k$, we get
...
Let $\DS E_k = \max_{i_k \leq i \leq i_{k+1}} |e_i|$ for
...
$\DS E_{-1} = \max_{0\leq j \leq m} |e_j|
...
\sum_{j=-1}^m\,a_j\,e_{i-j} = C_i \quad , \quad  m \leq i < N \ ,
\]
where $a_{-1} = -1$, $\DS e_j = u_j^{[2]} - u_j^{[1]}$ for
$0 \leq j \leq N$ and
\[
...
Let $\DS \delta_i^{[j]}(h) = h \sigma_i^{[j]}(h)$ for $j=1$ and $2$.
Moreover, let
\[
...
\leq h \sigma(h)
\]
because $\DS u_i^{[1]} = y(t_i) + \delta_i^{[1]}(h)$ and
$\DS u_i^{[2]} = y(t_i) + \delta_i^{[2]}(h)$ for
$0\leq i \leq m$.  Thus
\[
E_k \leq \left(
...
$\DS \lim_{i\rightarrow +\infty} w_i = 0$ for all solutions
...
$\RE \mu <0$.  Hence, all solutions
$\DS y(t) = e^{\mu (t-t_0)}y_0$ of the
differential equation (\ref{RKThe_equ}) satisfy
$\DS \lim_{t\to \infty} y(t) = 0$.
\end{rmk}
...
\end{split}
\end{equation}
If we substitute $\DS \lambda^i$ for $w_i$, we get
\[
\lambda^{i+1} = \left( 1 + (\gamma_1 + \gamma_2)\mu\, h + \gamma_2
\beta_{2,1} (\mu\, h)^2 \right) \lambda^i
\]
and, after dividing by $\DS \lambda^i$, we get the only non-null root
\[
\lambda = \left( 1 + (\gamma_1 + \gamma_2)\mu\, h + \gamma_2
\beta_{2,1} (\mu\, h)^2 \right) \ .
\]
Hence, the general solution of (\ref{RKAstabequ}) is
$\DS w_i = \lambda^i w_0$ for $i=0$, $1$, $2$, \ldots\ From
the condition $\gamma_1 + \gamma_2 = 1$ and $\beta_{2,1} \gamma_2 = 1/2$
...
\end{equation}
to get $\DS \lim_{i\to \infty} w_i = 0$.
...
$z \in \CC$ such that $\DS 1 + z + z^2/2$ is on the unit circle
is the black curve shown in Figure~\ref{reg_abs_stab_RK2}.
To draw this black curve, we may use the fact that
$\DS 1 + z + z^2/2 = e^{i\theta}$ is a quadratic equation whose
solutions are given by $\DS z = -1 \pm \sqrt{-1+2 e^{i \theta}}$.
The number $i$ in the previous sentence is the complex number such
that $\DS i^2 = -1$ and not the index $i$ in the Runge-Kutta method.
...
$\DS \left| 1 + z + z^2/2 \right| = 1.2$, the red curve in
Figure~\ref{reg_abs_stab_RK2}, and the curve generated by the set of
points $z$ such that
$\DS \left| 1 + z + z^2/2 \right| = 0.8$, the blue
...
$\DS \left|1 + z + z^2/2 \right|=1.2$ and $0.8$ (red and blue curves
...
$\DS w_i = (r(h\mu))^i w_0$ for $0 \leq i \leq N$.
...
It is enough to show that $\DS \VEC{c}^\top(\Id-zB)^{-1}\VEC{u}$ is a
...
the {\bfseries cofactor} $\DS (-1)^{i+j} \det A_{i,j}$,
where $A_{i,j}$ is obtained from $\Id - zB$ by removing the
$\DS i^{th}$ row and $\DS j^{th}$ column.
The cofactors are polynomials in $z$ of degree at most $s-1$.
Hence, $\DS \VEC{c}^\top(\Id-zA)^{-1}\VEC{u}$ is the quotient
...
+ |h \tau_{i+1}(h)| + |\delta_i| \\
&\qquad = |r(h\,u)|^{i+1} |y_0-u_0| + \sum_{j=0}^{i-1}
|r(h\mu)|^{j+1} \big(|h\tau_{i-j}(h)| + |\delta_{i-1-j}|\big)
+ |h\tau_{i+1}(h)| + |\delta_i| \\
&\qquad = |r(h\,u)|^{i+1} |y_0-u_0| + \sum_{j=1}^i
|r(h\mu)|^j \big(|h\tau_{i+1-j}(h)| + |\delta_{i-j}|\big)
+ |h\tau_{i+1}(h)| + |\delta_i| \\
&\qquad = |r(h\,u)|^{i+1} |y_0-u_0| + \sum_{j=0}^i
|r(h\mu)|^j \big(|h\tau_{i+1-j}(h)| + |\delta_{i-j}|\big)
\end{align*}
...
$\DS \max_{0\leq i <N} |\tau_{i+1}(h)| \leq \tau(h) \to 0$
...
order two, we have $\DS r(h\mu) = 1 + h\mu + (h\mu)^2/2$.  Then
\begin{align*}
1 - |r(h\mu)|
...
$\DS \frac{h\tau(h)}{1-|r(h\mu)|} + \frac{\delta}{1-|r(h\mu)|}$.
\end{rmk}
...
\]
So
$\DS B= \begin{pmatrix} 1/4 & -1/4 \\ 1/4 & 5/12 \end{pmatrix}$
and
$\DS \VEC{c}= \begin{pmatrix} 1/4 \\ 3/4 \end{pmatrix}$.  Thus,
$\DS \Id-z B
= \begin{pmatrix} 1-z/4 & z/4 \\ -z/4 & 1-5z/12 \end{pmatrix}$
and 
\begin{align*}
...
$|r(z)| < 1$ for $\DS z=\rho e^{i\theta}$ with $\rho>0$ and
...
$\DS 2\rho (1+\rho^2/9) >0$, $\cos(\theta) <0$ and
$\DS 2\rho^2 \cos^2(\theta)/3 + \rho^4/36 > 0$), the
...
\]
are the roots of $\DS 1-2z/3+z^2/6$; namely,
...
of order $p$, then $\DS r(z) = e^z +O(z^{p+1})$ as $z \rightarrow 0$.
\label{exp_approx}
\end{lemma}

\begin{proof}
By definition of order,
$\DS y(t_{i+1}) = y(t_i) + h\,\phi(t_i,y(t_i)) + O(h^{p+1})$ where
...
of (\ref{RKThe_equ}) is $\DS y(t) = e^{t\mu}w_0$.  Thus,
\[
...
by the Runge-Kutta method is
$\DS w_i = (r(h\lambda))^iw_0$ for $i\geq 0$,
...
$\DS e^z$ of order $2k$.  From Wanner-Hairer-Norsett
...
\]
If we subtract $\DS \lambda^{i+1}$ from both sides of this equality
and multiply them by $\DS \lambda^{m-i}$, we get
\[
...
$\DS 1 = \sum_{i=0}^m\,a_i$ (i.e.\ $p(1) = 0$) if the
...
\]
for $m \leq i < N$.  Since $\DS y(t) = A e^{\mu\, t}$ is the
general solution of $y' = \mu\,y$,
\[
...
the roots of the stability polynomial must approximate
$\DS e^{\mu h}$ for $h$ small.  This root is called the
...
such that $\DS \lim_{i\rightarrow +\infty} w_i = 0$ for all
solutions $\DS \{w_i\}_{i=0}^\infty$ of the difference
...
$\DS y(t) = e^{\mu(t-t_0)}y_0$ of the differential equation
(\ref{RKThe_equ}) satisfy $\DS \lim_{t\to \infty} y(t) = 0$.
\end{rmk}
...
The exact general solution of $y'= 1-2y$ is
$\DS y(t) = c e^{-2t} +1/2$.
...
\]
are the roots of $\DS \lambda^2 + 4h\lambda -1 =0$, and
$c_1$ and $c_2$ are arbitrary constants.  Note that $w_i= 1/2$ for all
$i$ is a solution of the non-homogeneous equation (\ref{finitdiffSOAB}) and
$\DS w_i =c_1\lambda^i + c_2\lambda_2^i$ is the general solution of
...
\]
where $\DS \lim_{h\to 0} (1-2h)^{-1/(2h)} = e$.  Thus, for
$h$ very small, we may assume that
$\DS (1-2h)^i \approx e^{-2 i h} = e^{-2t_i}$.
Similarly, $\DS (1+2h)^i \approx e^{2 t_i}$ for $h$ very small.
...
mathematically but the theory assumes that $\DS \delta_i(h) = O(h^2)$
...
$\DS \{w_i\}_{i=0}^\infty$ of this finite difference
equation is a linear combination of solutions of the form
$\DS \{i^n\lambda^i\}_{i=0}^\infty$,
...
if and only if any non-trivial solution $\DS \{w_i\}_{i=0}^\infty$ of
the finite difference equation above
satisfies $\DS \lim_{i\rightarrow +\infty} w_i = 0$ .  Namely,
...
solution is $\DS y(t) = y_0\,e^{\mu t}$.  The approximation $w_i$ of
...
\end{align*}
Thus, $\DS w_i = (1+h\mu)^iy_0$ for $i\geq 0$.
...
$\DS (1+h\mu)^i y_0 + (1+h\mu)^i \delta_0$ which differ from the
unperturbed value of $w_i$ by $\DS (1+h\mu)^i \delta_0$.  To have this
...
we may assume that $\DS \lambda = e^{\theta i}$ for some
$\theta \in [0,2\pi[$.  We get
\[
...
outside this segment are on curves given by
$\DS \lambda = r e^{\theta i}$
...
$\DS \lambda = e^{i\theta}$ (black curve), for
$\DS \lambda = 1.1 e^{i\theta}$
and $\DS \lambda = 1.5 e^{i\theta}$ (red curves) and
$\DS \lambda = 0.8 e^{i\theta}$ (blue curve).  The region of absolute
...
$z = - p(\lambda)/q(\lambda)$ for $\DS \lambda = e^{i\theta}$ with
$0\leq \theta < 2\pi$.   For the Adams-Bashforth method
of order four $\DS p(\lambda) = -\lambda^4 + \lambda^3$ and
$\DS q(\lambda) = (55\lambda^3-59\lambda^2+37\lambda - 9)/24$.
...
$\DS \lambda = e^{i\theta}$ with $0\leq \theta < 2\pi$.
For the Adams-Moulton method of order four
$\DS p(\lambda) = -\lambda^3 + \lambda^2$ and
$\DS q(\lambda) = (9\lambda^3+19\lambda^2-5\lambda + 1)/24$.
...
$\DS \lambda = 1.2 e^{i\theta}$ and
$\DS \lambda = 0.8 e^{i\theta}$ with
$0\leq \theta < 2\pi$ for the two methods.  The points $z$ on the
curves in red associated to $\DS \lambda = 1.2 e^{i\theta}$ correspond
...
in blue associated to $\DS \lambda = 0.8 e^{i\theta}$ correspond to
...
$z = - p(\lambda)/q(\lambda)$ for $\DS \lambda = e^{i\theta}$ (black
curve), for $\DS \lambda = 1.2 e^{i\theta}$ (red curve) and
$\DS \lambda = 0.8 e^{i\theta}$ (blue curve).  The region to
the left of the imaginary axis and inside the black curve is the
region of absolute stability.}{stab_region1}
...
$\DS \lambda = 0.8 e^{i\theta}$ (blue curve).  The region to
the left of the imaginary axis and inside the black curve is the
region of absolute stability.}{stab_region2}
...
$\DS p(\lambda) = -\lambda^2 + \lambda$ and
$\DS q(\lambda) = (\lambda^2 + \lambda)/2$.
...
$\DS \lambda = e^{i\theta}$ (black curve), for
$\DS \lambda = 1.2 e^{i\theta}$
(red curve) and $\DS \lambda = 0.8 e^{i\theta}$ (blue
curve).  The region to the left of the imaginary axis is the region of
absolute stability.}{trap_stable}
...
$\DS \left| \frac{1+h\mu(t_i)/2}{1-h\mu(t_{i+1})/2}
...
$\DS y(t) = 1-t^2+C e^{100t}$.  The initial condition $y(0)=1$ implies
...
number of subinterval $N=10$, the tolerance $\DS T=10^{-5}$ and the
...
Consider the quadratic equation $\DS p(z) = az^2+bz+c$,
where $a,b,c \in \CC$ and $a\not=0$.  Then, the roots of $p$ are
inside the closed disk of radius $1$ centred at the origin if and only if
$|a| \geq|c|$ and
$\DS \big| |a|^2-|c|^2\big| \geq |a\overline{b}-b\overline{c}|$
\label{CSCrit}
\end{lemma}
...
$\DS \{ z \in \CC : z = \rho\,e^\theta \ \text{with}\ \rho>0
\ \text{and}\ |\theta-\pi|<\alpha \}$.  For some multistep methods, $\alpha$ may
be closed to $\pi/2$.
\end{rmk}
...
\end{split}
\end{equation}
where $\DS f:[t_0,t_f]\times\RR^n \rightarrow \RR^n$.
...
\end{split}
\end{equation}
where $\DS \VEC{y}:\RR\rightarrow \RR^2$,
$\DS \VEC{y}_0 =  \begin{pmatrix} y_{0,1} \\ y_{0,2}
\end{pmatrix} \in \RR^2$, and
$\DS A = \begin{pmatrix} -\lambda & 1 \\
0 & -0.1 \end{pmatrix}$ for a large positive number $\lambda$.

Since $\DS A = Q B Q^{-1}$, where
$\DS Q=\begin{pmatrix} 1 & 1 \\
0 & \lambda -0.1 \end{pmatrix}$ and
$\DS B=\begin{pmatrix} -\lambda & 0 \\
0 & -0.1 \end{pmatrix}$, the solution of (\ref{stiff_syst}) is
\begin{equation}\label{sol_stiff_syst}
\VEC{y} = e^{tA}\VEC{y}_0 = Q e^{tB} Q^{-1} \VEC{y}_0
= \begin{pmatrix}
e^{-\lambda t} &
\DS (e^{-0.1 t}-e^{-\lambda t})/(\lambda-0.1) \\
0 & e^{-0.1 t}
\end{pmatrix}
\begin{pmatrix}
y_{0,1} \\ y_{0,2}
...
\end{equation}
The solution
$\DS \{\VEC{w}_j\}_{j=0}^\infty$ of (\ref{stiff_Euler}) is
given by
...
of $\DS \diff_{\VEC{y}} f(t,\VEC{y}(t))$ and
$\DS \max_i \{ \RE \lambda_i(t) \} \gg
...
\]
Since $\DS A = Q B Q^{-1}$ for
$\DS Q=\begin{pmatrix} 1 & 1 \\
0 & \lambda -0.1 \end{pmatrix}$ and
$\DS B=\begin{pmatrix} -\lambda & 0 \\
0 & -0.1 \end{pmatrix}$, we get
\[
...
&= Q
\begin{pmatrix}
\left(\DS \frac{1-\lambda h/2}{1+\lambda h/2}\right)^{i+1}
& 0 \\
0 &                                                                     
\left(\DS \frac{1- h/20}{1+ h/20}\right)^{i+1}
\end{pmatrix}
Q^{-1} \VEC{w}_0 \ .
\end{align*}
We finally get
...
\\
0 & \left(\DS \frac{1- h/20}{1+ h/20}\right)^{i+1}
\end{pmatrix} \VEC{w}_0 \ , \\
&\hspace{30em} 0 \leq i < N \ .
\end{align*}
Independently of the choice of $h$, we have that
$\VEC{w}_i \rightarrow \VEC{0}$ as $i \rightarrow \infty$ because
$\DS \bigg| \frac{1-\lambda h/2}{1+\lambda h/2} \bigg| < 1$
and
$\DS \bigg| \frac{1-h/20}{1+h/20} \bigg| < 1$
for all $h$.
...
\begin{center}
\begin{tabular}{*{1}{l@{\hspace{0.4em}}l@{\hspace{2em}}}l@{\hspace{0.4em}}l}
\subQ{a} &
$\DS \begin{array}{rl}
y'(t) &= 2y(t)+2 \ , \ 0 \leq t \leq 1 \\
y(0) &= 1
\end{array}$ &
\subQ{b} &
$\DS \begin{array}{rl}
y'(t) &= t^2 y(t) + 1 \ , \ 0 \leq t \leq 1 \\
y(0) &= 1
\end{array}$  \\[1em]
\subQ{c} &
$\DS \begin{array}{rl}
y'(t) &= t^2 \sin(y(t)) + y(t) \ , \ 0 \leq t \leq 1 \\
y(0) &= 1
\end{array}$ & &
\end{tabular}
\end{center}
...
\sol{\SOL}{
\subQ{a}
Let $\DS f(t,y) = t^2 \sin(y) + y$.  The function $f$ is
...
\]
Since $\DS |t^2 \cos(y) + 1| \leq 2$ for all
...
have magnitude less than $\DS 10^{-8}$. \\
...
We have to minimize
$\DS g(h) = \frac{h}{2} + \frac{10^{-8}}{h}$
for $h>0$.  We deduce from the following information about $g$ that it
has a global minimum for $h>0$ at $\DS h=\sqrt{2}/10^4$.
\[
\begin{array}{c|c|c|c}
...
\end{array}
\]
Thus, $\DS h = \sqrt{2}/10^4$ minimizes the error bound for the Euler
Method.

\subQ{c} We use the Euler Method with $t_0=1$, $t_f= 2$,
$y_0 = 0$ and $f(t,y) = 1 + y/t$.  Since
$\DS (t_f-t_0)/h = 10^4/\sqrt{2} \approx 7071.0678$
...
$\DS |e_{7072}| = |u_{7072} - y_{7072}| \approx 0.707 \times 10^{-4}$.
...
$\DS u(t) = \frac{1}{t-1}$ for $t\neq 1$.  Thus, the
solution of (\ref{rgktquest1}) is
$\DS y(t) = t - \frac{1}{t-1}$ for $t\neq 1$.

We have $t_0=2$, $t_f= t_{10} =3$, $y_0=1$ and
$\DS f(t,y) = 1 +(t-y)^2$.
...
where
$\DS K_1 = 1 +(t_i-w_i)^2$,
$\DS K_2 = 1 +\big((t_i+0.05) - (w_i +0.05 K_1)\big)^2$,
$\DS K_3 = 1 +\big((t_i + 0.05) - (w_i+ 0.05 K_2)\big)^2$ and
$\DS K_4 = 1 +\big(t_{i+1}- (w_i + 0.1 K_3) \big)^2$.
...
$\gamma(\tau) = 1$ and
$\DS \Psi(\tau) = \sum_{j=1}^2 \gamma_j = 1$.  So
$\Psi(\tau) = 1/\gamma(\tau)$.
...
$\gamma(\tau) = 2$ and
$\DS \Psi(\tau) = \sum_{j=1}^2 \gamma_j \alpha_j =
...
$\DS \beta - \beta^2 = \frac{1}{6}$; namely, only if
$\DS \beta = \frac{1}{2} \left(1 \pm \frac{1}{\sqrt{3}}\right)$.
\end{enumerate}
}
...
\]
Since
$\DS y(t_{i+1}) = y(t_i) + y'(t_i)\,h + y''(t_i)\,\frac{h^2}{2} +
y'''(\xi_i) \, \frac{h^3}{3!}$, $f(t_i,y(t_i)) = y'(t_i)$
and
$\DS f(t_{i+1},y(t_{i+1})) = y'(t_{i+1})
...
\end{align*}
If $\DS M= \max_{t_0 \leq t \leq t_f} | y'''(t) |$, we get
\[
| \tau_{i+1}(h) | \leq
|y'''(\xi_i)| \frac{h^2}{3!}  + |y'''(\eta_i)| \frac{h^2}{4}
\leq \frac{5}{12} M h^2
...
$\DS \tau_{i+1}(h) = O(h^2)$.  The method is consistent because
\[
...
\]
where
$\DS \tilde{K}_1 = f(\tilde{t}_i,\tilde{y}_i)$,
$\DS \tilde{K}_2 = f(t_{2i+1},y_{2i+1})
= f\left(\tilde{t}_i+ \frac{\tilde{h}}{2}, \tilde{y}_1
+ \frac{\tilde{h}}{4} ( \tilde{K}_1 + \tilde{K}_2) \right)$ and \\
$\DS \tilde{K}_3 = f(t_{2i+2},y_{2i+2})
...
$\DS t^2 - t + 1/6$, then the $2$-stage Runge-Kutta Method
given by the collocation method is of order $4$.  More details are given in
...
If $\DS q(t) = t^{n-1}$ with $1 \leq n \leq k$, then
(\ref{GradQ7A}) yields
\[
...
$\DS q(t) = \sum_{m=0}^{k-1} a_m t^m$, then
\begin{align*}
...
$\DS q(t) = \sum_{j=1}^k q(\alpha_j)\, \ell_j(t)$, where
...
$\DS a_{n+1,j} = \alpha_j^n$ for $0 \leq n < k$ and
$1 \leq j \leq k$.  Since the $\alpha_j$ are distinct, $A$ is a
non-singular matrix.  In fact, $A$ is a invertible Vandermonde matrix.
For $1 \leq i \leq k$, Let $\DS \VEC{w}^{[i]}$ be
the vector with components
$\DS w^{[i]}_j = \beta_{i,j} - \int_0^{\alpha_i} \ell_j(t) \dx{t}$
for $1 \leq j \leq k$.  Moreover, let $\DS \VEC{w}^{[k+1]}$
be the vector with the components 
$\DS w^{[k+1]}_j = \gamma_j - \int_0^1 \ell_j(t) \dx{t}$
for $1 \leq j \leq k$.

We have that (\ref{GradQ7C}) with $\DS q(t) = t^n$ for $0 \leq n < k$
yields the linear equations $\DS A \VEC{w}^{[i]}  = \VEC{0}$ for
$0 \leq i \leq k$.  Similarly, (\ref{GradQ7D}) with
$\DS q(t) = t^n$ for $0 \leq n < k$ yields the
linear equations $\DS A \VEC{w}^{[k+1]}  = \VEC{0}$.  Since $A$ is
...
\]
we have that $\DS \VEC{c}^\top(\Id-zA)^{-1}\VEC{u}$ is the quotient
...
$\DS \det(\Id-zB) = \prod_{j=1}^k (1- z \beta_{j,j})$.
}
...
polynomial $\DS q(t) = t^2 -t + 1/6$.  See
Example~\ref{Gauss_Leg_RK}.  It follows from
Theorem~\ref{RKcolAstable} that the method is A-stable.
...
\]
where $\DS r(z) = 1 + z\VEC{c}^\top (\Id - zB)^{-1} \VEC{u}$ with
\[
...
$\DS 12 - 6z + z^2$; namely,
$z_{\pm} = 3 \pm \sqrt{3} \, i$.  Thus, there is
...
$\DS w_i = A i^2 + B i$ for some constants $A$ and $B$.\\
...
$\DS w_i = r^i$, we get $\DS r^{i+1} = r^i$.  The
nontrivial solution is $r = 1$.  Thus, the general solution of the
linear difference equation is $w_i = C$, a constant, for all $i$.
...
\]
for all $i$.  Thus, $\DS 2A-h^2 = 0$ and
$\DS A + B - 5h^2/12 = 0$.  Solving for $A$ and $B$,
we find $\DS A = h^2/2$ and $\DS B = -h^2/12$.  Hence,
$\DS w_i = \frac{h^2 i^2}{2} - \frac{h^2i}{12}$ for all $i$.

The general solution of (a) is
$\DS w_i = \frac{h^2 i^2}{2} - \frac{h^2i}{12} + C$ for all $i$.

\subQ{c} With $w_0=0$, we get $\DS C = 0$.
Thus, $\DS w_i = \frac{h^2 i^2}{2} - \frac{h^2i}{12}$ for
all $i$.

\subQ{d} Since $t_i = hi$, we find
$\DS w_i = \frac{t_i^2}{2} - \frac{h t_i}{12}$.
...
$\DS p(w) = \lambda^2 - \lambda$.  It has two roots, $\lambda = 0$ and
...
\subQ{a} We consider $\DS p(w) = w^3-1$ and set $m=3$.  Using the
...
\end{align*}
Thus $\DS q(w) = \frac{3}{4} + \frac{9w^2}{4}$.  The
multistep method is
\begin{align*}
...
\subQ{b} We consider $\DS p(w) = w^3-1$ and set $m=3$.  Using the
substitution $w=v+1$, we get
\begin{align*}
\frac{p(w)}{\ln(w)} &= \frac{w^3-1}{\ln(w)}
= \frac{(v+1)^3-1}{\ln(v+1)}
...
\ .
\end{align*}
Thus $\DS q(w) = \frac{3}{8} + \frac{9w}{8} + \frac{9w^2}{8}
...
$\DS y(t_{i+1}) = \sum_{k=0}^5 \frac{1}{k!}\, y^{(k)}(t_i)\,h^k
+ \frac{1}{6!}\,y^{(6)}(\xi_i)\,h^6$,\\
$\DS y(t_{i-1}) = \sum_{k=0}^5 \frac{(-1)^k}{k!}\, y^{(k)}(t_i)\,h^k
...
$\DS b_1 = \frac{a_1}{2} - \frac{1}{2}$ from
$\DS \frac{1}{2} - \frac{a_1}{2} + b_1 = 0$
(we set the coefficient of $y''(t_i)h$ to $0$).
If we substitute this expression for $b_1$ in
$\DS \frac{1}{6} + \frac{a_1}{6} - \frac{b_1}{2} = 0$
(we set the coefficient of $\DS y^{(3)}(t_i) h^2$ to $0$), we
get that $\DS a_1 = 5$.

We have found that all the terms in $\DS h^{k}$ for $k<3$ vanish if
$a_0 = -4$, $a_1 = 5$. $b_0 = 4$ and $b_1 = 2$.
With these values of $a_1$ and $b_1$, we have that
$\DS \frac{1}{24} - \frac{a_1}{24} + \frac{b_1}{6}
= \frac{1}{6} \neq 0$ (the coefficient of $\DS y^{(4)}h^3$).  Hence,
...
$\DS \tau_{i+1}(h) = O(h^3)$.  So, the method is of order $3$.
...
$\DS p(\lambda) =  -\lambda^{m+1} + \sum_{j=0}^m\,a_j\,\lambda^{m-j}$
is the characteristic polynomial
and $\DS q(\lambda) = \sum_{j=-1}^m\,b_j\,\lambda^{m-j}$.
