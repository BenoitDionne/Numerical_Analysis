...
%
%   We use Gauss-Siedel Iterative Method, to find a solution of  A x = b.
%
...
%     is satisfied:
%     The total number of iterations is greater than  limit  or the
%     difference between two successive iterations is
...
function xx = gausssiedel(A,b,x,tol,limit)
  ...
  for k = 1:limit
    ...
    if dim > 2
      for m = 2:dim-1
        xx(m,1) = (b(m,1) - A(m,1:m-1)*xx(1:m-1,1) - A(m,m+1:dim)*x(m+1:dim,1))/A(m,m);
      end
    end
