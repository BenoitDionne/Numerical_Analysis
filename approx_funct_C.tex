\chapter{Least Square Approximation (in $\displaystyle \ell^2$)}
\label{ChaptApproxC}
...
$\displaystyle y=p(x)= ax^2+bx+c$ or
...
notion of $\displaystyle L^2$ approximation that we will study in the
next sections.
\end{enumerate}
...
\left(\sum_{i=1}^n x_i\right)^2} \ .
\end{align*}
Since $\displaystyle I:\RR^2\rightarrow [0,\infty[$ is a quadratic polynomial
...
$\displaystyle p(x) = b e^{ax}$ (or
$\displaystyle p(x) = b x^a = b e^{a\ln(x)}$).
...
$I$.  Thus, $\displaystyle y= be^{ax}$ may not be a ``best fit'' for
...
Finally, since $\displaystyle I:\RR^{2N+1} \rightarrow [0,\infty[$ is a
...
satisfying $\displaystyle i^2=-1$.  In particular, we will consider
...
\begin{prop}
The set $\displaystyle S = \{ e^{kx i}\}_{k\in \ZZ}$ satisfies
\[
\pps{e^{kx i}}{e^{j x i}} =
\begin{cases}
...
knowledge of Fourier series of complex valued functions in $\displaystyle L^2$.
...
$\displaystyle L^2$-integrable
function on $[0,2\pi]$, then we may write
\[
f = \sum_{k\in\ZZ} A_k e^{k x i} \ ,
\]
where the convergence is the $\displaystyle L^2$ convergence and
\[
...
\item Since $\displaystyle e^{-K x_n i} = e^{K x_n i}$ for all $n$
...
\]
In particular, if $\displaystyle g(x) = f(x) e^{-k x i}$ with $f$ a
...
$\displaystyle f(x) = \sin^2(x)$ at $x_n = 2n\pi/3$ for $n=0$, $1$ and $2$.
...
$\displaystyle f^{(j)}$ piecewise
continuous, one can prove that
$\displaystyle |A_k| = O(|k|^{-j-1})$ and that this implies
...
of period $N$ and let $\omega_N$ be the $\displaystyle N^{th}$ root of
unity defined by $\displaystyle \omega_N = e^{2\pi i/N}$.  The
...
$\displaystyle \FF_N^{-1}: \Pi_N \rightarrow \Pi_N$ such
that $\displaystyle \fct{x} = \FF_N^{-1} \fct{y}$ is defined by
\[
...
$\displaystyle \fct{y}^\ast = \FF^\ast_N \fct{x}$ is given by
\[
\fct{y}^\ast(n) \equiv \sum_{k=0}^{N-1} \omega_N^{-nk} \fct{x}(k)
\]
for $n \in \ZZ$.  In many books, $\displaystyle \FF^\ast_N$ is used as
...
$\displaystyle \fct{y}^\ast = \FF^\ast_N \fct{x}$.
...
$\displaystyle \fct{y}^\ast_{A_k P_k,q} \in \Pi_{B_k}$ by
\[
...
\end{equation}
for $b \in \ZZ$.
\item $\displaystyle \fct{y}^\ast_{A_k P_k, q}$ can be used to define
the function 
\begin{align*}
...
\end{align*}
because $\displaystyle \omega_{P_kB_k}^{-mP_kB_k} = 1^{-m} = 1$ for
all $m\in \ZZ$.  Thus
\[
\sum_{s=0}^{P_k-1} \fctt{y}_k(a+A_k s, b) \omega_{P_k B_k}^{-s(b+B_k p)}
...
$\displaystyle \fct{y}^\ast= \FF^\ast_N \fct{x}$ which is
denoted \verb!Z! in the code below.  As for the input, only the
components $\displaystyle \fct{y}^\ast(j)$ for $0 \leq j < N$ are returned
because of the periodicity of $\displaystyle \fct{y}^\ast$.
\small
\begin{verbatim}
function Z = FFT(X,MP)
  m = length(MP);
...
$\displaystyle \omega_{P_k B_k}^{-(b+B_k p)}$ to evaluate the expression
...
in the code above to compute $\displaystyle \FF^\ast_N \fct{x}$.  The evaluation
of omega and copying data has been ignored when computing the number
of operations.  The number of operations to compute
$\displaystyle \FF^\ast_N\fct{x}$ directly from the definition is
about $\displaystyle N^2$.
This is much larger than $N(P_1+P_2+\ldots+P_m)$ in general.
\item When $\displaystyle N=2^m$, an efficient Fast Fourier Transform
...
$\displaystyle \omega_{2^m}^{2n} = \omega_{2^{m-1}}^n$ to
get the fourth equality.  Moreover, since
$\displaystyle \omega_{2^m}^{-j-2^{m-1}} = -\omega_{2^m}^{-j}$ for
$\displaystyle 0 \leq j < 2^{m-1}$, we get that
\begin{align*}
\FF^\ast_N \fct{x}(n)
...
\omega_{2^m}^{-n} \, \FF^\ast_{2^{m-1}} \fct{x}_{o}(n)
\end{align*}
for $\displaystyle 0 \leq n < 2^{m-1}$.  The last equality, comes from
the fact that $\displaystyle \FF^\ast_{2^{m-1}} \fct{x}_{e}$ and
$\displaystyle \FF^\ast_{2^{m-1}} \fct{x}_{o}$ are of period
$\displaystyle 2^{m-1}$ because $\fct{x}_{e}$ and $\fct{x}_{o}$ are of
period $\displaystyle 2^{m-1}$.  This gives the following simple algorithm.
...
components $\displaystyle \fct{z}^\ast(j)$ for $0 \leq j < N$ are
returned because of the periodicity of $\displaystyle \fct{z}^\ast$.
\small
\begin{verbatim}
