\chapter{Least Square Approximation (in $\DS \ell^2$)}
\label{ChaptApproxC}
...
$\DS I(a,b) = \max_{1\leq i \leq n} \left| y_i - (ax_i+b) \right|$.
\item[Absolute Deviation]: Find $a$ and $b$ that minimize
$\DS I(a,b) = \sum_{i=1}^{n} \left| y_i - (ax_i+b)\right|$.
\item[Least Square]: Find $a$ and $b$ that minimize
$\DS I(a,b) = \sum_{i=1}^{n} \left( y_i - (ax_i+b)\right)^2$.
\end{description}

Instead of a straight line, we may have assumed that the data in
Figure~\ref{appr_DiscrLeastSqr} represent a parabola
$\DS y=p(x)= ax^2+bx+c$ or
...
notion of $\DS L^2$ approximation that we will study in the
next sections.
\end{enumerate}
...
\[
\begin{pmatrix}
\DS \sum_{i=1}^n x_i^2 & \DS \sum_{i=1}^n x_i \\
...
=
\begin{pmatrix}
\DS \sum_{i=1}^n x_i y_i \\ \DS \sum_{i=1}^n y_i
\end{pmatrix} \ .
\]
The solution of this system is
\begin{align*}
a &= \frac{\DS  n \sum_{i=1}^n x_i y_i -
\left(\sum_{i=1}^n x_i\right)\left( \sum_{i=1}^n y_i\right)}
{\DS n \left(\sum_{i=1}^n x_i^2\right) -
\left(\sum_{i=1}^n x_i\right)^2}
\intertext{and}
b &= \frac{\DS  \left(\sum_{i=1}^n x_i^2\right)
\left(\sum_{i=1}^n y_i\right) - \left(\sum_{i=1}^n x_i y_i\right)
\left(\sum_{i=1}^n x_i\right)}
{\DS n \left(\sum_{i=1}^n x_i^2\right) -
\left(\sum_{i=1}^n x_i\right)^2} \ .
\end{align*}
Since $\DS I:\RR^2\rightarrow [0,\infty[$ is a quadratic polynomial
...
$\DS p(x) = \sum_{j=0}^m a_j x^j$, then we must minimize
\[
...
\begin{align*}
a &= \frac{\DS  n \sum_{i=1}^n x_i \ln(y_i) -
\left(\sum_{i=1}^n x_i\right)\left( \sum_{i=1}^n \ln(y_i)\right)}
{\DS n \left(\sum_{i=1}^n x_i^2\right) -
\left(\sum_{i=1}^n x_i\right)^2}
\intertext{and}
\ln(b) &= \frac{\DS  \left(\sum_{i=1}^n x_i^2\right)
...
$I$.  Thus, $\DS y= be^{ax}$ may not be a ``best fit'' for
the set of data as we will expect with the least square method.

\section[Trigonometric Polynomial Approximation (Real Case)]{Trigonometric Polynomial Approximation (Real\\ Case)}

Suppose that
$\DS \left\{ (x_n,y_n) : n=0,1,2,\ldots, 2N-1\right\}$ are $2N$
data points, where
$\DS x_n = \frac{n \pi}{N}$ for $n=0$, $1$, $2$,\ldots, $2N-1$.
...
= \left(\frac{\DS 1- \left(e^{(r \pi/N)i}\right)^{2N}}
{1 - e^{(r \pi/N)i}}\right) = 0
\]
because $\DS \left(e^{(r \pi/N)i}\right)^{2N} = e^{2 r \pi i} = 1$
whatever $r$, and $\DS e^{(r \pi/N)i} \neq 1$ since $r$ is
...
$\DS \sum_{n=0}^{2N-1} \cos(2r x_n) = 0$.  Hence
\[
...
= N \ .
\]
A similar proof gives $\DS \sum_{n=0}^{2N-1} \sin^2(r x_n) = 1$.
\end{proof}

\begin{prop}
Suppose that $K\leq N$.  Let $\DS S_K =\{\phi_k\}_{k=0}^{2K}$,
where $\DS \phi_0(x) = 1/2$,
$\DS \phi_{2k}(x) = \cos(k x)$ and
$\DS \phi_{2k-1}(x) = \sin(k x)$ for $k=1$, $2$, \ldots, $K$.
...
\end{equation}
Namely, $\DS \pps{\phi_k}{\phi_j} = 0$ for $k\neq j$.
...
Finally, since $\DS I:\RR^{2N+1} \rightarrow [0,\infty[$ is a
...
$\DS \ps{f}{g} = \int_0^{2\pi} f(x)g(x) \dx{x}$ for $f$ and $g$
...
$\DS I(a_0, \ldots, b_K) = \int_{-\pi}^\pi (f(x)-p(x))^2 \dx{x}$,
...
$\DS e^{k x i}$ for $k\in \ZZ$, where $i$ is the complex number
satisfying $\DS i^2=-1$.  In particular, we will consider
...
Suppose that $\DS \left\{ (x_n,y_n) : n=0,1,2,\ldots, N-1\right\}$
are $N$ data points, where
$\DS x_n = \frac{2n \pi}{N}$ for $n=0$, $1$, $2$,\ldots, $N-1$.
...
= \frac{\DS 1- \left(e^{(2\pi(k-j)/N)i}\right)^{N}}
{1 - e^{(2\pi(k-j)/N)i}} = 0
\]
because
$\DS \left(e^{(2\pi(k-j)/N)i}\right)^{N} = e^{2(k-j)\pi i} = 1$
and $\DS e^{(2\pi(k-j)/N)i} \neq 1$ since
...
\]
because $\DS e^{(2n\pi(k-j)/N)i} = 1$ since $k-j$ is a
multiple of $N$.
\end{proof}
...
$\DS \pps{f(x)}{e^{K x i}} = \pps{f(x)}{e^{-K x i}}$
for $K=N/2$ because
\[
...
knowledge of Fourier series of complex valued functions in $\DS L^2$.
...
advantage of linking the discrete least square method above to
$\DS L^2$
...
We saw in Chapter~\ref{chaptApproxA} that if $f$ is an
$\DS L^2$-integrable
function on $[0,2\pi]$, then we may write
\[
f = \sum_{k\in\ZZ} A_k e^{k x i} \ ,
\]
where the convergence is the $\DS L^2$ convergence and
\[
...
that $\DS \pps{f}{e^{j_1 x i}} = \pps{f}{e^{j_2 x i}}$ for
...
$\DS e^{2\pi s n i} = 1$ for all $n$.  Thus
\[
...
$\DS \sum_{k\in \ZZ} A_k e^{k x i}$ to $f(x)$ for all $x$.
The last equality comes from
Proposition~\ref{approx_no_orth_compl_trig} and the absolute
convergence of the series $\DS \sum_{k\in \ZZ} A_k$.
\end{proof}
...
we have used $\DS e^{jx_n i} =  e^{k x_n i}$ for
...
$\DS N \bigg(\sum_{\substack{-N/2<k<-K\\K<k\leq N/2}}
...
$b_k=a_k$ for $|k| < K$ and $\DS b_{-K}=b_K = \frac{a_K}{2}$.
...
$\DS S_N= \left\{ e^{kx i} \right\}_{|k|\leq K}$ is a orthogonal set
...
$\DS I(r_{-K},r_{-K+1}, \ldots, r_K) = \pps{f-p}{f-p}$,
...
\item Since $\DS e^{-K x_n i} = e^{K x_n i}$ for all $n$
when $K=N/2$, it follows from the proof of the previous theorem that
\[
p(x) = \sum_{-N/2<k\leq N/2} a_k e^{k x i}
\]
minimizes $\DS \sum_{n=0}^{N-1} \left| y_n - p(x_n) \right|^2$
among all trigonometric polynomials of the form \\
$\DS \sum_{|k|\leq \intpt{N/2}} a_k e^{k x i}$,
where $\intpt{N/2}$ is the largest integer less than or equal to $N/2$.
\item \label{approx_compl_rmks_lbl1} Let
$\DS p(x) = \sum_{|k|\leq K} r_k e^{k x i}$
...
$\DS x_n = \frac{2n \pi}{N}$ for $0 \leq n \leq N$, we get
\[
...
\]
In particular, if $\DS g(x) = f(x) e^{-k x i}$ with $f$ a
$2\pi$-periodic complex valued function, we get
\[
...
$\DS a_k = \sum_{j\equiv k \pmod{N}} A_j$ are almost negligible.
\end{rmk}

\begin{egg}
Find the trigonometric polynomial
$\DS p(x) = \sum_{|k|\leq 1} b_k e^{kxi}$ that interpolates
$\DS f(x) = \sin^2(x)$ at $x_n = 2n\pi/3$ for $n=0$, $1$ and $2$.
...
$\DS p(x) = \sum_{|k|\leq K} b_k e^{kxi}$.  Suppose that
$f$ is a $2\pi$-periodic continuous function and let
$\DS \dist{f}{\Pi_K} = \inf_{q\in \Pi_K} \|f - q\|_\infty$, where
$\DS \|h\|_\infty = \max_{0\leq x \leq 2\pi} |h(x)|$ for
all continuous function $h:[0,2\pi]\rightarrow \CC$.  If
$\DS p(x) = \sum_{|k|\leq K} r_k e^{kxi}$ is the
...
Moreover, if $f$ is $j$-times differentiable with
$\DS f^{(j)}$ piecewise
continuous, one can prove that
$\DS |A_k| = O(|k|^{-j-1})$ and that this implies
that $\DS \dist{f}{\Pi_K} = O(K^{-j})$ \cite{CdB}.
\end{rmk}
...
of period $N$ and let $\omega_N$ be the $\DS N^{th}$ root of
unity defined by $\DS \omega_N = e^{2\pi i/N}$.  The
...
of $\FF_N$ is the mapping
$\DS \FF_N^{-1}: \Pi_N \rightarrow \Pi_N$ such
that $\DS \fct{x} = \FF_N^{-1} \fct{y}$ is defined by
\[
...
the infinite sequences $\DS \{x_n\}_{n\in\ZZ}$ and
$\DS \{y_n\}_{n\in\ZZ}$ respectively.  We do not use this
...
these applications in these lecture notes.   Henrici \cite{He} has a nice
...
$\DS \fct{y}^\ast = \FF^\ast_N \fct{x}$ is given by
\[
\fct{y}^\ast(n) \equiv \sum_{k=0}^{N-1} \omega_N^{-nk} \fct{x}(k)
\]
for $n \in \ZZ$.  In many books, $\DS \FF^\ast_N$ is used as
...
\]
for $n \in \ZZ$.
\item Suppose that $N = P_1 P_2 P_3 \cdots P_m$ and let
$N = A_k  P_k B_k$, where $A_k=P_1 P_2 \cdots P_{k-1}$ and
$B_k=P_{k+1} P_{k+2} \cdots P_m$.  If $k=0$, we set $P_k = 1$ and
$A_k = 1$.  If $k=m$, we set $B_k=1$. 

For each $q \in \ZZ$, we define the function
$\DS \fct{y}^\ast_{A_k P_k,q} \in \Pi_{B_k}$ by
\[
...
\end{equation}
for $b \in \ZZ$.
\item $\DS \fct{y}^\ast_{A_k P_k, q}$ can be used to define
the function 
\begin{align*}
...
\begin{rmkList}
\begin{enumerate}
\item For $k=m$, we have the special case $A_k=P_1 P_2 \cdots P_{m-1}$,
...
\]
for $q \in \NN$.  Namely, $\DS \fctt{y}_m(q,0) = \fct{x}(q)$ for
$q \in \NN$.
...
\]
for $b \in \NN$.  Namely,
$\DS \fctt{y}_0(0,b) = \FF^\ast_N \fct{x}(b)$
for $b \in \NN$.
\end{enumerate}
\end{rmkList}
...
\end{align*}
because $\DS \omega_{P_kB_k}^{-mP_kB_k} = 1^{-m} = 1$ for
all $m\in \ZZ$.  Thus
\[
\sum_{s=0}^{P_k-1} \fctt{y}_k(a+A_k s, b) \omega_{P_k B_k}^{-s(b+B_k p)}
...
assume that $N = P_1 P_2 \cdots P_m$.\\
\subI{Input} The vector $(P_1,P_2, \cdots P_m)$ and the column vector
...
$\DS \fct{y}^\ast= \FF^\ast_N \fct{x}$ which is
denoted \verb!Z! in the code below.  As for the input, only the
components $\DS \fct{y}^\ast(j)$ for $0 \leq j < N$ are returned
because of the periodicity of $\DS \fct{y}^\ast$.
\small
\begin{verbatim}
function Z = FFT(X,MP)
  m = length(MP);
...
$\DS \omega_{P_k B_k}^{-(b+B_k p)}$ to evaluate the expression
...
in the code above to compute $\DS \FF^\ast_N \fct{x}$.  The evaluation
of omega and copying data has been ignored when computing the number
of operations.  The number of operations to compute
$\DS \FF^\ast_N\fct{x}$ directly from the definition is
about $\DS N^2$.
This is much larger than $N(P_1+P_2+\ldots+P_m)$ in general.
\item When $\DS N=2^m$, an efficient Fast Fourier Transform
algorithm can be developed.  It is probably the must often used Fast Fourier
Transform algorithm.
\begin{align*}
\FF^\ast_N \fct{x}(n)
...
for $k \in \NN$.  We have used the relation
$\DS \omega_{2^m}^{2n} = \omega_{2^{m-1}}^n$ to
get the fourth equality.  Moreover, since
$\DS \omega_{2^m}^{-j-2^{m-1}} = -\omega_{2^m}^{-j}$ for
$\DS 0 \leq j < 2^{m-1}$, we get that
\begin{align*}
\FF^\ast_N \fct{x}(n)
...
\omega_{2^m}^{-n} \, \FF^\ast_{2^{m-1}} \fct{x}_{o}(n)
\end{align*}
for $\DS 0 \leq n < 2^{m-1}$.  The last equality, comes from
the fact that $\DS \FF^\ast_{2^{m-1}} \fct{x}_{e}$ and
$\DS \FF^\ast_{2^{m-1}} \fct{x}_{o}$ are of period
$\DS 2^{m-1}$ because $\fct{x}_{e}$ and $\fct{x}_{o}$ are of
period $\DS 2^{m-1}$.  This gives the following simple algorithm.
...
components $\DS \fct{z}^\ast(j)$ for $0 \leq j < N$ are
returned because of the periodicity of $\DS \fct{z}^\ast$.
\small
\begin{verbatim}
